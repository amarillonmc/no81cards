...
function c67200302.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOHAND+CATEGORY_SEARCH+CATEGORY_LEAVE_GRAVE)
...
function c67200302.spfilter(c,tp)
	return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_ONFIELD) and (c:IsSetCard(0x3675) or c:IsSetCard(0x3674)) and c:IsType(TYPE_MONSTER)
end
...
function c67200302.setfilter(c)
	return (c:IsSetCard(0x3675) or c:IsSetCard(0x3674)) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c67200302.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 and Duel.IsExistingMatchingCard(c67200302.setfilter,tp,LOCATION_GRAVE+LOCATION_EXTRA+LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(67200302,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c67200302.setfilter),tp,LOCATION_GRAVE+LOCATION_EXTRA+LOCATION_DECK,0,1,1,nil)
