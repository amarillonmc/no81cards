...
function cm.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_RECOVER+CATEGORY_TOHAND+CATEGORY_SEARCH)
...
function cm.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		local thg=Duel.GetMatchingGroup(function(c)return c:IsSetCard(0xcac) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()end,tp,LOCATION_DECK,0,nil)
		if (lv%2==0 and Duel.SelectYesNo(tp,aux.Stringid(m,1))) or (lv%2~=0 and #thg>0 and Duel.SelectYesNo(tp,aux.Stringid(m,2))) then
			Duel.BreakEffect()
			if lv%2==0 then Duel.Recover(tp,1000,REASON_EFFECT) else
