...
function s.ovfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x3a76)
end
...
function s.thfilter(c)
	return c:IsSetCard(0x3a76) and c:IsType(TYPE_QUICKPLAY) and c:IsSSetable()
end
...
function s.ovcon(e,tp,eg,ep,ev,re,r,rp)
	return re:IsActiveType(TYPE_XYZ) and re:GetHandler():IsSetCard(0x3a76) and rp==tp
end
