...
function cm.filter2(c)
	return c:IsSetCard(0xa629) and c:IsLevelBelow(5) and c:IsAbleToHand()
end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)~=0 then
		...
		local num=Duel.GetFlagEffect(tp,m)
		if byd.link(e:GetHandler(),6) and Duel.IsExistingMatchingCard(cm.filter2,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
