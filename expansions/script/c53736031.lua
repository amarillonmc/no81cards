...
function cm.filter(c)
	return c:IsCanAddCounter(0x9539,1) and c:IsFaceup() and c:IsSetCard(0x5539) and c:IsType(TYPE_SPELL) and c:GetSequence()<5
end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	while tc do
		tc:AddCounter(0x9539,1)
		tc=g:GetNext()
	end
end
function cm.hfilter(c)
	return c:IsFaceup() and c:IsCode(53736023)
end
