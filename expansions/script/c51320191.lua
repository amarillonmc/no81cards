...
function cm.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetCustomActivityCount(m,tp,ACTIVITY_SPSUMMON)==0 and not e:GetHandler():IsPublic() end
...
function cm.ffilter(c,e,tp)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x3a04) 
		and Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,c,e,tp)
end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		local g=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,tc,e,tp)
