...
function cm.ovfilter1(c)
	return c:IsFaceup() and c:IsRace(RACE_REPTILE) and aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_FMATERIAL)
end
function cm.filter(c,e,tp,mc)
	return (c:IsType(TYPE_FUSION) and c:IsLevelBelow(7) and c:IsRace(RACE_FIEND+RACE_DRAGON+RACE_PLANT) and c:IsAttackBelow(2600))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0 and aux.MustMaterialCheck(mc,tp,EFFECT_MUST_BE_FMATERIAL)
end
