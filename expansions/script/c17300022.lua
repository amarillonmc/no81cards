...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,TYPE_MONSTER+TYPE_EFFECT,0,0,1,RACE_ILLUSION,ATTRIBUTE_LIGHT) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		c:AddMonsterAttribute(TYPE_EFFECT+TYPE_MONSTER,ATTRIBUTE_LIGHT,RACE_ILLUSION,1,0,0)
...
function s.effcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsCanRemoveCounter(tp,1,0,0x1f50,2,REASON_COST) end
	Duel.RemoveCounter(tp,1,0,0x1f50,2,REASON_COST)
end
