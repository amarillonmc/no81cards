...
function c22021140.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(22021140,5))
	e2:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetCountLimit(1)
	e2:SetCondition(c22021140.erecon)
	e2:SetCost(c22021140.erecost)
	e2:SetTarget(c22021140.target)
	e2:SetOperation(c22021140.operation)
	c:RegisterEffect(e2)
end
...
function c22021140.filter(c)
	return c:IsSetCard(0x3ff1) and not c:IsCode(22020000) and c:IsType(TYPE_MONSTER) and (c:IsAbleToHand() or chk and c:IsCanBeSpecialSummoned(e,0,tp,false,false))
end
...
function c22021140.erecon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsPlayerAffectedByEffect(tp,22020980)
end
function c22021140.erecost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)
