...
function s.pend_core_op(e,tp,eg,ep,ev,re,r,rp)
	...
	local b1=s.c_disable_chk(e,tp,c,nil)
	local b2=s.c_place_chk(e,tp,c,nil)
	if not b1 and not b2 then return end

	local op
	if b1 and b2 then
		op=Duel.SelectOption(tp,aux.Stringid(CARD_MANAGER,2),aux.Stringid(CARD_MANAGER,3))
	elseif b1 then
		op=0
	else
		op=1
	...
	if op==0 then
		s.register_add_option(tp,CARD_CARBON_CRAB,ADD_C_PLACE,
			aux.Stringid(CARD_MANAGER,3),s.c_place_chk,s.c_place_tg,s.c_place_op,c)
		s.c_disable_op(e,tp,c,nil)
	else
		s.register_add_option(tp,CARD_CARBON_CRAB,ADD_C_DISABLE,
			aux.Stringid(CARD_MANAGER,2),s.c_disable_chk,s.c_disable_tg,s.c_disable_op,c)
		s.c_place_op(e,tp,c,nil)
	end
end

--==============================
-- Additional option A: 选场上1张卡的效果直到回合结束时无效
--==============================
function s.disfilter(c,e)
	return aux.NegateAnyFilter(c)
		and c:IsCanBeDisabledByEffect(e,false)
		and not c:IsImmuneToEffect(e)
end

function s.c_disable_chk(e,tp,source,fc)
	return Duel.IsExistingMatchingCard(s.disfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,e)
end

function s.c_disable_tg(e,tp,source,fc)
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,nil,1,PLAYER_ALL,LOCATION_ONFIELD)
end

function s.c_disable_op(e,tp,source,fc)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_NEGATE)
	local g=Duel.SelectMatchingCard(tp,s.disfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,e)
	local tc=g:GetFirst()
	if not tc then return end

	if tc:IsFaceup()
		and aux.NegateAnyFilter(tc)
		and tc:IsCanBeDisabledByEffect(e,false)
		and not tc:IsImmuneToEffect(e) then

		Duel.HintSelection(g)
		Duel.NegateRelatedChain(tc,RESET_TURN_SET)

		local rc=source or e:GetHandler()
...
--==============================
-- Additional option B: 放置「化形兽」灵摆怪兽
--==============================
...
function s.c_place_chk(e,tp,source,fc)
	return s.pzone_available(tp)
		and Duel.IsExistingMatchingCard(s.pplace_filter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil)
end

function s.c_place_tg(e,tp,source,fc)
	--Duel.SetOperationInfo(0,CATEGORY_TOFIELD,nil,1,tp,LOCATION_DECK+LOCATION_EXTRA)
end

function s.c_place_op(e,tp,source,fc)
	if not s.pzone_available(tp) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local g=Duel.SelectMatchingCard(tp,s.pplace_filter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,1,nil)
	local tc=g:GetFirst()
	if tc and s.pzone_available(tp) then
		Duel.HintSelection(g)
...
function s.leavefilter(c,tp)
	return c:IsPreviousControler(tp)
		...
		and aux.IsCodeListed(c,CARD_CARBON_CRAB)
end
