...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if b1 and b2 then
		if Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0 then
			op=Duel.SelectOption(tp,aux.Stringid(31401001,0),aux.Stringid(31401001,1),aux.Stringid(31401001,2))
		else
			op=Duel.SelectOption(tp,aux.Stringid(31401001,0),aux.Stringid(31401001,1))
		end
	elseif b1 then
		op=Duel.SelectOption(tp,aux.Stringid(31401001,0))
	else
		op=Duel.SelectOption(tp,aux.Stringid(31401001,1))+1
	end
	e:SetLabel(op)
...
function cm.filter1(c,e,tp)
	return c:IsCode(31401005) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
