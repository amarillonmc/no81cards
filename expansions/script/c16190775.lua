...
function s.distg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if e:GetHandler():IsStatus(STATUS_SET_TURN) and e:GetLabel()==1 then
    	e:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY+CATEGORY_DISABLE_SUMMON)
    end
end
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
	if Duel.NegateActivation(ev) and e:GetHandler():IsStatus(STATUS_SET_TURN) and e:GetLabel()==1 
    	and Duel.GetFlagEffect(tp,id)==0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
    	Duel.BreakEffect()
		...
		e2:SetCountLimit(1)
		e2:SetCondition(s.sumdiscon)
		e2:SetOperation(s.sumdisop)
...
function s.sumdiscon(e,tp,eg,ep,ev,re,r,rp)
	return tp~=ep and Duel.GetCurrentChain()==0 and eg:IsExists(Card.IsSummonLocation,1,nil,LOCATION_GRAVE)
end
function s.sumdisop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,id)
	local dg=eg:Filter(Card.IsSummonLocation,nil,LOCATION_GRAVE)
...
function s.rcop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Recover(p,d,REASON_EFFECT)~=0 and Duel.IsExistingMatchingCard(s.atkfilter,tp,LOCATION_MZONE,0,1,nil) 
