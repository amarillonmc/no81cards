...
function cm.filter(c,e)
	return c:IsFaceup() and c:IsSetCard(0x351) and not c:IsImmuneToEffect(e)
end
function cm.atkop(e,tp,eg,ep,ev,re,r,rp)
	local sg=Duel.GetMatchingGroup(cm.filter,tp,LOCATION_MZONE,0,nil,e)
...
function cm.pctg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local t2=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,TAMA_OPTION_CODE,0,TYPES_TOKEN_MONSTER,c:GetAttack(),0,c:GetLevel(),c:GetRace(),c:GetAttribute())
...
function cm.pcop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif e:GetLabel()==2 then
		...
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not c:IsRelateToEffect(e) or c:IsFacedown()
			or not Duel.IsPlayerCanSpecialSummonMonster(tp,TAMA_OPTION_CODE,0,TYPES_TOKEN_MONSTER,atk,0,lv,race,att) then return end
