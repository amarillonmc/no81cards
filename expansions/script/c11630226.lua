...
function this.mgfilter(c,e,tp,fusc)
	return c:IsControler(tp) and c:IsLocation(LOCATION_GRAVE)
		and c:GetReason()&(REASON_LINK+REASON_MATERIAL)==(REASON_LINK+REASON_MATERIAL) and c:GetReasonCard()==fusc
...
function this.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoDeck(tc,nil,SEQ_DECKTOP,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_EXTRA)
		and tc:IsSummonType(SUMMON_TYPE_LINK)
