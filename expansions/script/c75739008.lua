...
function s.thfilter(c,e,tp,sg)
	...
	return c:IsSetCard(0xc31) and c:IsType(TYPE_MONSTER) and Duel.GetLocationCount(tp,LOCATION_SZONE,tp,r)>0
		and sg:IsExists(s.tgfilter,1,c,e,tp)
end
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return sg:IsExists(s.thfilter,1,nil,e,tp,sg) end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=sg:FilterSelect(tp,s.thfilter,1,1,nil,e,tp,sg):GetFirst()
	if tc then
	...
	local tg=sg:FilterSelect(tp,s.tgfilter,1,1,nil,e,tp)
...
function s.sztg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(Card.IsAbleToDeck,tp,0,0x3c,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,1-tp,0x3c)
end
function s.szop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,CATEGORY_DISABLE)
	local tc=Duel.SelectMatchingCard(tp,Card.IsAbleToDeck,tp,0,0x3c,1,1,nil):GetFirst()
	if tc then
