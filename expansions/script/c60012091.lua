...
function s.discardfilter(c)
	return c:IsSetCard(0x5624) or (c:IsRace(RACE_FIEND) and c:IsType(TYPE_MONSTER))
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.discardfilter,tp,LOCATION_HAND,0,1,e:GetHandler())
