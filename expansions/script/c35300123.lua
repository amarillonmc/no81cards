...
function cm.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (tc:IsReason(REASON_EFFECT) and rp==1-tp) and tc:IsPreviousControler(tp)
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		if Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)~=0 and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_ONFIELD,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
