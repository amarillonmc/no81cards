...
function cm.chtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		if rp==tp then
			local chkf=tp
			local mg1=Duel.GetMatchingGroup(cm.filter0,tp,LOCATION_ONFIELD,0,aux.ExceptThisCard(e))
...
function cm.repop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetMatchingGroup(cm.filter1,tp,LOCATION_ONFIELD,0,aux.ExceptThisCard(e),e)
