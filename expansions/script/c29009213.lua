...
function c29009213.fieldcheck(c)
	return c:IsFaceup() and c:IsCode(29065510)
end
function c29009213.spcheck(c,e,tp)
	return (c:IsFaceupEx() or c:IsLocation(LOCATION_DECK)) and c:IsCode(29065502) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c29009213.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.IsExistingMatchingCard(c29009213.fieldcheck,tp,LOCATION_FZONE,0,1,nil) and Duel.IsExistingMatchingCard(c29009213.spcheck,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	local b2=not Duel.IsExistingMatchingCard(c29009213.fieldcheck,tp,LOCATION_FZONE,0,1,nil) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c29009213.filter),tp,LOCATION_GRAVE+LOCATION_DECK+LOCATION_REMOVED,0,1,nil,tp)
...
function c29009213.acop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif b1 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c29009213.spcheck),tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,e,tp)
