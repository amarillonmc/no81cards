...
function cm.pcon(e,c)
	...
	if c:IsLocation(LOCATION_PZONE) and not Duel.GetMatchingGroup(nil,tp,LOCATION_PZONE,0,c):GetFirst():IsAttribute(ATTRIBUTE_WIND) then return false end
	local canp=false
...
function cm.lffilter(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_WIND) and c:IsCanBeSpecialSummoned(e,0,tp,true,false) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
