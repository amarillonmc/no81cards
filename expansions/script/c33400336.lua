...
function cm.filter(c,e,tp)
	return c:IsSetCard(0x9342) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
		  ...
		  if Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,LOCATION_HAND+LOCATION_GRAVE,LOCATION_GRAVE,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then 
		   Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		  local rg=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,LOCATION_HAND+LOCATION_GRAVE,LOCATION_GRAVE,0,1,1,nil)
