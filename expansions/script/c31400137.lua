...
function cm.initial_effect(c)
	...
	e4:SetTargetRange(0xfe,0xfe)
	e4:SetValue(LOCATION_DECK)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	e5:SetTargetRange(0x1,0x1)
...
function cm.pcon(e,c)
	...
	if c:IsLocation(LOCATION_PZONE) and not Duel.GetMatchingGroup(nil,tp,LOCATION_PZONE,0,c):GetFirst():IsAttribute(ATTRIBUTE_WATER) then return false end
	local canp=false
...
function cm.lffilter(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_WATER) and c:IsCanBeSpecialSummoned(e,0,tp,true,false) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
