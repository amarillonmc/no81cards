...
TAMA_CODE_ADD_ELEMENT=13254030
...
function tama.tamas_getElements(v)
	local codes=tama.tamas_getOriginalElements(v)
	if aux.GetValueType(v)=="Card" and v:IsHasEffect(TAMA_CODE_ADD_ELEMENT) then
		codes1=tama.DeepCopy(codes)
...
function tama.tamas_getOriginalElements(v)
	local codes={}
...
--[[ 检查 codes 和 check 两个表中是否存在至少一个共同的元素，并且对于这个元素，codes 中的值不大于 check 中的对应值。
exist A <= exist B
...
--[[ 判断codes中的所有元素是否都不大于check中对应元素的值，反过来说就是确认check中的所有元素是否包含codes中全部种类和数量
...
--从g中筛选出拥有codes中的元素但是对应元素数量不大于codes中的数量的卡。可用于选一组在需求元素数量以下的卡。
...
--[[
判断codes中的所有元素是否都大于targetCodes中对应元素的值
...
--判断card中的所有元素是否都大于targetCard中对应元素的值
