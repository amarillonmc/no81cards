...
function s.ef1op(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(s.desfilter,tp,0,LOCATION_ONFIELD,1,nil) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local tc=Duel.SelectMatchingCard(tp,s.desfilter,tp,0,LOCATION_ONFIELD,1,1,nil):GetFirst()
	Duel.Destroy(tc,REASON_EFFECT)
end
function s.destory(c)
	local custom_code=aux.RegisterMergedDelayedEvent_ToSingleCard(c,id,EVENT_SSET)
...
function s.setfilter(c,tp,e)
	return c:IsControler(1-tp) and c:IsFacedown() and c:IsCanBeEffectTarget(e)
end
