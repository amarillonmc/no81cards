...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,m+1,0x876,TYPES_TOKEN_MONSTER,800,1200,3,RACE_MACHINE,ATTRIBUTE_EARTH) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not Duel.IsPlayerCanSpecialSummonMonster(tp,m+1,0x876,TYPES_TOKEN_MONSTER,800,1200,3,RACE_MACHINE,ATTRIBUTE_EARTH) then return end
...
function cm.tdfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x876) and c:IsAbleToDeck()
end
