...
function cm.spfilter(c,e,tp,lv)
	return c:IsLevel(lv) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_ATTACK)
end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsAttribute(ATTRIBUTE_WATER) and tc:IsLevelAbove(6) and tc:IsAbleToHand() then
		...
		if Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 then
			...
			if tg:GetCount()>=tc:GetLevel() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp,lv) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
				Duel.BreakEffect()
				...
				if Duel.SendtoDeck(stg,nil,0,REASON_EFFECT)~=0 then
					...
					local g=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp,lv)
