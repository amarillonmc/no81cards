...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_TOGRAVE+CATEGORY_TOHAND+CATEGORY_DISABLE)
...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,nil,c)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,nil,c)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,c)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	--这张卡以外的场上1张表侧卡回到手卡
	local g=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil,c)
	if #g>0 then
		...
		if Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 then
			s.registerdisable(c,tp,code)
			returned=true
		end
	end

	--那之后：这张卡以及相同纵列的所有表侧怪兽的卡名当做「底噪石窟」使用
	if returned and c:IsFaceup() and c:IsOnField() then
...
function s.registerdisable(c,tp,code)
