...
local cm=_G["c"..m]
local flag=true
function c33332205.initial_effect(c)
		--Activate
...
function cm.filter(c,e,tp)
	return ((c:IsSetCard(0x3568) and c:IsControler(tp)) or c:IsControler(1-tp)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		local tc=g:GetFirst()
		local tpc=tc:GetOwner()
		if Duel.SpecialSummon(tc,0,tp,tpc,false,false,POS_FACEUP)~=0 then
			local e1=Effect.CreateEffect(c)
