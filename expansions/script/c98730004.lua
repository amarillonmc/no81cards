...
function c98730004.initial_effect(c)
    ...
    e1:SetDescription(aux.Stringid(98730004,0))
    ...
    e1:SetTarget(c98730004.pctg)
    e1:SetOperation(c98730004.pcop)
    c:RegisterEffect(e1)
    --spsummon
    local e1=Effect.CreateEffect(c)
    ...
    e3:SetCondition(c98730004.spcon2)
    e3:SetTarget(c98730004.sptg2)
    e3:SetOperation(c98730004.spop2)
    c:RegisterEffect(e3)
end
function c98730004.psfilter(c)
    return c:IsCode(98730015,98730415) and not c:IsForbidden()
end
function c98730004.pctg(e,tp,eg,ep,ev,re,r,rp,chk)
    local seq=e:GetHandler():GetSequence()
    if chk==0 then return (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1))
        and Duel.IsExistingMatchingCard(c98730004.psfilter,tp,LOCATION_DECK,0,1,nil) end
end
function c98730004.pcop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.SelectMatchingCard(tp,c98730004.psfilter,tp,LOCATION_DECK,0,1,1,nil,tp)
    if g then
...
function c98730004.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(c98730004.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
    Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c98730004.spop2(e,tp,eg,ep,ev,re,r,rp)
    if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
    local tc=Duel.GetFirstMatchingCard(c98730004.spfilter,tp,LOCATION_DECK,0,nil,e,tp)
    if tc and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
        local e1=Effect.CreateEffect(e:GetHandler())
