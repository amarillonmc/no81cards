...
function c22348263.stfilter(c)
	return ((c:GetType()==0x20002 or c:GetType()==0x20004) and c:IsSetCard(0x70a3) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsEnvironment(22348260)) or (c:GetType()==0x80002 and c:IsCode(22348260)) and c:CheckActivateEffect(false,true,false)~=nil
end
...
function c22348263.cfilter(c,tp,rp)
	return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousTypeOnField(),TYPE_MONSTER)~=0
		and c:IsPreviousSetCard(0x70a3) and (c:IsReason(REASON_BATTLE) or (rp==1-tp and c:IsReason(REASON_EFFECT)))
end
