...
function c11560703.xspfil(c,e,tp) 
	...
	return c:IsType(TYPE_XYZ) and not c:IsRace(mc:GetRace()) and c:IsAttribute(mc:GetAttribute()) and c:IsRank(mc:GetRank()) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0  
end 
