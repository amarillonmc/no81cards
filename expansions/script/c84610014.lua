...
function c84610014.initial_effect(c)
    ...
    e1:SetCondition(c84610014.condition)
    e1:SetCost(c84610014.cost) 
    ...
    e2:SetCountLimit(1) 
    e2:SetCost(c84610014.spcost)
    e2:SetTarget(c84610014.target1)
    e2:SetOperation(c84610014.activate1)
    c:RegisterEffect(e2)
    --special summon
    ...
    e3:SetCondition(c84610014.spcon)
    e3:SetTarget(c84610014.target)
    e3:SetOperation(c84610014.operation)
    c:RegisterEffect(e3)
end
function c84610014.matfilter(c)
    return c:IsLinkSetCard(0x10) and not c:IsLinkType(TYPE_LINK)
end
function c84610014.condition(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsSummonType(SUMMON_TYPE_LINK)
end
function c84610014.cosfilter(c)
    return c:IsSetCard(0x10) and c:IsAbleToGraveAsCost()
end
function c84610014.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c84610014.fselect(c,tp,rg,sg,tc)
    sg:AddCard(c)
    if sg:GetCount()<2 then
        res=rg:IsExists(c84610014.fselect,1,sg,tp,rg,sg,tc)
    else
        res=c84610014.fgoal(tp,sg,tc)
    end
    sg:RemoveCard(c)
    return res
end
function c84610014.fgoal(tp,sg,tc)
    if sg:GetCount()>0 and Duel.GetLocationCountFromEx(tp,tp,sg,tc)>0 then
...
function c84610014.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if chk==0 then return rg:IsExists(c84610014.fselect,1,nil,tp,rg,g,c) end
    while g:GetCount()<2 do
        Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
        local sg=rg:FilterSelect(tp,c84610014.fselect,1,1,g,tp,rg,g,c)
        g:Merge(sg)
    end
...
function c84610014.filter(c,e,tp)
    return c:IsCode(84610014) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LINK,tp,false,false)
end
function c84610014.target(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCountFromEx(tp)>0
        and Duel.IsExistingMatchingCard(c84610014.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
    Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function c84610014.operation(e,tp,eg,ep,ev,re,r,rp)
    if Duel.GetLocationCountFromEx(tp)<=0 then return end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
    local g=Duel.SelectMatchingCard(tp,c84610014.filter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
    if g:GetCount()>0 then
