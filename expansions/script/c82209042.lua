...
function cm.negfilter(c)  
	return c:IsType(TYPE_DUAL) and c:IsFaceup() and cm.isKieju(c:GetCode()) and not c:IsDualState()
end  
function cm.negtg(e,tp,eg,ep,ev,re,r,rp,chk)  
	if chk==0 then return Duel.IsExistingMatchingCard(cm.negfilter,tp,LOCATION_MZONE,0,1,nil) end  
end  
function cm.negop(e,tp,eg,ep,ev,re,r,rp)   
	...
	local tc=g:GetFirst()  
	if tc then tc:EnableDualState() end
	if tc:IsDualState() and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then  
