...
function s.filter2(c)
	return c:IsAbleToDeck() and (c:IsSetCard(0x607) or c:IsType(TYPE_TRAP))
end
function s.prop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
	if Duel.GetLP(tp)-Duel.GetLP(1-tp)>=2000 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.filter2),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,3,nil) and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
