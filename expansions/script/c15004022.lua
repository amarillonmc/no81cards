...
function cm.spfilter(c)
	return c:IsAbleToHand() and c:IsCode(15004030,15004032,15004034,15004036)
end
...
function cm.sp2op(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)~=0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>=1 and g:GetFirst():IsSSetable() and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
			Duel.BreakEffect()
			Duel.SSet(tp,g)
		end
	end
