...
function s.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.eqfilter,tp,LOCATION_GRAVE,0,1,nil)  end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SendtoHand(tc,nil,REASON_EFFECT)
		...
		if Duel.IsExistingMatchingCard(s.eqfilter3,tp,LOCATION_MZONE,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
		Duel.BreakEffect()
		local eqc=Duel.SelectMatchingCard(tp,s.eqfilter2,tp,LOCATION_HAND,0,1,1,nil):GetFirst()
