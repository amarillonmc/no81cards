...
function c91300063.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=c91300063.thop(e,tp,eg,ep,ev,re,r,rp,0)
	local b2=false
	local b3=c91300063.cpop(e,tp,eg,ep,ev,re,r,rp,0)
	local b4=c91300063.acop(e,tp,eg,ep,ev,re,r,rp,0)
	local b5=c91300063.drop(e,tp,eg,ep,ev,re,r,rp,0)
	--local b6=false
...
function c91300063.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:IsActivated() then
		if dc~=1 then Crossroads_dice_effect_list[aux.Stringid(91300063,0)]=c91300063.thop end
		if dc~=2 then Crossroads_dice_effect_list[aux.Stringid(91300063,1)]=c91300063.rdop end
		if dc~=3 then Crossroads_dice_effect_list[aux.Stringid(91300063,2)]=c91300063.cpop end
		if dc~=4 then Crossroads_dice_effect_list[aux.Stringid(91300063,3)]=c91300063.acop end
		if dc~=5 then Crossroads_dice_effect_list[aux.Stringid(91300063,4)]=c91300063.drop end
		if dc~=6 then Crossroads_dice_effect_list[aux.Stringid(91300063,5)]=c91300063.slop end
		if dc~=99 then Crossroads_dice_effect_list[aux.Stringid(91300063,6)]=c91300063.lpop end
	end
	if dc==1 then
		c91300063.thop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==2 then
		c91300063.rdop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==3 then
		c91300063.cpop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==4 then
		c91300063.acop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==5 then
		c91300063.drop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==6 then
		c91300063.slop(e,tp,eg,ep,ev,re,r,rp,1)--return
	elseif dc==99 then
		c91300063.lpop(e,tp,eg,ep,ev,re,r,rp,1)
	end
end
function c91300063.thop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(c91300063.thfilter,tp,LOCATION_DECK,0,1,nil,0)
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,0)]=c91300063.thop
...
function c91300063.rdop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return c91300063.target(e,tp,eg,ep,ev,re,r,rp,0)
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,1)]=c91300063.rdop
		c91300063.activate(e,tp,eg,ep,ev,re,r,rp)
	end
end
function c91300063.cpop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local res=false
		...
		Crossroads_dice_except_list[aux.Stringid(91300063,2)]=c91300063.cpop
		local codes={}
		for _,code in pairs({91300063,91300065,91300067,91300069,91300071,91300073,91300075,91300077,91300079,91300081,91300083}) do
			local tc=Crossroads_card_list[code]
...
function c91300063.acop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsExistingMatchingCard(c91300063.acfilter,tp,LOCATION_DECK,0,1,nil,tp)
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,3)]=c91300063.acop
...
function c91300063.drop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.IsPlayerCanDraw(tp,2)
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,4)]=c91300063.drop
		Duel.Draw(tp,2,REASON_EFFECT)
	end
end
function c91300063.slop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return c91300063.target(e,tp,eg,ep,ev,re,r,rp,0)
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,5)]=c91300063.slop
		local b1=c91300063.thop(e,tp,eg,ep,ev,re,r,rp,0)
		local b2=true
		local b3=c91300063.cpop(e,tp,eg,ep,ev,re,r,rp,0)
		local b4=c91300063.acop(e,tp,eg,ep,ev,re,r,rp,0)
...
function c91300063.lpop(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Dead_Town_99_Check
	else
		Crossroads_dice_except_list[aux.Stringid(91300063,6)]=c91300063.lpop
