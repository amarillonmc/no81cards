...
function c22020660.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SelectOption(tp,aux.Stringid(22020660,0))
end
...
function c22020660.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>=1 then
		Duel.SelectOption(tp,aux.Stringid(22020660,1))
	end
	if ct>=2 then
		local dg=Duel.GetMatchingGroup(c22020660.desfilter,tp,0,LOCATION_MZONE,nil)
		if dg:GetCount()>0 then
			Duel.SendtoGrave(dg,REASON_EFFECT)
			Duel.SelectOption(tp,aux.Stringid(22020660,2))
		end
	end
	if ct>=3 then
		Duel.Damage(1-tp,3000,REASON_EFFECT)
		Duel.SelectOption(tp,aux.Stringid(22020660,3))
	end
end