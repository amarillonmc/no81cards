...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    local b1 = Duel.IsExistingTarget(Card.IsType,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,TYPE_MONSTER)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if op==0 then
        ...
        if tc and tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)>0 then
            local ct = math.min(2, Duel.GetFieldGroupCount(tp,0,LOCATION_DECK))
            if ct>0 then
                Duel.BreakEffect()
