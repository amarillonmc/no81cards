...
function s.regop(e,tp,eg,ep,ev,re,r,rp)
	for tc in aux.Next(eg) do
		...
		local tup=tc:GetSummonPlayer()
		if Duel.GetTurnCount()~=s[2] then
			s[0]={}
			s[1]={}
			s[2]=Duel.GetTurnCount()
		end
		if Duel.GetFlagEffect(tup,88884398)>0 then return end
		if tc:IsSetCard(0x893) then
			e:SetLabel(0)
			...
			if e:GetLabel()==0 then table.insert(s[tup],ccode) end
			if #s[tup]>=3 then 
				Duel.RegisterFlagEffect(tup,88884398,RESET_PHASE+PHASE_END,0,0)
			end
		end
	end
end
...
function s.cfilter2(c)
	return c:IsFacedown() or (not c:IsSetCard(0x893))
end
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	return ep==1-tp
end
function s.cfilter(c)
	return c:IsSetCard(0x893) and c:IsAbleToGraveAsCost()
end
