...
function c118776531.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c118776531.sumfilter,tp,LOCATION_HAND,0,1,nil)
		...
		e:SetLabel(1)
		Duel.RegisterFlagEffect(tp,118776531,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
	end
...
function c118776531.shop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if dg:GetCount()>0 and Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE,0,1,nil,57761191) and Duel.SelectYesNo(tp,aux.Stringid(118776531,1)) then
			Duel.BreakEffect()
