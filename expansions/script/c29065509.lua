...
function c29065509.initial_effect(c)
	...
	e2:SetCondition(c29065509.descon)
	...
	e3:SetCountLimit(1,29065512)
	e3:SetCondition(c29065509.secon)
	e3:SetTarget(c29065509.thtg)
	e3:SetOperation(c29065509.thop)
	c:RegisterEffect(e3)
end
...
function c29065509.descon(e,tp,eg,ep,ev,re,r,rp)
...
function c29065509.ffilter(c,chk)
	return c:IsCode(29065500) and c:IsFaceup()
end
function c29065509.secon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c29065509.ffilter,tp,LOCATION_MZONE,0,1,nil,true)
end
function c29065509.filter(c,e,tp,ft)
...
function c29065509.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c29065509.filter(chkc,e,tp,ft) end
	if chk==0 then return Duel.IsExistingTarget(c29065509.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp,ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c29065509.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp,ft)
end
function c29065509.thop(e,tp,eg,ep,ev,re,r,rp)
