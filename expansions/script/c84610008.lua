...
function c84610008.initial_effect(c)
    ...
    e4:SetDescription(aux.Stringid(73240432,0))
...
function c84610008.spfilter(c,tp)
...
function c84610008.hspcon(e,c)
    ...
    return Duel.IsExistingMatchingCard(c84610008.spfilter,tp,LOCATION_GRAVE,0,1,nil,tp)
end
function c84610008.hspop(e,tp,eg,ep,ev,re,r,rp,c)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
    local g=Duel.SelectMatchingCard(tp,c84610008.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,tp)
    Duel.SendtoDeck(g,nil,1,REASON_COST)
end
function c84610008.filter(c,e,tp)
...
function c84610008.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(c84610008.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
    Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c84610008.spop(e,tp,eg,ep,ev,re,r,rp)
    if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
    local g=Duel.SelectMatchingCard(tp,c84610008.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
    local tc=g:GetFirst()
...
function c84610008.tgfilter(c,tp)
    return c:IsSetCard(0x8) and c:IsType(TYPE_MONSTER)
end
function c84610008.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
    local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
    if chk==0 then return Duel.IsExistingMatchingCard(c84610008.tgfilter,tp,LOCATION_DECK,0,1,nil,e,tp,ft) end
end
function c84610008.spop2(e,tp,eg,ep,ev,re,r,rp)
    local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
    local g=Duel.SelectMatchingCard(tp,c84610008.tgfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp,ft)
    local tc=g:GetFirst()
    if tc then
