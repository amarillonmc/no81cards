...
function c43480065.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingTarget(c43480065.eqfilter,tp,LOCATION_MZONE,0,1,nil,tp) and Duel.IsExistingMatchingCard(c43480065.eqeqfilter,tp,LOCATION_DECK,0,1,nil,tp) 
	end
...
function c43480065.eqop(e,tp,eg,ep,ev,re,r,rp) 
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsControler(tp) then
		...
		local ec=Duel.SelectMatchingCard(tp,c43480065.eqeqfilter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
		if ec then
			Duel.Equip(tp,ec,tc) 
			--equip limit
			local e1=Effect.CreateEffect(c)
