...
function c29010009.cfilter(c,e,tp)
	return c:IsSetCard(0x77af) and c:IsType(TYPE_TRAP)
end
function c29010009.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP) then
		...
		if g2:GetCount()>0 and Duel.IsExistingMatchingCard(c29010009.setfilter,tp,0,LOCATION_MZONE,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(29010009,0)) then
			Duel.BreakEffect()
