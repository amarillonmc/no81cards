...
function s.filter(c,e,tp,zone,atk)
	return c:GetAttack()<atk and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,tp,zone)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local zone=Duel.GetLinkedZone(tp)
	local g1=Duel.GetMatchingGroup(Card.IsFaceup,tp,LOCATION_MZONE,0,nil)
	local g2=Duel.GetMatchingGroup(Card.IsFaceup,tp,0,LOCATION_MZONE,nil)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_DECK,0,1,nil,e,tp,zone,g2:GetSum(Card.GetAttack)-g1:GetSum(Card.GetAttack)) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp,zone,g2:GetSum(Card.GetAttack)-g1:GetSum(Card.GetAttack)):GetFirst()
