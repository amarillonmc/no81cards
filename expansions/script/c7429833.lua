...
function cm.zonelimit(e)
	...
	local zone=0
	for i=0,1 do
		if Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<i)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+1))>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+2))>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+3))>0 then
			zone=zone|(1<<i)
		end
	end
	return zone
...
function cm.splimit(e,c,tp)
	...
	if not zone_check or Duel.GetLocationCount(tp,LOCATION_MZONE)<3   
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429251,0,TYPES_TOKEN_MONSTER,1000,1000,2,RACE_FISH,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,2)
...
function cm.sumlimit(e,c,tp)
	...
	for i=0,1 do
		if Duel.GetFieldCard(tp,LOCATION_MZONE,i) then
			if not Duel.GetFieldCard(tp,LOCATION_MZONE,i):IsReleasable(c) or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+1))>0 or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+2))>0 or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+3))>0 or a==0 then
			else
				zone_check=true
			end
		else
			if Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<i)>0 or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+1))>0 or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+2))>0 or Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,1<<(i+3))>0 then 
			else
				zone_check=true
			end
		end
	...
	if not zone_check  
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429251,0,TYPES_TOKEN_MONSTER,1000,1000,2,RACE_FISH,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,1) then return false end
	return true
end
function cm.tokenop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,(1<<(e:GetHandler():GetSequence()+1)))>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,(1<<(e:GetHandler():GetSequence()+2)))>0 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,(1<<(e:GetHandler():GetSequence()+3)))>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,7429251,0,TYPES_TOKEN_MONSTER,1000,1000,2,RACE_FISH,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) and not Duel.IsPlayerAffectedByEffect(tp,59822133) then
		for i=1,3 do
			local token=Duel.CreateToken(tp,7429250+i)
...
function cm.btfilter(c)
	return c:IsFaceup() and c:IsCode(7429251)
end
...
function cm.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ag=eg:Filter(cm.actfilter,nil,tp)
	local c=e:GetHandler()
	local atk=ag:GetSum(Card.GetTextAttack)
	local def=ag:GetSum(Card.GetTextDefense)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
