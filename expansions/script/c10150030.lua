...
function c10150030.spfilter(c,e,tp,rc)
	return c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) and Duel.GetLocationCountFromEx(tp,tp,rc,c)>0
end
function c10150030.cfilter(c,e,tp)
	return Duel.GetLocationCountFromEx(tp,tp,c)>0 and Duel.IsExistingMatchingCard(c10150030.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function c10150030.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local b2=Duel.CheckReleaseGroup(tp,c10150030.cfilter,1,nil,e,tp)
	...
	if e:GetLabel()==100 then
	   e:SetLabel(0)
	   local g=Duel.SelectReleaseGroup(tp,c10150030.cfilter,1,1,nil,e,tp)
