...
function c33700322.tgfilter(c)
	return c:IsSetCard(0x1449) and (c:GetOriginalLevel()>0 or c:GetOriginalRank()>0) and c:IsAbleToGraveAsCost()
end
function c33700322.atcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c33700322.tgfilter,tp,LOCATION_SZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tc=Duel.SelectMatchingCard(tp,c33700322.tgfilter,tp,LOCATION_SZONE,0,1,1,nil):GetFirst()
