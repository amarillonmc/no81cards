...
function cm.initial_effect(c)
	...
	--[[e2:SetTarget(function(e,tp,eg,ep,ev,re,r,rp,chk)
					...
					if chk==0 then return c:GetFlagEffect(m)<3 end
...
function cm.thfilter(c,atk,race)
	return c:IsAbleToHand() and c:IsType(TYPE_MONSTER) and c:IsAttackBelow(atk) and c:IsRace(race)
end
...
function cm.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		local g=Duel.SelectMatchingCard(tp,cm.thfilter,tp,LOCATION_DECK,0,1,1,nil,atk,race)
		if #g>0 then
