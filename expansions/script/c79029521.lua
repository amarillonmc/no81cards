...
function c79029521.initial_effect(c)
	...
	e1:SetOperation(c79029521.ghop0)
	c:RegisterEffect(e1)
	local e1=Effect.CreateEffect(c)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	...
	e1:SetCode(EVENT_SPSUMMON_SUCCESS)
	e1:SetRange(LOCATION_MZONE)
	e1:SetOperation(c79029521.ghop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
...
function c79029521.ghop(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsContains(e:GetHandler()) then return end
	Duel.Hint(HINT_CARD,0,79029521)
	local c=e:GetHandler()
	local tc=eg:GetFirst()
	while tc do
	c:CopyEffect(tc:GetOriginalCode(),0,0)
	local e1=Effect.CreateEffect(c)
...
function c79029521.ghop0(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,79029521)
	local c=e:GetHandler()
