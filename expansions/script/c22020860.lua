...
function c22020860.filter(c)
	return (c:IsCode(22020850) or aux.IsCodeListed(c,22020850)) and c:IsAbleToHand()
end
function c22020860.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c22020860.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c22020860.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c22020860.filter,tp,LOCATION_DECK,0,1,1,nil)
