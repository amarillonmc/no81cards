...
function c22348103.efop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c22348103.immcon)
	e1:SetCost(c22348103.immcost)
	e1:SetOperation(c22348103.immop)
...
function c22348103.immcon(e,tp,eg,ep,ev,re,r,rp)
	return rp~=tp and  e:GetHandler():IsSummonType(SUMMON_TYPE_XYZ)
end
function c22348103.immcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c22348103.imfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x704)
end
function c22348103.immop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c22348103.imfilter,tp,LOCATION_MZONE,0,nil)
	local tc=g:GetFirst()
	while tc do
			local e1=Effect.CreateEffect(c)
			e1:SetType(EFFECT_TYPE_SINGLE)
			e1:SetCode(EFFECT_IMMUNE_EFFECT)
			e1:SetValue(c22348103.efilter)
			e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_CHAIN)
			tc:RegisterEffect(e1)
			tc:RegisterFlagEffect(c22348103,RESET_CHAIN,0,1)
			tc=g:GetNext()
	end
end
function c22348103.efilter(e,re)
