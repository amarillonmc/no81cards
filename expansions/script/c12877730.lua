...
function s.tdfilter(c,code)
	return not c:IsCode(code) and c:IsSetCard(0x5a73) and c:IsAbleToDeck() and c:IsFaceup() and c:IsType(TYPE_MONSTER)
end
function s.e1tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,1,nil,c:GetCode()) 
		and c:GetFlagEffect(id)==0 end
		Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_GRAVE+LOCATION_REMOVED)
end
function s.check(g)
	return g:IsExists(s.tdfilter,1,nil)
end
function s.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=g:SelectSubGroup(tp,s.check,false,1,5)
	if #sg>0 then
		Duel.HintSelection(sg)
		Duel.SendtoDeck(sg,nil,2,REASON_EFFECT) 
	end
	local og=Duel.GetOperatedGroup()
	if og:FilterCount(Card.IsLocation,nil,LOCATION_DECK+LOCATION_EXTRA)>=3 and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
		Duel.BreakEffect()
