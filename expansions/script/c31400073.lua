...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
		if c:GetMaterial():FilterCount(Card.IsType,nil,TYPE_EFFECT)~=c:GetMaterial():GetCount() and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
...
function cm.actlimit(e,re,rp)
	...
	return re:IsActiveType(TYPE_MONSTER) and rc:IsOnField() and rc:GetSummonLocation()==LOCATION_EXTRA and not rc:IsRace(RACE_WYRM)
end
