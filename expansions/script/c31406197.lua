...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local chk_1=Duel.IsExistingMatchingCard(cm.actfilter_1,tp,LOCATION_GRAVE,0,1,nil)
	local chk_2=Duel.IsExistingMatchingCard(cm.show_filter,tp,LOCATION_HAND,0,1,nil) and Duel.IsExistingMatchingCard(cm.actfilter_2,tp,LOCATION_GRAVE,0,1,nil)
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetMatchingGroup(cm.actfilter_1,tp,LOCATION_GRAVE,0,nil)
	if op==1 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
		local g=Duel.SelectMatchingCard(tp,cm.show_filter,tp,LOCATION_HAND,0,1,1,nil)
		Duel.ConfirmCards(1-tp,g)
		tg=Duel.GetMatchingGroup(cm.actfilter_2,tp,LOCATION_GRAVE,0,nil)
	end
...
function cm.btg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.bfilter_th,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_GRAVE)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_HAND)
end
function cm.bop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local th=Duel.SelectMatchingCard(tp,cm.bfilter_th,tp,LOCATION_GRAVE,0,1,1,nil):GetFirst()
	if th then
