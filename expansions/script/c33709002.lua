...
function cm.initial_effect(c)
		--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcFunRep(c,cm.ffilter,7,true)
...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,3,tp,0)
...
function cm.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local cadr3=Duel.CreateToken(tp,33709012)
...
function cm.changeop(e,tp)
	...
	if b then
		Duel.Win(1-tp,0x144f)
	end
end
function cm.check(c)
	return c:IsFaceup() and c:IsCode(33709003)
end
function cm.condition1(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(cm.check,tp,LOCATION_ONFIELD,0,1,nil)
end
...
function cm.operation1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local card1=Duel.CreateToken(tp,33709003)
	Duel.MoveToField(card1,tp,tp,LOCATION_SZONE,POS_FACEUP,true)
end