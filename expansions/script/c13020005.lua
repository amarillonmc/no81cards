...
function cm.initial_effect(c)
	...
	local e1=cm.AddContactFusionProcedure(c,cm.ffilter,LOCATION_ONFIELD+LOCATION_HAND,0,Duel.Remove,POS_FACEUP,REASON_COST+REASON_MATERIAL):SetValue(SUMMON_VALUE_SELF)
...
function cm.ContactFusionCondition(filter,self_location,opponent_location)
	return  function(e,c)
				...
				local mg=Duel.GetMatchingGroup(cm.ContactFusionMaterialFilter,tp,self_location,opponent_location,c,c,filter)
				return #mg>=2
			end
end
