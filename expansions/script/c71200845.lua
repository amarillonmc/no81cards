--禁じられた一滴
function c71200845.initial_effect(c)
	--Activate
	...
	e1:SetCountLimit(1,71200845+EFFECT_COUNT_CODE_OATH)
	e1:SetCondition(aux.dscon)
	e1:SetCost(c71200845.cost)
	e1:SetTarget(c71200845.target)
	e1:SetOperation(c71200845.activate)
	c:RegisterEffect(e1)
end
function c71200845.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_EFFECT)
end
function c71200845.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100)
	if chk==0 then return true end
end
function c71200845.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c71200845.filter,tp,0,LOCATION_MZONE,nil)
	if chk==0 then
	...
	if e:IsHasType(EFFECT_TYPE_ACTIVATE) then
		Duel.SetChainLimit(c71200845.chlimit(ctype))
	end
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,dg,cg:GetCount(),0,0)
end
function c71200845.chlimit(ctype)
	return function(e,ep,tp)
		return tp==ep or e:GetHandler():GetOriginalType()&ctype==0
	end
end
function c71200845.activate(e,tp,eg,ep,ev,re,r,rp)
	local label,count=e:GetLabel()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_OPERATECARD)
	local g=Duel.SelectMatchingCard(tp,c71200845.filter,tp,0,LOCATION_MZONE,count,count,nil)
