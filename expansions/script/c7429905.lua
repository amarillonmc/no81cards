...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsAttribute(ATTRIBUTE_WATER) and tc:IsLevelAbove(6) and tc:IsAbleToHand() then
		...
		if Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 then
			...
			if tg:GetCount()>=tc:GetLevel() and sg:GetClassCount(Card.GetCode)>=5 and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
				Duel.BreakEffect()
				...
				if Duel.SendtoDeck(stg,nil,0,REASON_EFFECT)~=0 then
					...
					local rg=sg:SelectSubGroup(tp,aux.dncheck,false,5,5)
