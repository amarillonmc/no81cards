...
function cm.mtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	for fc in aux.Next(g) do
		if not Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK,0,1,nil,fc,c,tp) then q1=false end
...
function cm.mop(e,tp,eg,ep,ev,re,r,rp)
	...
	for fc in aux.Next(g) do
		if not Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK,0,1,nil,fc,c,tp) then q1=false end
		typ=typ|bit.band(fc:GetType(),0x7)
		mg:Merge(Duel.GetMatchingGroup(cm.filter,tp,LOCATION_DECK,0,nil,fc,c,tp))
	end
	local ct=0
	for i=0,2 do if typ&(1<<i)>0 then ct=ct+1 end end
	...
	if opt==1 then
		...
		local g=mg:SelectSubGroup(tp,cm.fselect,false,ct,ct,typ)
		if #g>0 then
