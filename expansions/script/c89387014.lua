...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local zone=bit.band(e:GetHandler():GetLinkedZone(tp),0x1f)
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and zone~=0 and aux.NecroValleyFilter()(tc) then
...
function cm.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetMZoneCount(tp)>0 and eg:IsExists(cm.repfilter,1,nil,e,tp) end
