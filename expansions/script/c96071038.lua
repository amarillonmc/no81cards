...
function cm.initial_effect(c)
	--link summon
	aux.AddLinkProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0xff7),1,1)
	c:EnableReviveLimit()
	--to hand
...
function cm.thfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xff7) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
...
function cm.cfilter(c,ft,tp)
	return c:IsFaceup() and c:IsSetCard(0xff7) and c:IsType(TYPE_MONSTER) and c:IsControler(tp)
