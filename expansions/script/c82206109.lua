...
function cm.setcheck(c,tp)
	return c:IsCode(82206108) and c:IsSSetable() and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
end
function cm.negop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) then
		if Duel.SendtoDeck(eg,nil,2,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(cm.check,tp,LOCATION_ONFIELD,0,1,nil) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(cm.setcheck),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,tp) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
			Duel.BreakEffect()
