	...
	if g:GetCount()>0 then
		...
		if Duel.Destroy(g,REASON_EFFECT)~=0 then
            ...
            if og:IsExists(Card.IsPreviousControler,1,nil,tp) 
            	and Duel.GetFieldGroupCount(tp,0,LOCATION_ONFIELD+LOCATION_HAND)>0
                and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
            	Duel.BreakEffect()
            	local hg=Duel.GetFieldGroup(tp,0,LOCATION_HAND)
				local fg=Duel.GetFieldGroup(tp,0,LOCATION_ONFIELD)
				local g
				if hg:GetCount()>0 and (fg:GetCount()==0 or Duel.SelectOption(tp,aux.Stringid(id,4),aux.Stringid(id,5))==0) then
