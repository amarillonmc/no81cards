...
function cm.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.DiscardDeck(tp,1,REASON_EFFECT)==1 then
		...
		if tc:IsSetCard(0x539) and tc:IsAbleToHand() and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
...
function cm.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc:IsSetCard(0x539) and tc:IsType(TYPE_SPELL+TYPE_TRAP) then return end
