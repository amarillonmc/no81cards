...
function c60152601.initial_effect(c)
	...
	e1:SetTarget(c60152601.e1tar)
	e1:SetOperation(c60152601.e1op)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
    e2:SetCountLimit(1,6012601)
	e2:SetCondition(c60152601.e2con)
	c:RegisterEffect(e2)
end
function c60152601.e01con(e,tp,eg,ep,ev,re,r,rp)
	local tp=e:GetHandlerPlayer()
	local c=e:GetHandler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1
end
function c60152601.e01op(e,tp,eg,ep,ev,re,r,rp)
	local tp=e:GetHandlerPlayer()
	local c=e:GetHandler()
...
function c60152601.e1tarfilter(c)
	return c:IsSetCard(0x6b27) and c:IsType(TYPE_MONSTER) and not c:IsCode(60152601) and c:IsAbleToHand()
end
function c60152601.e1tar(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c60152601.e1tarfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c60152601.e1opfilter(c,e,tp)
	return c:IsSetCard(0x6b27) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c60152601.e1op(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c60152601.e1tarfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
		if e:GetHandler():GetColumnGroupCount()==0 then
			if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
			local g=Duel.GetMatchingGroup(c60152601.e1opfilter,tp,LOCATION_GRAVE,0,nil,e,tp)
...
function c60152601.e2confilter(c)
	return c:IsFaceup() and c:IsSetCard(0x6b27) and c:IsType(TYPE_XYZ)
end
function c60152601.e2con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c60152601.e2confilter,tp,LOCATION_MZONE,0,1,nil)
end