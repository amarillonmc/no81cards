...
function cm.initial_effect(c)
	...
	if not cm.global_check then
		...
		function Card.RegisterEffect(c,e,bool)
			...
			if res and Duel.GetTurnCount()>0 and e:IsActivated() and c:IsLocation(LOCATION_DECK) and PNFL_MIRROR_COPY and PNFL_MIRROR_COPY[c:GetControler()][e] then
				...
				if (PNFL_MIRROR_COPIED[1-c:GetControler()][te] or (PNFL_MIRROR_MULTI[c:GetOriginalCode()] and PNFL_MIRROR_COPIED[c:GetControler()][te])) then
					...
					if c:GetFlagEffect(m)>=4 and c:IsLocation(LOCATION_DECK) and c:IsAbleToHand() and #sg>0 and Duel.SelectEffectYesNo(c:GetControler(),c,aux.Stringid(m,m-c:GetOriginalCode())) then
