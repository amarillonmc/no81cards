...
function s.thfilter(c)
    return (c:IsCode(65823000) or aux.IsCodeListed(c,65823000)) and c:IsAbleToHand() and not c:IsCode(id)
end

function s.retfilter(c)
    return c:IsAbleToDeck() and (c:IsLocation(LOCATION_EXTRA) and c:IsFaceup()
        or c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED))
end

function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
    local b1=Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil)
    local b2=Duel.IsExistingMatchingCard(s.retfilter,tp,0,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED,1,nil)
    if chk==0 then return b1 or b2 end
    if b1 then Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK) end
    if b2 then Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,1-tp,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED) end
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
    ...
    if sel==2 or sel==3 then
        ...
        local g=Duel.GetMatchingGroup(s.retfilter,tp,0,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED,nil)
        if #g>0 then
