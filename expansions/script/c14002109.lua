...
function cm.cnfilter(c,lv)
	return c:IsFaceupEx() and c:IsLevel(lv)
end
function cm.thfilter(c,tp)
	return cm.Almotaher(c) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not Duel.IsExistingMatchingCard(cm.cnfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,nil,c:GetLevel())
end
...
function cm.spfilter(c,e,tp)
	return cm.Yachiyo(c) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
...
function cm.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(cm.spfilter),tp,LOCATION_GRAVE,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(m,3)) then
		Duel.BreakEffect()
