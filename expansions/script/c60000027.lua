...
local cm,m,o=GetID()
local m,cm=rscf.DefineCard(m)
function cm.initial_effect(c)
	aux.EnablePendulumAttribute(c,false)
	--xyz summon
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_WATER),12,5,nil,nil,12)
	c:EnableReviveLimit()
	--spsummon limit
...
function cm.thop(e,tp)
	local c=e:GetHandler()
	local tct=Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_PZONE,0,1,c,m) and 3 or 1
	for p=0,1 do
