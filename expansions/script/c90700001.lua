...
function cm.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(90700001,4))
...
function cm.addcop(e,tp,eg,ep,ev,re,r,rp)
	...
	while c do
		if not c:IsLocation(LOCATION_FZONE) and c:IsLocation(LOCATION_ONFIELD) and c:IsPosition(POS_FACEUP) and c:IsSetCard(0x5ac0) then
			...
			if (adcon or countercon or sercon) and Duel.SelectYesNo(tp,aux.Stringid(90700001,5)) then
				local ope=-1
				if adcon then
					if countercon then
						if sercon then
							ope=Duel.SelectOption(tp,aux.Stringid(90700001,0),aux.Stringid(90700001,1),aux.Stringid(90700001,2))
						else
							ope=Duel.SelectOption(tp,aux.Stringid(90700001,0),aux.Stringid(90700001,1))
						end
					else
						if sercon then
							ope=Duel.SelectOption(tp,aux.Stringid(90700001,0),aux.Stringid(90700001,2))
							if ope==1 then
								ope=2
							end
						else
					...
					if countercon then
						if sercon then
							ope=Duel.SelectOption(tp,aux.Stringid(90700001,1),aux.Stringid(90700001,2))
							ope=ope+1
						else
							ope=2
						end
				...
				if ope==0 then
					Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(90700001,3))
					local opt={}
					for i=1,Counters do
						opt[i]=i
					end
