...
function c49811159.slcon1(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():GetFlagEffect(49811159)>2
end
function c49811159.slcon2(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():GetFlagEffect(49811160)>2
end
function c49811159.cfilter1(c,tp)
    return c:IsSummonPlayer(tp) and not c:IsSummonableCard()
end
function c49811159.operation(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if eg:IsExists(c49811159.cfilter1,1,nil,tp) then
        c:RegisterFlagEffect(49811159,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
    end
    if eg:IsExists(c49811159.cfilter1,1,nil,1-tp) then
        c:RegisterFlagEffect(49811160,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
    end
end