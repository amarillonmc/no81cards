...
function c28323723.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
...
function c28323723.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local b3=Duel.IsExistingMatchingCard(c28323723.chkfilter,tp,LOCATION_HAND,0,2,nil,0x28a)
		and Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_EXTRA,0,1,nil,0x283)
	...
	local op=aux.SelectFromOptions(tp,
		...
		{true,aux.Stringid(28323723,4)})
	if op~=5 then c28323723.confirm(tp,op) end
	if op==1 then
	...
	elseif op==3 then
		...
		local tg=Duel.SelectMatchingCard(tp,Card.IsSetCard,tp,LOCATION_EXTRA,0,1,1,nil,0x283)
...
function c28323723.confirm(tp,op)
	Duel.ShuffleHand(tp)
	local code=op==1 and 0x288 or op==2 and 0x289 or op==3 and 0x28a or op==4 and 0x28b
	local g=Duel.GetMatchingGroup(c28323723.chkfilter,tp,LOCATION_HAND,0,nil,code)
	if #g>2 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
