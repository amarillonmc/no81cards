...
function s.drcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return g:GetCount()>0 and not g:IsExists(s.costfilter,1,nil) end
	local g=Duel.GetMatchingGroup(Card.IsType,tp,LOCATION_GRAVE,LOCATION_GRAVE,nil,TYPE_MONSTER)
...
function s.dract(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(s.damfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	local dam=g:GetSum(Card.GetAttack)
	Duel.Damage(1-p,dam,REASON_EFFECT)
end
