...
function cm.thfilter(c)
	return c:IsSetCard(0x5876) and c:IsAbleToHand()
end
...
function cm.tffilter(c,tp)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS) and c:IsSetCard(0x5876) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
...
function cm.tfop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.MoveToField(tc,tp,tp,LOCATION_SZONE,POS_FACEUP,true)~=0 then
		...
		if tc:IsFaceup() and tc:IsOnField() and tc:IsCanAddCounter(0x0876,1) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then tc:AddCounter(0x0876,1) end
	end
end
function cm.drcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsCanRemoveCounter(tp,1,0,0x0876,6,REASON_COST) end
	Duel.RemoveCounter(tp,1,0,0x0876,6,REASON_COST)
end
...
function cm.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not g:IsExists(aux.NOT(Card.IsSetCard),1,nil,0x876) and #sg>0 and Duel.SelectYesNo(tp,aux.Stringid(m,4)) then
		Duel.BreakEffect()
		Duel.ConfirmCards(1-tp,g)
