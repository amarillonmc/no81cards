...
function c22348260.doubcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (a:IsSetCard(0x70a3) and a:IsControler(tp) and not a:IsDualState() and a:IsType(TYPE_DUAL))
	or (b and b:IsSetCard(0x70a3) and b:IsControler(tp) and not b:IsDualState() and b:IsType(TYPE_DUAL))
end
function c22348260.doubop(e,tp,eg,ep,ev,re,r,rp,chk)
	Duel.Hint(HINT_CARD,0,22348260)
	local a=Duel.GetAttacker()
	local b=Duel.GetAttackTarget()
	if a:IsSetCard(0x70a3) and a:IsControler(tp) and not a:IsDualState() and a:IsType(TYPE_DUAL) then
	...
	if b and b:IsSetCard(0x70a3) and b:IsControler(tp) and not b:IsDualState() and b:IsType(TYPE_DUAL) then
...
function c22348260.reccon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and eg:GetFirst():IsControler(tp) and eg:GetFirst():IsSetCard(0x70a3) 
end
