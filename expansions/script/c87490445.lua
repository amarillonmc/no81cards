...
function c87490445.discon(e,tp,eg,ep,ev,re,r,rp) 
	local c=e:GetHandler()
	return Duel.GetFlagEffect(tp,87490445)==0 and rp==1-tp and Duel.IsChainDisablable(ev) and c:CheckRemoveOverlayCard(tp,1,REASON_EFFECT) and re:IsActiveType(TYPE_MONSTER)   
end 
function c87490445.disop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler() 
	if Duel.GetFlagEffect(tp,87490445)==0 and Duel.IsChainDisablable(ev) and c:CheckRemoveOverlayCard(tp,1,REASON_EFFECT) and Duel.SelectYesNo(tp,aux.Stringid(87490445,0)) then 
		Duel.Hint(HINT_CARD,0,87490445) 
		Duel.RegisterFlagEffect(tp,87490445,RESET_PHASE+PHASE_END,0,1) 
