...
function c116511113.mfilter1(c,tp,xyzc)
	return c:IsCanBeXyzMaterial(xyzc) and c116511113.mfilter2(c,tp)
end
function c116511113.mfilter2(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsSetCard(0x108a) and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_XYZ) and Duel.GetLocationCountFromEx(tp,tp,c)>0
end
function c116511113.mfilter3(c,e,xyzc)
	return c:IsCanBeXyzMaterial(xyzc) and not c:IsImmuneToEffect(e)
end
function c116511113.xyzcon1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return eg:IsExists(c116511113.mfilter1,1,nil,tp,c) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false) and Duel.IsChainDisablable(ev)
end
function c116511113.xyzop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SelectYesNo(tp,aux.Stringid(116511113,0)) then
		...
		mg:Merge(eg:Filter(c116511113.mfilter3,nil,e,c))
		if not rc:IsType(TYPE_TOKEN) and not rc:IsImmuneToEffect(e) then mg:AddCard(rc) end
...
function c116511113.xyzcon2(e,tp,eg,ep,ev,re,r,rp)
	...
	return eg:IsExists(c116511113.mfilter1,1,nil,tp,c) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,true,false)
end
function c116511113.xyzop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,116511113)==0 and Duel.SelectYesNo(tp,aux.Stringid(116511113,0)) then
		...
		Duel.NegateAttack()
		mg:Merge(eg:Filter(c116511113.mfilter3,nil,e,c))
		if rc:IsCanBeXyzMaterial(c) and not rc:IsImmuneToEffect(e) then mg:AddCard(rc) end
		local sg=Group.CreateGroup()
		local tc=mg:GetFirst()
		while tc do
			tc:CancelToGrave()
...
function c116511113.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c116511113.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
...
function c116511113.limval(e,re,rp)
	local rc=re:GetHandler()
	return re:IsActiveType(TYPE_MONSTER) and not rc:IsSetCard(0x108a) and not rc:IsImmuneToEffect(e)
end
