...
function cm.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_TOKEN+CATEGORY_DRAW)
...
function cm.filter(c)
	return c:IsCode(7419700,7419701,7419702,7419703) and c:IsFaceup() and c:IsAttackAbove(1500)
end
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)~=0 then
		if Duel.IsPlayerCanDraw(tp,1)
			and Duel.GetMatchingGroupCount(cm.filter,tp,LOCATION_MZONE,0,nil)>=1
...
function cm.thcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetMatchingGroupCount(cm.filter,tp,LOCATION_MZONE,0,nil)>=1
end
