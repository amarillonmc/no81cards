...
function c60152305.e1tgfilter2(c,code)
	return not c:IsCode(code) and c:IsSetCard(0xcb26) and c:IsAbleToGrave()
end
...
function c60152305.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c60152305.e1tgfilter2,tp,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,0,nil,code)
...
function c60152305.e2tgfilter(c)
	return c:IsSetCard(0xcb26) and c:IsAbleToDeck()
end
...
function c60152305.e2op(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if Duel.SendtoDeck(g,nil,2,REASON_EFFECT)~=0 and Duel.Draw(tp,1,REASON_EFFECT) then
