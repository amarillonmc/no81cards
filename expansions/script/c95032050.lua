...
function s.initial_effect(c)
	...
	-- 效果②：召唤·特殊召唤时发动，把手卡1只怪兽给对方，然后可选展示并检索（必发部分：送怪兽；选发部分：展示并检索）
	...
	-- 效果③：「捣蛋少女」以外的怪兽召唤·特殊召唤之际，将其无效并破坏（必发，一回合一次）
...
function s.handop(e, tp, eg, ep, ev, re, r, rp)
	...
	-- 2. 选发：展示3张不同名的捣蛋少女卡，然后从卡组检索1只捣蛋少女怪兽
	-- 先收集卡组中所有捣蛋少女卡，按卡名去重
	...
	if #unique_list >= 3 then
		if Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
			...
			-- 从卡组把1只捣蛋少女怪兽加入手卡
			local monsters = Duel.GetMatchingGroup(function(c)
				return c:IsSetCard(s.named_setcode) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
			end, tp, LOCATION_DECK, 0, nil)
			if #monsters > 0 then
				Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_ATOHAND)
				local add = monsters:Select(tp, 1, 1, nil)
				if #add > 0 then
...
function s.negcon(e, tp, eg, ep, ev, re, r, rp)
	...
	return sumc and not sumc:IsSetCard(s.named_setcode)
end

function s.negtg(e, tp, eg, ep, ev, re, r, rp, chk)
	if chk == 0 then return true end
	local sumc = eg:GetFirst()
	Duel.SetOperationInfo(0, CATEGORY_DISABLE, sumc, 1, 0, 0)
	Duel.SetOperationInfo(0, CATEGORY_DESTROY, sumc, 1, 0, 0)
end

function s.negop(e, tp, eg, ep, ev, re, r, rp)
	local sumc = eg:GetFirst()
	if sumc and sumc:IsLocation(LOCATION_MZONE) and not sumc:IsSetCard(s.named_setcode) then
		Duel.NegateSummon(sumc)
		Duel.Destroy(sumc, REASON_EFFECT)
	end
end