...
function cm.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e:SetLabel(0)
	if g:GetFirst():GetOriginalType()&TYPE_SPELL>0 then
		e:SetLabel(1)
	end
	g:AddCard(c)
...
function cm.thfilter(c)
	return c:IsSetCard(0xaf1a) and c:IsAbleToHand() and c:IsType(TYPE_SPELL)
end
...
function cm.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.GetMatchingGroup(cm.tdfilter,tp,LOCATION_REMOVED,0,nil,e)
	local selected_g=g:SelectSubGroup(tp,cm.tdgfilter,false,1,#g,e,tp,c)
	Duel.SetTargetCard(selected_g)
	e:SetLabel(#selected_g*2)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,selected_g,#selected_g,0,0)
