...
function cm.th(c)
	...
	return not c:IsCode(m) and b1 and b2 
end
function cm.limitop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local lc=e:GetLabelObject()
	local tc=eg:GetFirst()
	if lc==tc and tc:IsSummonType(SUMMON_TYPE_ADVANCE) then 
		local tck=Duel.IsExistingMatchingCard(cm.th,tp,LOCATION_DECK,0,1,nil) 
		if tck and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
