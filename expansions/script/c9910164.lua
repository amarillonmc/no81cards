...
function c9910164.tgfilter(c)
	return c:IsSetCard(0x9958) and c:IsAbleToGrave()
end
...
function c9910164.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) and c:IsRelateToChain() then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c9910164.thfilter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x9958) and c:IsAbleToHand()
end
function c9910164.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD+LOCATION_GRAVE) and chkc:IsControler(tp) and c9910164.thfilter(chkc) end
	if chk==0 then return e:GetHandler():IsLevelAbove(2)
		and Duel.IsExistingTarget(c9910164.thfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c9910164.thfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c9910164.cfilter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x9958)
end
function c9910164.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ct=Duel.GetMatchingGroupCount(c9910164.cfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
	if c:IsFaceup() and c:IsRelateToChain() and c:IsLevelAbove(2) and ct>0 then
		local clv=c:GetLevel()-1
		if clv<ct then ct=clv end
