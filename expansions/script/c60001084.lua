...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(cm.rmfilter,tp,0,LOCATION_HAND,1,c,tp) then ablerm_hand=1 end
	if Duel.IsExistingMatchingCard(cm.rmfilter,tp,0,LOCATION_DECK,1,c,tp) then ablerm_deck=1 end
	if Duel.IsExistingMatchingCard(cm.rmfilter,tp,0,LOCATION_EXTRA,1,c,tp) then ablerm_extra=1 end
	...
	local g=Duel.GetMatchingGroup(cm.rmfilter,tp,0,loc,c,tp)
