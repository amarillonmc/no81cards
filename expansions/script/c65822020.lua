...
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and not re:GetHandler():IsLocation(LOCATION_ONFIELD)
end
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,0,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,1-tp,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED)
end
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.tdfilter,tp,0,LOCATION_EXTRA+LOCATION_GRAVE+LOCATION_REMOVED,nil)
