...
function c119828752.pcop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c119828752.pcfilter),tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,2,nil)
...
function c119828752.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and not tc:IsImmuneToEffect(e) then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c119828752.filter2),tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,1,nil)
		Duel.Overlay(tc,g)
	end
end
