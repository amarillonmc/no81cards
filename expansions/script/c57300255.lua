...
function s.filter1(c,e,tp)
	local rk=c:GetRank()
	return rk>0 and c:IsFaceup() and c:GetOverlayCount()==0
...
function s.filter2(c,e,tp,mc,rk)
	...
	return c:IsRank(rk) and mc:IsCanBeXyzMaterial(c) and not c:IsRace(mc:GetRace()) and not c:IsAttribute(mc:GetAttribute())
