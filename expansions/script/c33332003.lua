...
local m=33332003
local cm=_G["c"..m]
function c33332003.initial_effect(c)
	--spsummon
...
function cm.rmop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g1=eg:Filter(cm.cfilter,nil,1-tp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_HAND+LOCATION_REMOVED,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
		if Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
