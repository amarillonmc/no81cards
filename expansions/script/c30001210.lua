...
function cm.limitop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=eg:GetFirst()
	if tc==lc and tc:IsSummonType(SUMMON_TYPE_ADVANCE) and tc:IsLocation(LOCATION_MZONE) then 
		Duel.Readjust()
		local cg=tc:GetColumnGroup():Filter(Card.IsControler,nil,1-tp)
		if #cg>0 and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
			local tcc=cg:GetFirst()
			while tcc do
				if tcc:IsFaceup() then
					Duel.NegateRelatedChain(tcc,RESET_TURN_SET)
