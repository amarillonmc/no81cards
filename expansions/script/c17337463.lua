...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return d and d:IsControler(tp) and d:IsFaceup() and d:IsSetCard(0x3f50)
end
function s.tfilter(c,tp)
	return c:IsControler(tp) and c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(0x3f50)
end
...
function s.reveal_filter(c,mc)
	return c:IsFaceup() and c:IsSetCard(0x3f50) and c~=mc
end
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then	  
		...
		local sum=(math.max(0,c:GetBaseAttack())+math.max(0,tc:GetBaseAttack()))/2
