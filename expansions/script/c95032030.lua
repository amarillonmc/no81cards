...
function s.initial_effect(c)
	...
	-- 效果②：召唤·特殊召唤时发动，把手卡1只怪兽给对方，然后可选展示并放置永续魔法
	...
	-- 效果③：「捣蛋少女」以外的魔法·陷阱卡发动时，无效那个发动（必发，一回合一次）
	...
	if not s.global_check then
		...
		function Duel.ConfirmCards(player, targets)
			...
			local function raise_event(card)
				-- 只有当确认的卡是「捣蛋少女」字段、在手卡、且控制者不是确认者（即对方确认了己方手卡）时触发
...
function s.handop(e, tp, eg, ep, ev, re, r, rp)
	...
	if #unique_list >= 3 then
		if Duel.SelectYesNo(tp, aux.Stringid(id, 3)) then
			...
			-- 从卡组把1张「捣蛋少女」永续魔法卡在自己场上表侧表示放置
			local magic_group = Duel.GetMatchingGroup(function(c)
				return c:IsSetCard(s.named_setcode) and c:IsType(TYPE_SPELL) and c:IsType(TYPE_CONTINUOUS) and c:IsSSetable()
			end, tp, LOCATION_DECK, 0, nil)
			if #magic_group > 0 then
				Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_SET)
				local magic = magic_group:Select(tp, 1, 1, nil):GetFirst()
				if magic then
					Duel.MoveToField(magic, tp, tp, LOCATION_SZONE, POS_FACEUP, true)
				end
			end
		end
	end
...
-- 效果③的条件：魔法·陷阱卡发动，且不是「捣蛋少女」字段
function s.discon(e, tp, eg, ep, ev, re, r, rp)
	if not re then return false end
	local rc = re:GetHandler()
	return re:IsHasType(EFFECT_TYPE_ACTIVATE) and (not rc:IsSetCard(s.named_setcode))
end
