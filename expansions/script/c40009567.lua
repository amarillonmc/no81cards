...
function cm.initial_effect(c) 
	aux.AddCodeList(c,40009569,40009571,40009579)
	--special summon
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		if Duel.IsExistingMatchingCard(cm.spfil,tp,LOCATION_HAND,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then table.insert(xtable,aux.Stringid(m,2)) end 
		local op=Duel.SelectOption(tp,table.unpack(xtable))+1  
		if xtable[op]==aux.Stringid(m,1) then 
...
function cm.thfilter(c,tp) 
	if Duel.IsExistingMatchingCard(function(c) return c:IsFaceup() and c:IsCode(40009569,40009571) end,tp,LOCATION_ONFIELD,0,1,nil) then 
		return (c:IsCode(40009569,40009571) or (aux.IsCodeListed(c,56099748) and c:IsType(TYPE_SPELL+TYPE_TRAP))) and c:IsAbleToHand() 
	else 
