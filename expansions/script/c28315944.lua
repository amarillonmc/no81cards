...
function c28315944.initial_effect(c)
	...
	e1:SetCountLimit(1,28315944+1)
	e1:SetCondition(c28315944.icon)
	e1:SetCost(c28315944.cost)
	...
	e0:SetCondition(c28315944.qcon)
	c:RegisterEffect(e0)
	--recover
...
function c28315944.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local ct=Duel.GetFlagEffectLabel(tp,28315944) or 0
	ct=ct|e:GetLabel()
	if ct==e:GetLabel() then
		Duel.RegisterFlagEffect(tp,28315944,RESET_PHASE+PHASE_END,0,1,ct)
	else
		Duel.SetFlagEffectLabel(tp,28315944,ct)
	end
