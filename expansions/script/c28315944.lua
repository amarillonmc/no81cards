...
function c28315944.gspcon(e,c)
	...
	return Duel.GetLP(p)>=9000 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c28315944.alfilter,p,LOCATION_MZONE,0,1,nil)
end
...
function c28315944.recop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.Recover(tp,1000,REASON_EFFECT)>0 and Duel.GetLP(tp)>=10000 then
		...
		local ops,opval={},{}
		if b1 then
			ops[off]=aux.Stringid(28315944,0)
			opval[off]=0
			off=off+1
		end
		if b2 then
			ops[off]=aux.Stringid(28315944,1)
			opval[off]=1
			off=off+1
		end
		if true then
			ops[off]=aux.Stringid(28315944,2)
			opval[off]=2
			off=off+1
		end
