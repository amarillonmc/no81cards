...
local cm=_G["c"..m]
cm.name="心术魔导 LV4"
cm.Card_Prophecy_LV4=true
--[[if not Card_Prophecy_OP_M_Check then
	Card_Prophecy_OP_M_Check=true
	cm[1]=Card.RegisterEffect
	Card.RegisterEffect=function(sc,se,bool)
		local op=se:GetOperation()
		se:SetOperation(function(...)
			Card_Prophecy_OP_Check=true
			op(...)
		end)
	...
	if not Card_Prophecy_Cheat_1 then
		...
		Card.IsCanBeSpecialSummoned=function(sc,se,st,sp,bool1,bool2,spos,stp,sz)
			...
			if lv>0 and (ly>0 or Duel.GetFlagEffectLabel(0,53759000)==0) and not sc:IsLocation(LOCATION_MZONE) then
				--if spos&POS_FACEUP==0 then b=false end
...
function cm.cost1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.spcfilter,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local cg=Duel.SelectMatchingCard(tp,cm.spcfilter,tp,LOCATION_HAND,0,1,1,e:GetHandler())
...
function cm.relfilter(c,tp)
	return c:IsSetCard(0x41) and c:IsReleasableByEffect() and Duel.GetMZoneCount(tp,c)>0 and c:IsLevel(3,4)
end
...
function cm.setfilter(c)
	return c:IsSetCard(0x6e) and c:IsType(TYPE_QUICKPLAY) and c:IsSSetable()
end
