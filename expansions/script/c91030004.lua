...
local m=91030004
local cm=c91030004
function c91030004.initial_effect(c)
...
function cm.thfilter(c)
	return c:IsSetCard(0x9d3) and not c:IsCode(m) and c:IsAbleToHand()
end
...
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	 if chk==0 then return Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) or  Duel.IsExistingMatchingCard(Card.IsLinkSummonable,tp,LOCATION_EXTRA,0,1,nil,nil) or Duel.IsExistingMatchingCard(cm.thfilter,tp,LOCATION_DECK,0,1,nil)  end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		if Duel.IsPlayerCanDraw(tp,1)
			and Duel.GetMatchingGroupCount(Card.IsType,tp,LOCATION_GRAVE,0,nil,TYPE_QUICKPLAY)>=3
