...
function cm.actfilter_1(c)
	return c:IsCode(31406029,31406047) and c:IsAbleToHand()
end
function cm.show_filter(c)
	return c:IsCode(31406029,31406047) and not c:IsPublic()
end
function cm.actfilter_2(c)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_PSYCHO) and c:IsAbleToHand()
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local chk_1=Duel.IsExistingMatchingCard(cm.actfilter_1,tp,LOCATION_DECK,0,1,nil)
	local chk_2=Duel.IsExistingMatchingCard(cm.show_filter,tp,LOCATION_HAND,0,1,nil) and Duel.IsExistingMatchingCard(cm.actfilter_2,tp,LOCATION_DECK,0,1,nil)
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetMatchingGroup(cm.actfilter_1,tp,LOCATION_DECK,0,nil)
	if op==1 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
		local g=Duel.SelectMatchingCard(tp,cm.show_filter,tp,LOCATION_HAND,0,1,1,nil)
		Duel.ConfirmCards(1-tp,g)
		tg=Duel.GetMatchingGroup(cm.actfilter_2,tp,LOCATION_DECK,0,nil)
	end
	...
	if tg1:GetCount()~=0 then
		if tg1:GetCount()==tg:GetCount() or Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
...
function cm.bfilter_th(c)
	return c:IsSetCard(0x6311) and c:IsAbleToHand()
end
function cm.bfilter_td(c)
	return c:IsSetCard(0x6311) and c:IsAbleToDeck()
end
function cm.btg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.bfilter_th,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_HAND)
end
function cm.bop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local tg=Duel.GetMatchingGroup(cm.bfilter_th,tp,LOCATION_DECK,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local tg1=tg:Filter(Card.IsPosition,nil,POS_FACEUP_DEFENSE)
	local tg2=tg:__sub(tg1)
	local th
	if tg1:GetCount()~=0 then
		if tg1:GetCount()==tg:GetCount() or Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
			th=tg1:Select(tp,1,1,nil)
		else
			th=tg2:Select(tp,1,1,nil)
		end
	else
		th=tg:Select(tp,1,1,nil)
	end
	if th then
