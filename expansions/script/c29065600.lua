...
function c29065600.thfil(c) 
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x7ad) and not c:IsCode(29065600)
end
...
function c29065600.rettg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c29065600.exqfil,tp,0,LOCATION_MZONE,1,nil) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
	local g=nil 
	if c:IsType(TYPE_XYZ) then
	local sz=Duel.GetLocationCount(tp,LOCATION_SZONE)
	if sz>=2 then sz=2 end 
	g=Duel.SelectTarget(tp,c29065600.exqfil,tp,0,LOCATION_MZONE,1,sz,nil)
	else
