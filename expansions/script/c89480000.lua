...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_POSITION+CATEGORY_TOHAND+CATEGORY_SEARCH+CATEGORY_MSET)
	e1:SetType(EFFECT_TYPE_IGNITION)
...
function s.thfilter(c)
	return not c:IsCode(id) and c:IsType(TYPE_MONSTER) and c:IsSetCard(0xc21) and c:IsAbleToHand()
end
