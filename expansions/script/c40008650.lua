...
function c40008650.initial_effect(c)
	...
	e2:SetCondition(c40008650.condition)
...
function c40008650.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c40008650.spfilter(c,e,tp)
...
function c40008650.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c40008650.spfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c40008650.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c40008650.spfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
