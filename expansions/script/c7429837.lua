...
function cm.splimit(e,c,tp)
	...
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429151,0,TYPES_TOKEN_MONSTER,0,0,4,RACE_FIEND,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,2)
...
function cm.sumlimit(e,c,tp)
	...
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429151,0,TYPES_TOKEN_MONSTER,0,0,4,RACE_FIEND,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,1)
	or Duel.IsPlayerAffectedByEffect(tp,59822133) then return false end
	return true
end
function cm.tokenop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>3
		and Duel.IsPlayerCanSpecialSummonMonster(tp,7429151,0,TYPES_TOKEN_MONSTER,0,0,4,RACE_FIEND,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) then
		for i=0,3 do
			local token=Duel.CreateToken(tp,7429151+i)
			local seq=0
			local seq_i=i
...
function cm.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if g:GetCount()>0 then
		...
		GM_global_to_deck_check=true
		if rp==1-tp and ct>0 and tdg:GetCount()>0 then
			local tg=tdg:RandomSelect(0,ct)
