...
function s.e2tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local bl1=Duel.IsExistingMatchingCard(s.e2tohfilter,tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil)
	local bl2=Duel.IsExistingMatchingCard(s.e2spfilter,tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil,e,tp) 
...
function s.e2op(e,tp,eg,ep,ev,re,r,rp)
	local bl1=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.e2tohfilter),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil)
	local bl2=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.e2spfilter),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,nil,e,tp) 
	...
	if op==0 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.e2tohfilter),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,1,nil)
		Duel.SendtoHand(g,nil,REASON_EFFECT)
	end
	
	if op==1 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.e2spfilter),tp,LOCATION_EXTRA+LOCATION_REMOVED,0,1,1,nil,e,tp)
