...
function cm.filter(c,e,tp)
	return c:IsSetCard(0x9341) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g2=Duel.GetMatchingGroupCount(cm.recfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,500*g2)
end
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
		  ...
		  Duel.SpecialSummon(g1,0,tp,tp,false,false,POS_FACEUP)
		   ...
		   local g=Duel.GetMatchingGroupCount(cm.recfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
