...
function cm.extfilter(c,e,tp,chk1,chk2,chk3)
	if chk2 and not chk1 and not chk3 then return c:IsType(TYPE_NORMAL) and ((not c:IsPublic() and c:IsLocation(LOCATION_HAND)) or (c:IsAbleToRemoveAsCost() and c:IsLocation(LOCATION_GRAVE) and Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_GRAVE,0,1,c,e,tp))) end
	return c:IsType(TYPE_NORMAL) and ((not c:IsPublic() and c:IsLocation(LOCATION_HAND)) or (c:IsAbleToRemoveAsCost() and c:IsLocation(LOCATION_GRAVE)))
end
...
function cm.costop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg2=function(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
				...
				local chk3=(cm[tp]&0x4>0)
				local extg=Duel.GetMatchingGroup(cm.extfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil,e,tp,chk1,chk2,chk3)
				if #extg>0 and (chk1 or chk2 or chk3) and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
					...
					if #g>0 then
						...
						local op2=function(e,tp,eg,ep,ev,re,r,rp)
									...
									elseif sel==2 then
										...
										if not cm.reg then
											...
											e1:SetCondition(function(e,tp,eg,ep,ev,re,r,rp) return Duel.GetFlagEffect(tp,m)>0 and re:GetActiveType()==TYPE_SPELL and re:IsHasType(EFFECT_TYPE_ACTIVATE) and rp==tp end)
						...
						e1:SetOperation(function(e,tp,eg,ep,ev,re,r,rp) re:SetOperation(op0) re:SetProperty(prop,prop2) end)
