...
function c67200758.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(67200758,1))
	...
	e3:SetCountLimit(1,67200747)
	e3:SetCost(c67200758.tscost)
	e3:SetTarget(c67200758.tstg)
	e3:SetOperation(c67200758.tsop)
	c:RegisterEffect(e3)
...
--
function c67200758.costfilter(c)
	return c:IsSetCard(0x367d) and c:IsAbleToRemoveAsCost()
end
function c67200758.tscost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c67200758.costfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c67200758.costfilter,tp,LOCATION_HAND,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c67200758.tstg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand()
		or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false)) end
end
function c67200758.tsop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
