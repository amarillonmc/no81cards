...
function cm.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(90700033,1))
	...
	e4:SetDescription(aux.Stringid(90700033,2))
	e4:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_HAND)
	e4:SetCountLimit(1,290700033)
	e4:SetCost(cm.discost)
	e4:SetTarget(cm.distg)
	e4:SetOperation(cm.disop)
	c:RegisterEffect(e4)
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(90700033,3))
...
function cm.rlvcop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(aux.FilterBoolFunction(Card.IsCode,90700033),tp,0xff,0,nil)
	local tc=g:GetFirst()
	while tc do
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringid(90700033,0))
...
function cm.ptgfilter2(c)
	return c:IsSetCard(0x6312) and c:IsAbleToGrave()
end
...
function cm.distgfilter(c)
	return c:IsSetCard(0x6312) and c:IsType(TYPE_MONSTER) and c:IsType(TYPE_PENDULUM) and c:IsAbleToHand()
end
