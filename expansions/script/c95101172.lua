...
function c95101172.initial_effect(c)
	...
	e1:SetCountLimit(1,95101172)
	e1:SetCondition(c95101172.setcon)
	e1:SetTarget(c95101172.settg)
	e1:SetOperation(c95101172.setop)
	c:RegisterEffect(e1)
	--spsummon
	...
	e2:SetCountLimit(1,95101172+1)
	e2:SetCost(c95101172.spcost)
	e2:SetTarget(c95101172.sptg)
	e2:SetOperation(c95101172.spop)
	c:RegisterEffect(e2)
end
function c95101172.setcon(e,tp,eg,ep,ev,re,r,rp)
...
function c95101172.setop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.CheckLocation(tp,LOCATION_PZONE,0) and not Duel.CheckLocation(tp,LOCATION_PZONE,1) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,c95101172.penfilter,tp,LOCATION_DECK,0,1,1,nil):GetFirst()
	if tc and Duel.MoveToField(tc,tp,tp,LOCATION_PZONE,POS_FACEUP,true) then
		Duel.BreakEffect()
		Duel.SendtoHand(e:GetHandler(),nil,REASON_EFFECT)
	end
end
function c95101172.costfilter(c,tp)
	return aux.IsCodeListed(c,95101001) and c:IsFaceup() and Duel.GetMZoneCount(tp,c)>0 and c:IsAbleToHandAsCost()
end
function c95101172.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c95101172.costfilter,tp,LOCATION_ONFIELD,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectMatchingCard(tp,c95101172.costfilter,tp,LOCATION_ONFIELD,0,1,1,nil,tp)
	Duel.SendtoHand(g,nil,REASON_COST)
end
function c95101172.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
