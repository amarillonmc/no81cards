...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,3,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,3,3,e:GetHandler())
...
function s.costfilter(c)
	return c:IsRace(RACE_REPTILE) and c:IsDiscardable()
end
...
function s.setfilter(c)
	return c:IsSetCard(0xc20) and c:IsSSetable() and c:IsType(TYPE_SPELL+TYPE_TRAP)
end
