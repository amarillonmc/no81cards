...
function cm.condition(e,tp)
	return Duel.GetTurnCount()<4 or (Duel.IsPlayerAffectedByEffect(tp,10419902) and Duel.GetFlagEffect(tp,10419902)==0)
end
...
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.GetTurnCount()>=4 then Duel.RegisterFlagEffect(tp,10419902,RESET_PHASE+PHASE_END,0,1) end
...
function cm.targett(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) and Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)>0 and cm.TargetReflect1(e,tp,eg,ep,ev,re,r,rp,0)
	   and cm.TargetReflect2(e,tp,eg,ep,ev,re,r,rp,0) 
	   and cm.TargetReflect3(e,tp,eg,ep,ev,re,r,rp,0) 
	   and cm.TargetReflect4(e,tp,eg,ep,ev,re,r,rp,0) 
	   and cm.TargetReflect5(e,tp,eg,ep,ev,re,r,rp,0) end
...
function cm.activatet(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler()) and Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)==0 then return false end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,e:GetHandler())
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g2=Duel.SelectMatchingCard(tp,aux.TRUE,tp,LOCATION_HAND,0,1,1,e:GetHandler())
	g:Merge(g2)
	if g:GetCount()>0 then
