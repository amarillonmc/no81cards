...
function c11533712.initial_effect(c)
	...
	e1:SetCountLimit(1,11533712+EFFECT_COUNT_CODE_OATH)
...
function c11533712.accost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetActivityCount(tp,ACTIVITY_SUMMON)==0 and g:GetCount()>2 and g:GetCount()==g:FilterCount(Card.IsAbleToRemoveAsCost,nil) end
...
function c11533712.actg(e,tp,eg,ep,ev,re,r,rp,chk) 
	local g=Duel.GetMatchingGroup(nil,tp,LOCATION_HAND,0,e:GetHandler()) 
	if chk==0 then return Duel.IsExistingMatchingCard(c11533712.thfil,tp,LOCATION_DECK,0,1,nil) end  
