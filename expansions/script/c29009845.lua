...
function c29009845.refilter(c)
	return c:IsType(TYPE_TOKEN) and c:IsReleasable()
end
function c29009845.thtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c29009845.refilter,tp,LOCATION_MZONE,0,1,nil) and Duel.IsExistingMatchingCard(c29009845.rthfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c29009845.thop2(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c29009845.rthfilter,tp,LOCATION_DECK,0,nil)
	local g3=Duel.GetMatchingGroup(c29009845.refilter,tp,LOCATION_MZONE,0,nil)
	local ct=g:GetClassCount(Card.GetCode)
	if ct>1 and #g3>0 then
		local ct=ct-1
		local rg=Duel.SelectMatchingCard(tp,c29009845.refilter,tp,LOCATION_MZONE,0,1,ct,nil)
...
function c29009845.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,29009846,0,TYPES_TOKEN_MONSTER,0,0,5,RACE_THUNDER,ATTRIBUTE_LIGHT) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c29009845.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,29009846,0,TYPES_TOKEN_MONSTER,0,0,5,RACE_THUNDER,ATTRIBUTE_LIGHT) then return end
