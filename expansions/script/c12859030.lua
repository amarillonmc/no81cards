...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsDestructable() and Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_ONFIELD,1,nil) end
...
function s.thfilter(c)
    return c:IsSetCard(0xCA7D) and c:IsType(TYPE_MONSTER) and c:GetLevel()~=7 and c:IsAbleToHand()
end
