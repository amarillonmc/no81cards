--秘林诞地神庙
local s,id,o=GetID()
function c33300766.initial_effect(c)
	--Activate
...
function s.thfilter(c)
	return c:IsSetCard(0xc569) and c:IsRace(RACE_REPTILE) and c:IsAbleToHand()
end
...
function s.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsCanRemoveCounter(tp,1,0,0xc569,1,REASON_COST) or Duel.IsExistingTarget(s.costfil,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) end
	if Duel.IsExistingTarget(s.costfil,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
