...
function s.initial_effect(c)
	...
	e1:SetTarget(s.eqsumtg)
	e1:SetOperation(s.eqsumop)
	c:RegisterEffect(e1)
	--
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x54c) and not c:IsType(TYPE_SYNCHRO) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
function s.eqsumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK+LOCATION_GRAVE)
end
function s.eqsumop(e,tp,eg,ep,ev,re,r,rp)
