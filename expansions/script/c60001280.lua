...
function cm.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_DAMAGE+CATEGORY_DRAW+CATEGORY_TOGRAVE+CATEGORY_DECKDES+CATEGORY_DESTROY)
...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if xiadie.damage(e)~=0 then 
		...
		if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT+REASON_DISCARD) and Duel.Draw(tp,1,REASON_EFFECT) then
			...
			if Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE,0,1,nil,60001275) and Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler()) and Duel.SelectEffectYesNo(tp,e:GetHandler()) then
