...
function cm.spsop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	if Duel.IsPlayerCanSpecialSummonMonster(tp,31470999,0x3312,0x4011,0,0,2,RACE_FAIRY,ATTRIBUTE_LIGHT) then
		local token=Duel.CreateToken(tp,31470999)
...
function cm.imuop(e,tp,eg,ep,ev,re,r,rp)
	...
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(31470006,1))
	...
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(31470006,0))
