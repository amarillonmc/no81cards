...
function cm.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.mfilter,tp,0,LOCATION_MZONE,1,nil) end
end
function cm.disop(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISABLE)
	local g=Duel.SelectMatchingCard(tp,cm.mfilter,tp,0,LOCATION_MZONE,1,1,nil)
	local tc=g:GetFirst()
	if tc then
...
function cm.thfilter(c)
	return c:IsSetCard(0x48c) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
