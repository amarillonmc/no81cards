...
function s.tdfilter(c,tp)
	return Duel.GetMZoneCount(tp,c)>0 and c:IsAbleToDeck() and c:IsSetCard(0xa220) and c:IsFaceup() and c:IsType(TYPE_RITUAL)
end
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_MZONE,0,1,nil,tp)
		and c:IsCanBeSpecialSummoned(e,0,tp,false,true,POS_FACEUP) end
...
function s.cpfilter(c)
	return c:IsFaceup() and not c:IsType(TYPE_TOKEN) and not c:IsCode(id)
end
