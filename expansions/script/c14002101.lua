...
function cm.spcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and (Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0
			or Duel.IsExistingMatchingCard(cm.Yachiyo,tp,LOCATION_ONFIELD,0,1,nil))
end
...
function cm.thfilter(c,tc)
	return c:IsAbleToHand() and c:GetCode()~=tc:GetCode()
end
...
function cm.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if tc and Duel.Destroy(tc,REASON_EFFECT)~=0 then
			if (cm.Yachiyo(tc) or cm.Almotaher(tc)) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(cm.thfilter),tp,LOCATION_GRAVE,0,1,nil,tc) and Duel.SelectYesNo(tp,aux.Stringid(m,3)) then
				Duel.BreakEffect()
