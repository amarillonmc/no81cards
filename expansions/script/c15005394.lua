...
function cm.rmfilter(c,tp)
	return c:IsFaceup() and c:IsLevelAbove(1) and Duel.IsPlayerCanSpecialSummonMonster(tp,15005397,nil,TYPES_TOKEN_MONSTER,c:GetTextAttack(),c:GetTextDefense(),c:GetOriginalLevel(),RACE_DRAGON,c:GetOriginalAttribute())
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and cm.rmfilter(chkc,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(cm.rmfilter,tp,LOCATION_REMOVED,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELF)
	Duel.SelectTarget(tp,cm.rmfilter,tp,LOCATION_REMOVED,0,1,1,nil,tp)
...
function cm.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) then
		...
		tc:RegisterFlagEffect(15005394,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,tcid,aux.Stringid(15005394,7))
...
function cm.bedesop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsReason(REASON_DESTROY) then
		...
		if tc and tc:GetFlagEffect(15005394)~=0 then
			local tcid=tc:GetFlagEffectLabel(15005394)
			if tcid==tc:GetFieldID() then
				Duel.Hint(HINT_CARD,1-tp,15005394)
...
function cm.srcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (ph==PHASE_MAIN1 or ph==PHASE_MAIN2) and not Duel.IsExistingMatchingCard(cm.nonfilter,tp,LOCATION_MZONE,0,1,nil,ATTRIBUTE_WATER)
end
...
function cm.srtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.srfilter,tp,LOCATION_DECK,0,1,nil,ATTRIBUTE_WATER) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function cm.srop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,cm.srfilter,tp,LOCATION_DECK,0,1,1,nil,ATTRIBUTE_WATER)
