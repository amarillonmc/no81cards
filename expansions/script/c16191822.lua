...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if sg:GetCount()>0 then
        Duel.RegisterFlagEffect(e:GetHandler():GetOwner(),16170120,RESET_PHASE+PHASE_END,0,1)
    end    
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,e:GetHandler()) 
    	and Duel.GetFlagEffect(tp,16170120)<=1 end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD,e:GetHandler())
    local c=e:GetHandler()  
    if Duel.GetFlagEffect(tp,16170120)==0 then    	  
    	local e1=Effect.CreateEffect(c)
    ...
    elseif Duel.GetFlagEffect(tp,16170120)>0 then
...
function s.thfilter(c)
	return  c:IsAbleToHand() and not c:IsCode(16170120) and aux.IsCodeListed(c,16170120)
end
