...
function c22348030.sprcon(e,c)
	...
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and (Duel.IsExistingMatchingCard(c22348030.filter,tp,LOCATION_HAND,0,2,nil) or (Duel.IsPlayerCanDiscardDeckAsCost(tp,2) and Duel.IsPlayerAffectedByEffect(tp,22348041)))
end
function c22348030.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=Duel.SelectMatchingCard(tp,c22348030.filter,tp,LOCATION_HAND,0,2,2,nil)
	local tc=g:GetFirst()
	while tc do
	...
	elseif Duel.IsPlayerAffectedByEffect(tp,22348041) and not Duel.IsExistingMatchingCard(c22348030.filter,tp,LOCATION_HAND,0,2,nil) then
