...
function c40009314.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0x4f1d),aux.FilterBoolFunction(Card.IsSetCard,0x4f1d),3,3)
	c:EnableReviveLimit() 
...
function c40009314.spfilter(c,tp,sync)
...
function c40009314.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsSummonType(SUMMON_TYPE_SYNCHRO)
		and ct>0
		and mg:FilterCount(c40009314.spfilter,nil,tp,c)==ct end
