...
function cm.chop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g1~=0 and #g2==0 then
		if Duel.IsExistingMatchingCard(nil,0,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) then
	...
	if #g1==0 and #g2~=0 then
		if Duel.IsExistingMatchingCard(nil,1,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) then
...
function cm.selfilter(c)
	return c:GetEffectCount(15000382)~=0 and not Duel.IsPlayerAffectedByEffect(c:GetControler(),15000383) and not c:IsDisabled()
end
function cm.repop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local se=Duel.IsPlayerAffectedByEffect(0,15000382)
	if not se then se=Duel.IsPlayerAffectedByEffect(1,15000382) end
	local op=nil
	local p=0
	if se then
		local x=1
		for _,i in ipairs{Duel.IsPlayerAffectedByEffect(0,15000382)} do
	...
	if Duel.IsExistingMatchingCard(nil,p,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) then
		Duel.BreakEffect()
		Duel.Hint(HINT_CARD,1-p,15000382)
		Duel.Hint(HINT_SELECTMSG,p,HINTMSG_DESTROY)
		local g=Duel.SelectMatchingCard(p,nil,p,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
...
function cm.thfilter(c)
	return c:IsSetCard(0xaf30) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function cm.filter(c,tp)
	return c:IsType(TYPE_FIELD) and c:GetActivateEffect():IsActivatable(tp,true,true) and c:IsCode(15000384)
end
