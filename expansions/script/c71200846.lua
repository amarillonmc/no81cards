--無限泡影
function c71200846.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c71200846.target)
	e1:SetOperation(c71200846.activate)
	c:RegisterEffect(e1)
	--act in hand
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(71200846,0))
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_TRAP_ACT_IN_HAND)
	e2:SetCondition(c71200846.handcon)
	c:RegisterEffect(e2)
end
function c71200846.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c71200846.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsCanBeDisabledByEffect(e) then
		...
		if not c:IsStatus(STATUS_ACT_FROM_HAND) and c:IsLocation(LOCATION_SZONE) then
			...
			e5:SetOperation(c71200846.disop)
