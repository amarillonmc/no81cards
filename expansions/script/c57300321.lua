...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_POSITION+CATEGORY_TOEXTRA)
	...
	e4:SetCategory(CATEGORY_POSITION+CATEGORY_TOEXTRA)
...
function s.ctdfilter(c)
	return c:IsRankAbove(1) and c:IsFaceupEx() and c:IsAbleToExtra()
end
function s.gcheck(g,tp,c)
	return g:GetSum(s.lv_or_rk)==s.lv_or_rk(c)
end
...
function s.setfilter(c,tp)
	local g=Duel.GetMatchingGroup(s.ctdfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	return c:IsFaceup() and (c:IsLevelAbove(1) or c:IsRankAbove(1)) and c:IsCanTurnSet()
		and g:CheckSubGroup(s.gcheck,1,12,tp,c)
end
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and s.setfilter(chkc) and chkc:IsControler(1-tp) end
...
function s.setop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() then
		local sg=g:SelectSubGroup(tp,s.gcheck,false,1,12,tp,tc)
		if Duel.SendtoDeck(sg,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)~=0 and sg:FilterCount(Card.IsLocation,nil,LOCATION_EXTRA)~=0 then
