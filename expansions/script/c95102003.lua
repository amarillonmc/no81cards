...
function c95102003.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c95102003.filter1,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,tp) end
end
function c95102003.op1(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c95102003.filter1),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,tp):GetFirst()
...
function c95102003.op2(e,tp,eg,ep,ev,re,r,rp)
    ...
    if c:IsRelateToEffect(e) and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 then
        ...
        e1:SetDescription(aux.Stringid(95102003,2))
        e1:SetValue(1)
        ...
        e3:SetDescription(aux.Stringid(95102003,3))
        c:RegisterEffect(e3,true)
        local e4=e1:Clone()
        e4:SetCode(EFFECT_CANNOT_BE_SYNCHRO_MATERIAL)
        e4:SetDescription(aux.Stringid(95102003,4))
        c:RegisterEffect(e4,true)
        local e5=e1:Clone()
        e5:SetCode(EFFECT_CANNOT_BE_XYZ_MATERIAL) 
        e5:SetDescription(aux.Stringid(95102003,5))
        c:RegisterEffect(e5,true)
        local e6=e1:Clone()
        e6:SetCode(EFFECT_CANNOT_BE_LINK_MATERIAL)
        e6:SetDescription(aux.Stringid(95102003,6))
        c:RegisterEffect(e6,true)
    end
end
function c95102003.lim(e,c,st)
