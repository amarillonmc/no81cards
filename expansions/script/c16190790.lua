...
function s.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_RECOVER+CATEGORY_SPECIAL_SUMMON+CATEGORY_DECKDES)
...
function s.distg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    if e:GetHandler():IsStatus(STATUS_SET_TURN) and e:GetLabel()==1 then
    	e:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY+CATEGORY_SPECIAL_SUMMON+CATEGORY_GRAVE_SPSUMMON+CATEGORY_DECKDES)
    end
end
...
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateActivation(ev) and rc:IsRelateToEffect(re) and Duel.Destroy(eg,REASON_EFFECT)~=0
    	and e:GetHandler():IsStatus(STATUS_SET_TURN) and e:GetLabel()==1 
...
function s.dspfilter(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsCode(16190750)
end
function s.rcop(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	if Duel.Recover(p,d,REASON_EFFECT)~=0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
