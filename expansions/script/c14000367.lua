...
function cm.spfilter1(c,tp,mc)
	...
	return c:IsCode(14000357) and c:IsReleasable() and Duel.IsExistingMatchingCard(cm.spfilter2,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,g)
end
...
function cm.stg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.tdfilter,tp,LOCATION_GRAVE,0,3,nil) and Duel.IsExistingMatchingCard(function(c) return c:IsAbleToRemove() and not c:IsType(TYPE_TOKEN) end,tp,0,LOCATION_ONFIELD+LOCATION_HAND+LOCATION_GRAVE,1,nil) end
...
function cm.sop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		local hg=Duel.GetMatchingGroup(function(c) return c:IsAbleToRemove() and not c:IsType(TYPE_TOKEN) end,tp,0,LOCATION_HAND,nil)
		local g=Duel.GetMatchingGroup(function(c) return c:IsAbleToRemove() and not c:IsType(TYPE_TOKEN) end,tp,0,LOCATION_ONFIELD+LOCATION_GRAVE,nil)
...
function cm.filter(c,e,tp)
	return c:IsCode(14000357) and (c:IsAbleToHand() or c:IsCanBeSpecialSummoned(e,0,tp,false,false))
end
