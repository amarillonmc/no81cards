...
function cm.cfilter(c,e,tp)
	return c:IsDiscardable() and Duel.IsExistingMatchingCard(cm.spellfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,c,e,tp)
end
...
function cm.filter1_1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function cm.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c,14000055) and (not f or f(c))
...
function cm.futg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		local chkf=tp
		local mg1=Duel.GetMatchingGroup(Card.IsCanBeFusionMaterial,tp,LOCATION_ONFIELD,0,c)
		local mg2=Duel.GetMatchingGroup(cm.filter0,tp,0,LOCATION_ONFIELD,nil)
		mg1:Merge(mg2)
...
function cm.fuop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetMatchingGroup(cm.filter1_1,tp,LOCATION_ONFIELD,0,nil,e)
	local mg2=Duel.GetMatchingGroup(cm.filter1,tp,0,LOCATION_ONFIELD,nil,e)
	mg1:Merge(mg2)
