...
function c10113096.filter(c,e,tp,m1,m2,ft)
	...
	if ft>0 then
		...
		return mg:IsExists(c10113096.mfilterf,1,nil,tp,mg,c)
	end
end
...
function c10113096.mfilter(c)
	return c:GetLevel()>0 and c:IsReason(REASON_RITUAL) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemove()
end
function c10113096.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg1=Duel.GetRitualMaterial(tp)
		local mg2=Duel.GetMatchingGroup(c10113096.mfilter,tp,LOCATION_GRAVE,0,nil)
		local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
		return ft>-1 and Duel.IsExistingMatchingCard(c10113096.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg1,mg2,ft)
	end
...
function c10113096.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c10113096.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg1,mg2,ft)
	local tc=g:GetFirst()
	if tc then
		...
		if ft>0 then
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
			mat=mg:FilterSelect(tp,c10113096.mfilterf,1,1,nil,tp,mg,tc)
