...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then 
		return (b1 or b2) and g:GetSum(Card.GetAttack)>=2600
	end

	local sg=Group.CreateGroup()
	while true do

		local mg=g:Filter(aux.TRUE,sg)
		if #mg==0 then break end

		local finish = sg:GetSum(Card.GetAttack)>=2600
		
		local min = finish and 0 or 1
		
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
		local sel=mg:Select(tp,min,1,nil)
