...
function c40009193.settg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c40009193.setfilter,tp,LOCATION_PZONE,0,1,nil,e,tp,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c40009193.setfilter,tp,LOCATION_PZONE,0,1,1,nil,e,tp,e:GetHandler())
...
function c40009193.filter1(c,e,tp)
	...
	local rk=c:GetRank()
	return c:IsFaceup() and c:IsSetCard(0xf1c)
		and Duel.IsExistingMatchingCard(c40009193.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk+1)
...
function c40009193.filter2(c,e,tp,mc,rk)
	return c:IsRank(rk) and (c:IsType(TYPE_XYZ) and c:IsType(TYPE_PENDULUM)) and mc:IsCanBeXyzMaterial(c)
