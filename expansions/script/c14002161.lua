...
function cm.spfilter(c,e,tp)
	return cm.Almotaher(c) and c:IsLevelBelow(5) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function cm.sumcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tp~=ep and eg and eg:IsExists(cm.cfilter,1,nil,1-tp)
end
function cm.exfilter(c,e,tp,mc)
	return c:IsCode(14002128) and mc:IsCanBeXyzMaterial(c) and c:IsType(TYPE_XYZ)
...
function cm.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_XMATERIAL)
		and Duel.IsExistingMatchingCard(cm.exfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function cm.sumsuc(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local mg=Group.FromCards(c)
	if not aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_XMATERIAL) or c:IsStatus(STATUS_BATTLE_DESTROYED) then return end
	if Duel.SelectYesNo(tp,aux.Stringid(m,3)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,cm.exfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,c)
		local tc=g:GetFirst()
		if tc then
			tc:SetMaterial(mg)
			Duel.Overlay(tc,mg)
