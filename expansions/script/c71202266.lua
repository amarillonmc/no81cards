...
function s.initial_effect(c)
	--①: 自己·对方回合1次，这张卡在墓地存在的场合，可以从以下选择1个发动（这个卡名的以下效果1回合各能选择1次，同一连锁上不能发动）
...
function s.oop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		...
		if c:IsRelateToEffect(e)
			...
			-- 以自己场上的怪兽为对象发动的场合，可以再选对方墓地的怪兽和魔法·陷阱卡各最多1张除外
			if tc:IsControler(tp) then
				local g1=Duel.GetMatchingGroup(s.rmfilter,tp,0,LOCATION_GRAVE,nil)
				local m=g1:Filter(Card.IsType,nil,TYPE_MONSTER)
				local st=g1:Filter(Card.IsType,nil,TYPE_SPELL+TYPE_TRAP)
				local msel=nil
				local stsel=nil
				if #m>0 and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
					msel=Duel.SelectMatchingCard(tp,s.rmfilter,tp,0,LOCATION_GRAVE,1,1,nil)
				end
				if #st>0 and Duel.SelectYesNo(tp,aux.Stringid(id,5)) then
					Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
					stsel=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.rmfilter),tp,0,LOCATION_GRAVE,1,1,nil,TYPE_SPELL+TYPE_TRAP)
					stsel=stsel:Filter(Card.IsType,nil,TYPE_SPELL+TYPE_TRAP)
				end
				local g=Group.CreateGroup()
				if msel then g:Merge(msel) end
				if stsel then g:Merge(stsel) end
				if #g>0 then
					Duel.BreakEffect()
