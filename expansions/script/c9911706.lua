...
function c9911706.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH+CATEGORY_GRAVE_ACTION+CATEGORY_SSET)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_CUSTOM+9911706)
	e2:SetProperty(EFFECT_FLAG_DELAY)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetCondition(c9911706.thcon)
	...
	e3:SetCondition(c9911706.recon)
	e3:SetValue(LOCATION_REMOVED)
	c:RegisterEffect(e3)
	if not c9911706.global_check then
		c9911706.global_check=true
		local ge1=Effect.CreateEffect(c)
		ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge1:SetCode(EVENT_SUMMON_SUCCESS)
		ge1:SetOperation(c9911706.checkop)
...
function c9911706.checkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	while tc and tc:IsFaceup() do
		tc:RegisterFlagEffect(9911706,RESET_EVENT+RESETS_STANDARD-RESET_TURN_SET,0,1)
		tc=eg:GetNext()
	end
	Duel.RaiseEvent(eg,EVENT_CUSTOM+9911706,re,r,rp,ep,ev)
end
...
function c9911706.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return eg:IsContains(c) and c:GetFlagEffect(9911706)>0
end
