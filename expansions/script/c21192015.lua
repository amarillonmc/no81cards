...
function cm.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DAMAGE+CATEGORY_RECOVER)
...
function cm.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetFlagEffect(tp,m-15)<=2 and (b1 or b2) end
	Duel.RegisterFlagEffect(tp,m-15,RESET_PHASE+PHASE_END,0,1)
...
function cm.w(c)
	return c:IsFaceupEx() and c:IsSetCard(setcard) and c:IsType(1) and not c:IsForbidden() and not c:IsCode(m) 
end
function cm.tg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(cm.w,tp,0x30,0,1,nil) and Duel.GetLocationCount(tp,8)>0 end
	Duel.Hint(3,t,HINTMSG_TOFIELD)
	Duel.SelectTarget(tp,cm.w,tp,0x30,0,1,1,nil)
end
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) and Duel.GetLocationCount(tp,8)>0 and Duel.MoveToField(tc,tp,tp,LOCATION_SZONE,POS_FACEUP,true) then
	local e1=Effect.CreateEffect(c)
