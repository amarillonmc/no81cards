...
function c40009038.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(40009038,0))
	...
	e7:SetDescription(aux.Stringid(40009038,1))
	...
	e7:SetCost(c40009038.spcost)
	e7:SetCondition(c40009038.spcon)
	e7:SetTarget(c40009038.sptg)
	e7:SetOperation(c40009038.spop)
	c:RegisterEffect(e7)  
	c:RegisterEffect(e1)   
	--atk up
...
function c40009038.cfilter(c)
	return c:GetSequence()>=5
end
function c40009038.spcon(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(c40009038.cfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c40009038.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)
end
function c40009038.spfilter1(c,e,tp,ec)
...
function c40009038.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c40009038.spfilter1,tp,LOCATION_EXTRA,0,1,nil,e,tp,e:GetHandler()) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function c40009038.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c40009038.spfilter1,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,e:GetHandler())
	local tc=g:GetFirst()
