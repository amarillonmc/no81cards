...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif c:IsLocation(LOCATION_SZONE) then
		if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
			...
			if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.sfilter,tp,LOCATION_ONFIELD,0,1,nil,e,tp) then
				if Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
				local g=Duel.SelectMatchingCard(tp,s.sfilter,tp,LOCATION_ONFIELD,0,1,1,nil,e,tp) 
					if #g>0 then
