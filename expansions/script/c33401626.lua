...
function cm.filter0(c,e)
	return c:IsSetCard(0x341) and c:IsType(TYPE_MONSTER) and c:IsCanBeFusionMaterial() and c:IsAbleToGrave() and  Duel.IsExistingMatchingCard(cm.ckfilter2,tp,LOCATION_MZONE,0,1,nil,c:GetAttribute())
end
...
function cm.filter22(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x6344) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,chkf)
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg5=Duel.GetMatchingGroup(cm.filter0,tp,LOCATION_DECK,0,nil,e)
		mg5:Merge(mg1)
