...
function s.spfilter(c,e,tp,sc)
    return c:IsCode(id)==false and 
        (c:IsSetCard(0x85) or c:IsCode(71071546,68812773)) 
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
        and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e,tp,e:GetHandler()) 
    end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)    
    ...
    local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,e,tp,e:GetHandler())
    if #g>0 then
