...
function s.initial_effect(c)
	...
	e4:SetCondition(s.rmlicon)
	e4:SetTarget(s.rmlimit)
	c:RegisterEffect(e4)
end
function s.chkfilter(c)
	return c:IsCode(71200276) and not c:IsPublic()
end
...
function s.thfilter(c)
	return c:IsSetCard(0x8887) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
