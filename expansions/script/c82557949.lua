...
function c82557949.filter12(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x829)
		and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c82557949.filter2),tp,LOCATION_GRAVE,LOCATION_GRAVE,1,c)
end
...
function c82557949.mtop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c82557949.filter2),tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,tc,e)
	if g:GetCount()>0 then
