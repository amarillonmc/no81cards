...
function s.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(88884426,0))
...
function s.thfilter(c,lv)
	return c:IsType(TYPE_MONSTER) and c:GetLevel()<lv and c:IsSetCard(0x893) and c:IsAbleToHand() 
end
function s.cfilter(c,tp)
	local lv=c:GetLevel()
	return c:IsPreviousLocation(LOCATION_MZONE) and c:IsSetCard(0x893)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local g=eg:Filter(Card.IsSetCard,nil,0x893)
