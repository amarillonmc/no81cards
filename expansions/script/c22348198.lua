...
function cm.initial_effect(c)
	...
	--count
		...
		e5:SetCondition(c22348198.checkcon)
		e5:SetOperation(c22348198.checkop)
		c:RegisterEffect(e5)
end
...
function c22348198.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local gg=ag:Filter(c22348198.thhfilter,nil,re)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 and gg:GetCount()~=0 and Duel.SelectYesNo(tp,aux.Stringid(22348198,3)) then
		Duel.BreakEffect()
...
function c22348198.tdcon2(e,tp,eg,ep,ev,re,r,rp)
	local ttp=e:GetHandler():GetControler()
	return Duel.IsPlayerAffectedByEffect(tp,22348205) and Duel.GetFlagEffect(ttp,22349198)==0 and not (e:GetHandler():GetFlagEffect(22348198)>0)
end
...
function c22348198.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c22348198.tdfilter,tp,LOCATION_GRAVE,0,1,3,e:GetHandler())
