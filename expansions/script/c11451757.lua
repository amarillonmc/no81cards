...
function cm.initial_effect(c)
	...
	e10:SetCondition(cm.XyzLevelFreeCondition(cm.mfilter,cm.xyzcheck,2,99))
	e10:SetTarget(cm.XyzLevelFreeTarget(cm.mfilter,cm.xyzcheck,2,99))
	e10:SetOperation(cm.XyzLevelFreeOperation(cm.mfilter,cm.xyzcheck,2,99))
	...
	e1:SetCategory(CATEGORY_POSITION+CATEGORY_SSET+CATEGORY_MSET)
...
function cm.XyzLevelFreeCondition(f,gf,minct,maxct)
	return  function(e,c,og,min,max)
				...
				if og then
					...
					local loc=c:GetFlagEffect(m)>0 and LOCATION_HAND+LOCATION_MZONE or LOCATION_MZONE
				...
				aux.GCheckAdditional=function(g,...) return aux.TuneMagicianCheckAdditionalX(EFFECT_TUNE_MAGICIAN_X)(g,...) and (c:GetFlagEffect(m)==0 or g:FilterCount(Card.IsLocation,nil,LOCATION_HAND)<=1) end
				local gfc=function(g,...) return gf(g,...) and (c:GetFlagEffect(m)==0 or g:FilterCount(Card.IsLocation,nil,LOCATION_HAND)<=1) end
...
function cm.XyzLevelFreeOperation(f,gf,minct,maxct)
	return  function(e,tp,eg,ep,ev,re,r,rp,c,og,min,max)
				if og and not min then
					...
					c:SetMaterial(mg)
					Duel.Overlay(c,mg)
					if mg:IsExists(Card.IsPreviousLocation,1,nil,LOCATION_HAND) then Duel.ShuffleHand(tp) end
					mg:DeleteGroup()
				end
			end
end
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local res=(c:IsCanBeSpecialSummoned(te,SUMMON_TYPE_XYZ,tp,true,true) and cm.XyzLevelFreeCondition(cm.mfilter,cm.xyzcheck,2,99)(te,c,nil,2,99) and c:IsAbleToDeck() and c:GetFlagEffect(m-10)==0)
		c:ResetFlagEffect(m)
		return res
	end
