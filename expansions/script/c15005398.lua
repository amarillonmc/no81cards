...
function cm.rfilter(c)
	return c:IsSetCard(0xaf3c) and c:IsAbleToRemove() and c:IsType(TYPE_SPELL+TYPE_TRAP)
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.rfilter,tp,LOCATION_DECK,0,1,nil,tp) end
...
function cm.tkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (ph==PHASE_MAIN1 or ph==PHASE_MAIN2) and not Duel.IsExistingMatchingCard(cm.nonfilter,tp,LOCATION_MZONE,0,1,nil,ATTRIBUTE_WIND)
end
...
function cm.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.rmfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil,tp) end
...
function cm.tkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		tc:RegisterFlagEffect(15005398,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,tcid,aux.Stringid(15005398,7))
...
function cm.bedesop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsReason(REASON_DESTROY) then
		...
		if tc and tc:GetFlagEffect(15005398)~=0 then
			local tcid=tc:GetFlagEffectLabel(15005398)
			if tcid==tc:GetFieldID() then
				Duel.Hint(HINT_CARD,1-tp,15005398)
