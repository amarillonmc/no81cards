...
function c60152316.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(60152316,1))
	...
	e99:SetDescription(aux.Stringid(60152316,2))
...
function c60152316.e1tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c60152316.e1tgfilter,tp,LOCATION_GRAVE+LOCATION_OVERLAY,LOCATION_GRAVE+LOCATION_OVERLAY,1,nil,e) and e:GetHandler():IsFaceup() and e:GetHandler():IsLocation(LOCATION_MZONE) end
...
function c60152316.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c60152316.e1tgfilter),tp,LOCATION_GRAVE+LOCATION_OVERLAY,LOCATION_GRAVE+LOCATION_OVERLAY,1,1,nil,e)
...
function c60152316.e2op(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		while tc do
			...
			if tc:GetAttack()<=1800 and not tc:IsDisabled() then
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		if g:GetCount()>0 then
			...
			if Duel.IsPlayerAffectedByEffect(tp,60152321) then
				...
				e1:SetValue(c60152316.e99opfilter)
