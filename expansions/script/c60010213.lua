...
function cm.hsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsExistingMatchingCard(cm.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,e:GetHandler()) end
...
function cm.hspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Destroy(Duel.SelectMatchingCard(tp,cm.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,nil),REASON_EFFECT) then
...
function cm.rstg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local res=mg:IsContains(c) and Duel.IsExistingMatchingCard(aux.RitualUltimateFilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,cm.rfilter,e,tp,mg,nil,Card.GetLevel,"Greater")
...
function cm.rsop(e,tp,eg,ep,ev,re,r,rp)
	...
	aux.RCheckAdditional=cm.rcheck(c)
	local tg=Duel.SelectMatchingCard(tp,aux.RitualUltimateFilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,cm.rfilter,e,tp,mg,nil,Card.GetLevel,"Greater")
