...
function cm.mfilter(c)
	...
	return not cm[tp+10000][rc] and cm[tp+10002][rc] and c:IsFaceup() and c:IsAbleToDeckOrExtraAsCost()
end
...
function cm.imop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)>=Duel.GetFieldGroupCount(tp,0,LOCATION_HAND) or Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)>=Duel.GetFieldGroupCount(tp,0,LOCATION_ONFIELD) then return end
	local res=true
	local i=1
	while type(cm[i])=="table" do
		local re,rp=table.unpack(cm[i])
...
function cm.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsRace(RACE_PSYCHO) then
		...
		if cm[p+2][rc] then cm[p+2][rc]=cm[p+2][rc]+1 else cm[p+2][rc]=1 end
	end
end
function cm.regop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if cm[p+2][rc] then cm[p+2][rc]=cm[p+2][rc]-1 else cm[p+2][rc]=nil end
end
