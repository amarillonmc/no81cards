...
function s.eqfilter(c,e,tp)
	return c:IsSetCard(0x54c) and c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_GRAVE) or c:IsLocation(LOCATION_REMOVED))
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsLocation(LOCATION_SZONE) and c:GetEquipTarget()~=nil
		...
		and (c:IsAbleToHand() or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)))
end
