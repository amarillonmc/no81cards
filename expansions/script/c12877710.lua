...
function s.cfilter(c,code)
	return c:IsSetCard(0x5a73) and c:IsType(TYPE_MONSTER) and not c:IsCode(code) and c:IsDiscardable(REASON_EFFECT)
end
function s.e1tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return aux.nbcon(tp,re) and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_HAND,0,1,nil,c:GetCode()) and c:GetFlagEffect(id)==0 end
...
function s.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_HAND,0,1,1,nil,c:GetCode())
	if #sg>0 and Duel.SendtoGrave(sg,REASON_EFFECT+REASON_DISCARD)>0 then
		if Duel.NegateActivation(ev) and re:GetHandler():IsRelateToEffect(re) and re:GetHandler():IsAbleToRemove(tp,POS_FACEDOWN,REASON_EFFECT) then
