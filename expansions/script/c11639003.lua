...
function cm.initial_effect(c)
	...
	e4:SetCategory(CATEGORY_COUNTER+CATEGORY_TODECK)
...
function cm.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,PLAYER_ALL,LOCATION_SZONE)
end
function cm.tgop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsCanRemoveCounter(tp,LOCATION_ONFIELD,LOCATION_ONFIELD,0x1164,1,REASON_EFFECT) then
		if Duel.RemoveCounter(tp,LOCATION_ONFIELD,LOCATION_ONFIELD,0x1164,1,REASON_EFFECT) and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,LOCATION_SZONE,LOCATION_SZONE,1,nil) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
			local g=Duel.SelectMatchingCard(tp,Card.IsAbleToDeck,tp,LOCATION_SZONE,LOCATION_SZONE,1,1,nil)
			if g:GetCount()>0 then
