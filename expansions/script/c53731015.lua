...
function cm.thfilter(c)
	return c:IsSetCard(0x9533) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
function cm.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() and Duel.IsExistingMatchingCard(cm.thfilter,tp,LOCATION_GRAVE,0,1,nil) end
...
function cm.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SendtoHand(Group.__add(g,c),nil,REASON_EFFECT)
end
