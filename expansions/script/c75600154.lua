...
function s.costcheck(c,ec,e,tp)
	...
	or Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,c)
end
function s.tgfilter(c,ec,e,tp)
	return c:IsFaceup() and (c:IsSetCard(0xa2c1) or (c:IsSetCard(0x52c0) and c:IsType(TYPE_EQUIP))) and c:IsAbleToGraveAsCost() and s.costcheck(c,ec,e,tp)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.IsCanRemoveCounter(tp,1,0,0x1b,2,REASON_COST) and s.costcheck(nil,e:GetHandler(),e,tp)
...
function s.spfilter(c,e,tp,rc)
	return c:IsFaceupEx() and c:IsSetCard(0xa2c1) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetMZoneCount(tp,rc)>0
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b1=(Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,nil) or b0)
