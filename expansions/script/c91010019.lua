...
function s.excon(tp)
	return not Duel.IsExistingMatchingCard(s.exconfilter,tp,LOCATION_MZONE,0,1,nil)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if s.excon(tp) then
			local mg2=Duel.GetMatchingGroup(s.fexfilter,tp,LOCATION_EXTRA,0,nil)
			if mg2:GetCount()>0 then
				mg1:Merge(mg2)
				aux.FCheckAdditional=s.frcheck
