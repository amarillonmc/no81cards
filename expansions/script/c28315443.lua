...
function c28315443.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.IsExistingTarget(aux.NecroValleyFilter(c28315443.tdfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,aux.NecroValleyFilter(c28315443.tdfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,2,nil)
...
function c28315443.atkfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x284) and not c:IsCode(28315443)
end
function c28315443.atkcon(e)
	local c=e:GetHandler()
	return Duel.IsExistingMatchingCard(c28315443.atkfilter,c:GetControler(),LOCATION_MZONE,0,1,nil)
end
