...
function cm.thfilter(c)
	return c:IsSetCard(0x3f44) and c:IsAbleToHand() and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_TUNER)
end
...
function cm.mbop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		local ph=Duel.GetCurrentPhase()
		if ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE and g:GetFirst():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
			Duel.BreakEffect()
