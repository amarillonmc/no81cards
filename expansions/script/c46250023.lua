...
function c46250023.initial_effect(c)
	...
	e2:SetCondition(c46250023.sppcon)
	e2:SetOperation(c46250023.sppop)
	c:RegisterEffect(e2)
...
function c46250023.cfilter1(c,tp,g)
	return g:IsExists(c46250023.cfilter2,1,c,tp,c)
end
function c46250023.cfilter2(c,tp,mc)
	return c:IsType(TYPE_MONSTER) and mc:IsType(TYPE_MONSTER) and (c:IsFusionAttribute(ATTRIBUTE_DARK) and not mc:IsFusionAttribute(ATTRIBUTE_DARK) or not c:IsFusionAttribute(ATTRIBUTE_DARK) and mc:IsFusionAttribute(ATTRIBUTE_DARK)) and Duel.GetLocationCountFromEx(tp,tp,Group.FromCards(c,mc))>0
end
function c46250023.sppcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(Card.IsAbleToGraveAsCost,tp,LOCATION_ONFIELD,0,nil)
	return g:IsExists(c46250023.cfilter1,1,nil,tp,g)
end
function c46250023.sppop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(Card.IsAbleToGraveAsCost,tp,LOCATION_ONFIELD,0,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g1=g:FilterSelect(tp,c46250023.cfilter1,1,1,nil,tp,g)
	local mc=g1:GetFirst()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g2=g:FilterSelect(tp,c46250023.cfilter2,1,1,mc,tp,mc)
	g1:Merge(g2)
...
function c46250023.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsReleasable() and Duel.IsExistingMatchingCard(Card.IsReleasable,tp,LOCATION_MZONE,LOCATION_MZONE,1,c) end
