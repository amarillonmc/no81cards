...
function cm.tgfilter(c,e,tp)
	if c:IsFacedown() or (c:GetOriginalType()&TYPE_MONSTER>0 and c:IsSummonableCard()) then return false end
	return Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_EXTRA,0,1,nil,c,e,tp)
end
function cm.spfilter(c,tc,e,tp)
	local x0,y0=cm.xylabel(tc,tp)
	local list={5/4,3/2,7/4,1,1000,0,3/4,1/2,1/4}
	local lk={}
	for i=0,8 do
		if c:IsLinkMarker(1<<i) and not Duel.IsExistingMatchingCard(cm.isdir,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c,x0,y0,tp,list[i+1]*math.pi) then return false end
	end
	return c:IsType(TYPE_LINK) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
function cm.stg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and cm.tgfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.GetLocationCountFromEx(tp)>0 and Duel.IsExistingTarget(cm.tgfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,cm.tgfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,e,tp)
...
function cm.sop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not sc or not sc:IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,sc,e,tp)
	local tc=g:GetFirst()
	if not tc then return end
	if Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 and c:IsRelateToEffect(e) and tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() then
		Duel.Equip(tp,c,tc)
		--equip limit
		local e1=Effect.CreateEffect(c)
