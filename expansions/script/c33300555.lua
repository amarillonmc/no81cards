...
function s.spfilter(c,e,tp)
	return not c:IsCode(id) and c:IsSetCard(0x5569) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,1-tp)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SpecialSummon(g,0,tp,1-tp,false,false,POS_FACEUP)~=0 then
		if Duel.IsExistingMatchingCard(aux.AND(Card.IsAttribute,Card.IsFaceupEx),tp,LOCATION_REMOVED,LOCATION_MZONE,3,nil,ATTRIBUTE_LIGHT) and Duel.IsExistingMatchingCard(Card.IsAbleToHand,tp,0,LOCATION_ONFIELD,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			Duel.BreakEffect()
