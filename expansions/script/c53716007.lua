...
function cm.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (c:IsLocation(LOCATION_HAND) or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,m,0x353b,TYPES_NORMAL_TRAP_MONSTER,1900,700,4,RACE_ZOMBIE,ATTRIBUTE_LIGHT))) and Duel.GetFlagEffect(tp,m)==0 and not c:IsLocation(LOCATION_ONFIELD) end
