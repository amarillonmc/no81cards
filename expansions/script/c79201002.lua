...
function s.initial_effect(c)
	...
	e2:SetCost(s.sppcost)
	e2:SetTarget(s.spptg)
	e2:SetOperation(s.sppop)
	c:RegisterEffect(e2)
end
...
function s.sppfilter(c,tp,rp)
	return c:IsControler(tp)
end
function s.epfilter(c,e,tp)
	return c:IsSetCard(0x10e3) and c:IsAbleToRemoveAsCost()
end
function s.sppcost(e,tp,eg,ep,ev,re,r,rp,chk) 
	if chk==0 then return Duel.IsExistingMatchingCard(s.epfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil) end
