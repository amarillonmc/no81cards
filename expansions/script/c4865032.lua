...
function cm.initial_effect(c)
    --synchro summon
    aux.AddSynchroMixProcedure(c,aux.Tuner(Card.IsSetCard,0x332b),nil,nil,aux.NonTuner(nil),1,99,cm.syncheck(c))
    c:EnableReviveLimit()
...
function cm.cfilter(c,syncard)
    return c:IsSetCard(0x332b) and c:IsSynchroType(TYPE_SYNCHRO) and c:IsNotTuner(syncard)
end
...
function cm.atkfilter(c)
    return c:IsSetCard(0x332b) and c:IsType(TYPE_MONSTER)
end
function cm.atkval(e,c)
    return Duel.GetMatchingGroupCount(cm.atkfilter,e:GetHandlerPlayer(),LOCATION_GRAVE,0,nil)
end
