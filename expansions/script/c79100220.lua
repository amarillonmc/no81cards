...
function cm.initial_effect(c)
	aux.AddCodeList(c,79100200)
	--ritual summon
	local e1=aux.AddRitualProcGreater(c,cm.filter,LOCATION_HAND+LOCATION_GRAVE,nil,nil,true,cm.extraop)
...
function cm.filter1(c)
	return c:IsType(TYPE_RITUAL) and c:IsType(TYPE_SPELL) and (c:IsAbleToHand() or c:IsAbleToGrave())
end
