...
function s.tdfilter(c)
	return not c:IsCode(id) and c:IsSetCard(0x3cf4) and c:IsAbleToDeck() and c:IsLocation(LOCATION_GRAVE)
end
...
function s.disfilter(c)
	return c:IsLevelAbove(8) and c:IsFaceup() and c:IsSummonType(SUMMON_TYPE_ADVANCE)
end
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.disfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.CheckReleaseGroup(tp,nil,1,nil,tp)
		and rp==1-tp and Duel.IsChainDisablable(ev) and re:IsActiveType(TYPE_MONSTER)
