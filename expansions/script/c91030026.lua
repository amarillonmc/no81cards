...
function cm.tgfilter(c,tp)
	return c:IsFaceup() and Duel.IsExistingMatchingCard(cm.eqfilter,tp,LOCATION_REMOVED+LOCATION_GRAVE,0,1,c)
end
function cm.eqfilter(c,tp)
	return c:IsFaceup()  and c:CheckUniqueOnField(tp) and not c:IsForbidden() and c:IsSetCard(0x9d3)
end
...
function cm.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ec=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(cm.eqfilter),tp,LOCATION_REMOVED+LOCATION_GRAVE,0,1,1,tc,tp):GetFirst()
