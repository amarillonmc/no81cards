...
function c98730319.spfilter(c,e,tp)
    return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsFaceup() and c:IsCanBeSpecialSummoned(e,nil,tp,false,false) and c:GetOriginalLevel()>0 and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
function c98730319.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c98730319.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
