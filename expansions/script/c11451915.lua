...
function cm.psptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local lock1=(c:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp,LOCATION_MZONE)==0) or (c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp)==0)
		local lock2=false
