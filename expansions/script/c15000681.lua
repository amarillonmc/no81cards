...
function cm.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return tc and tc:IsReason(REASON_BATTLE+REASON_EFFECT) and not tc:IsReason(REASON_REPLACE) and c:IsCanRemoveCounter(tp,0x1f35,1,REASON_EFFECT) end
	return Duel.SelectEffectYesNo(tp,c,96)
end
function cm.desrepop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=c:GetEquipTarget()
	c:RemoveCounter(tp,0x1f35,1,REASON_EFFECT+REASON_REPLACE)
	Duel.RaiseEvent(e:GetHandler(),EVENT_REMOVE_COUNTER+0x1f35,e,REASON_EFFECT+REASON_REPLACE,tp,tp,1)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	...
	tc:RegisterEffect(e1)
	Duel.Hint(HINT_CARD,0,15000681)
end