...
function cm.matfilter(c)
	return c:IsLinkSetCard(0x876) and not c:IsLinkCode(m)
end
...
function cm.ddop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.DiscardDeck(tp,1,REASON_EFFECT)==1 then
		...
		if tc:IsSetCard(0x876) and tc:IsAbleToHand() and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
...
function cm.rmfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_SPELL) and c:IsSetCard(0x876)
end
...
function cm.cpfilter(c)
	return c:IsType(TYPE_SPELL) and c:IsSetCard(0x876) and c:IsAbleToRemoveAsCost() and c:CheckActivateEffect(false,true,false)
end
