...
function s.initial_effect(c)
	...
	e1:SetCondition(s.XyzCondition(nil,7,2,2))
	e1:SetTarget(s.XyzTarget(nil,7,2,2))
	e1:SetOperation(s.XyzOperation(nil,7,2,2))
...
function s.rmlimit(e,c,rp,r,re)
	return c==e:GetHandler() and r&REASON_EFFECT>0 and r&REASON_REDIRECT==0 and rp~=e:GetHandlerPlayer()
end
function s.raval(e,c)
	return e:GetHandler():GetOverlayCount()-1
end
function s.repfilter(c)
	return c:IsLocation(LOCATION_HAND+LOCATION_DECK+LOCATION_ONFIELD) and c:GetDestination()==LOCATION_GRAVE
end
...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tde=Effect.CreateEffect(e:GetHandler())
...
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	local sg=e:GetLabelObject():Filter(function(c) return c:GetFlagEffect(id)>0 end,nil)
	for tc in aux.Next(sg) do
