...
function s.extg(e, tp, eg, ep, ev, re, r, rp, chk)
	...
	Duel.SetOperationInfo(0, CATEGORY_DAMAGE, nil, 1000, 1-tp, 0)
end
...
function s.exop(e, tp, eg, ep, ev, re, r, rp)
	...
	if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 then
		...
		if tdg and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then 
			Duel.BreakEffect()
			local tc = tdg:Select(tp,1,1,nil)
...
function s.lfop(e,tp,eg,ep,ev,re,r,rp)
			local c= e:GetHandler()
			if (not Duel.SelectYesNo(tp,aux.Stringid(id,1)))  or  Duel.Destroy(e:GetHandler(),REASON_EFFECT) == 0 then return end
...
function s.damcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return ep==tp and (re:GetHandler():IsCode(31740001) or aux.IsCodeListed(re:GetHandler()))
end
...
function s.skrfilter(c)
	return c:IsFaceup() and c:IsCode(31740001) and c:IsAbleToRemove()
end
