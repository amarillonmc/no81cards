...
function cm.initial_effect(c)
	...
	e0:SetTarget(cm.hatg)
	e0:SetOperation(cm.haop)
	c:RegisterEffect(e0)
	local e1=e0:Clone()
...
function cm.hatg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.AND(Card.IsFaceupEx,Card.IsCode),tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,nil,14002125) and Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_MZONE,1,nil) end
...
function cm.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	return rc:IsOnField() and re:IsActiveType(TYPE_MONSTER)
end
function cm.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then 
		local g=Duel.GetDecktopGroup(tp,1)
		return #g>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) 
	end
