...
function c46250007.lkfilter(c,tc)
    return c:IsSetCard(0x2fc0) and c:IsLinkSummonable(nil,tc)
end
function c46250007.lktg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c46250007.lkfilter,tp,LOCATION_EXTRA,0,1,nil,e:GetHandler()) end
    Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function c46250007.lkop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if c:IsControler(1-tp) or not c:IsRelateToEffect(e) then return end
    local g=Duel.GetMatchingGroup(c46250007.lkfilter,tp,LOCATION_EXTRA,0,nil,c)
