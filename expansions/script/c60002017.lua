...
function cm.cvfilter1(c)
	return c:IsSetCard(0xa620) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(m)
end
function cm.cvop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetTurnCount()+t>=9 and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
	   Duel.Draw(tp,9,REASON_EFFECT)
	end
end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	local t=Duel.GetFlagEffect(tp,60002009)
	if Duel.GetTurnCount()+t>=9 then
		local u=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_HAND,0,nil)
		local n=Duel.GetMatchingGroupCount(aux.TRUE,tp,LOCATION_HAND,0,nil)
		for i=1,n do
			local g1=u:GetFirst()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g=Duel.SelectMatchingCard(tp,Card.IsCode,tp,LOCATION_DECK,0,1,1,nil,g1:GetCode())
			if g:GetCount()>0 then
