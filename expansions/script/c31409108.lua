...
function cm.con(e)
	return e:GetHandler():GetFlagEffect(m)~=0 or Duel.GetFlagEffect(e:GetHandlerPlayer(),m)==0
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return cm.con(e) and Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK,0,1,nil) end
	e:GetHandler():RegisterFlagEffect(m,RESET_PHASE+PHASE_END+RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,nil,aux.Stringid(m,0))
	Duel.RegisterFlagEffect(e:GetHandlerPlayer(),m,RESET_PHASE+PHASE_END,nil,1)
