--霓创深域载入
local s,id,o=GetID()
function c30553401.initial_effect(c)
	--Activate
...
function s.checkfilter(c)
	return c:IsCode(30553411) and c:IsFaceup()
end
function s.thfilter(c,check)
	local b1=c:IsCode(30553411)
	local b2=check and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsSetCard(0x309)
	return  (b1 or b2) and c:IsSSetable()
end
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x309) and c:IsOnField()
