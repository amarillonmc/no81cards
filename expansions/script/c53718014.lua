...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SendtoGrave(g,REASON_EFFECT)~=0 then
		...
		if #dg>0 and Duel.IsExistingMatchingCard(function(c)return c:IsFaceup() and c:IsSetCard(0x653c)end,tp,LOCATION_MZONE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
			Duel.BreakEffect()
