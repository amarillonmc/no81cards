...
function s.pbop(e,tp,eg,ep,ev,re,r,rp)
 ...
 --   e2:SetValue(1)
  ...
  --  e2:SetLabelObject(e1)
   s[c]=e1
	c:RegisterFlagEffect(1082946,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,EFFECT_FLAG_CLIENT_HINT,5,0,nil)
	c:RegisterFlagEffect(4879171,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,EFFECT_FLAG_CLIENT_HINT,5,0,nil)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp) 
  ...
  if  Duel.Destroy(sg,REASON_EFFECT) then
	local dg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.spfilter),tp,0,LOCATION_GRAVE,nil,e,tp)
