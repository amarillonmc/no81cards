...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,id)==0 and Duel.GetCustomActivityCount(id,tp,ACTIVITY_CHAIN)==0 and
	Duel.IsExistingMatchingCard(s.thcfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,nil) end
	...
	local g=Duel.SelectMatchingCard(tp,s.thcfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,e:GetHandler(),tp)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local chkf=tp
	local mg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.filter1),tp,LOCATION_REMOVED,0,nil,e)
