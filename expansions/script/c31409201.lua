...
function cm.actfilter(c)
	return c:IsSetCard(0x5310) and c:IsAbleToHand() and not c:IsCode(m)
end
function cm.actop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,m)==0 and ffcount>=6 and Duel.IsExistingMatchingCard(cm.actfilter,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then
...
function cm.serfilter(c)
	return c:IsCode(31409208) and c:IsAbleToHand() and (c:IsPosition(POS_FACEUP) or not c:IsLocation(LOCATION_REMOVED))
end
