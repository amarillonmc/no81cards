...
function c9980707.filter(c)
	return c:IsSetCard(0x3bc1) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c9980707.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c9980707.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c9980707.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(c9980707.filter,tp,LOCATION_DECK,0,1,1,nil)
