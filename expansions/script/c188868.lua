...
function cm.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		local spg=Duel.GetMatchingGroup(function(c)return c:IsSetCard(0xcac) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)end,tp,LOCATION_DECK,0,nil,e,tp)
		if (lv%2==0 and Duel.SelectYesNo(tp,aux.Stringid(m,1))) or (lv%2~=0 and #spg>0 and Duel.SelectYesNo(tp,aux.Stringid(m,2))) then
			Duel.BreakEffect()
			if lv%2==0 then
				local e1=Effect.CreateEffect(c)
