...
function cm.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_TOGRAVE+CATEGORY_SEARCH+CATEGORY_TOHAND+CATEGORY_DAMAGE)
...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Damage(1-tp,1,REASON_EFFECT,true)
	Duel.RegisterFlagEffect(tp,m,0,0,1)
	local num=Duel.GetFlagEffect(tp,m)
	if num%33==0 and Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
	...
	if num&100==0 then Duel.RDComplete() end
