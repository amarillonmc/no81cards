...
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsPlayerCanSpecialSummonCount(tp,2)
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,15005002,0xaf41,TYPES_TOKEN_MONSTER,0,0,3,RACE_THUNDER,ATTRIBUTE_LIGHT)
	end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		if Duel.Destroy(g,REASON_EFFECT)~=0 and c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
			if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,15005002,0xaf41,TYPES_TOKEN_MONSTER,0,0,3,RACE_THUNDER,ATTRIBUTE_LIGHT) then
				Duel.BreakEffect()
				local token=Duel.CreateToken(tp,15005002)
...
function cm.thfilter(c)
	return c:IsSetCard(0xaf41) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and not c:IsCode(15005001)
end
