function c117837139.initial_effect(c)
	c:EnableReviveLimit()
	c:SetUniqueOnField(1,1,117837139)
...
function c117837139.sumcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c117837139.sumop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or Duel.GetLocationCountFromEx(1-tp)<=0 then return end
...
function c117837139.filter(c)
	return not c:IsAbleToChangeControler() and c:GetSequence()<5
end
