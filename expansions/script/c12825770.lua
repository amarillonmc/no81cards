...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_MZONE,0,1,nil,e,tp) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
...
function s.thop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)<=0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) and Duel.IsPlayerCanDraw(tp,1) then
		Duel.BreakEffect()
		Duel.ShuffleDeck(tp)
