...
function c98941049.initial_effect(c)
	--c:EnableReviveLimit()
	aux.AddCodeList(c,98941049)
	--spsummon condition
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCode(EFFECT_SPSUMMON_CONDITION)
	e1:SetValue(c98941049.splimit)
	c:RegisterEffect(e1)
	--SpecialSummon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(98941049,1))
	e2:SetCategory(CATEGORY_RELEASE+CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_HAND)
	e2:SetCondition(c98941049.spcon1)
	e2:SetTarget(c98941049.sptg)
	e2:SetOperation(c98941049.spop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e3:SetCondition(c98941049.spcon2)
	c:RegisterEffect(e3)
	--tohand
	...
	e4:SetTarget(c98941049.thtg)
	e4:SetOperation(c98941049.thop)
	c:RegisterEffect(e4)
	--atk up
	...
	e5:SetCountLimit(1,98941049)
	e5:SetCondition(c98941049.atkcon)
	e5:SetTarget(c98941049.atktg)
	e5:SetOperation(c98941049.atkop)
	c:RegisterEffect(e5)
end
function c98941049.splimit(e,se,sp,st)
	return se:GetHandler():IsSetCard(0xb4)
end
function c98941049.hspfilter(c,tp)
	return c:IsReleasable() and c:IsControler(tp)
end
function c98941049.spcon1(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0
end
function c98941049.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if c==nil then return not e:GetHandler():IsStatus(STATUS_CHAINING) end
	local tp=c:GetControler()
	local ct=0
	if Duel.CheckReleaseGroup(tp,c98941049.hspfilter,1,nil,tp) then ct=ct-1 end
...
function c98941049.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=nil
	if Duel.GetTurnPlayer()==tp and ft>0 and Duel.SelectYesNo(tp,aux.Stringid(98941049,2)) then
...
function c98941049.spfilter(c,tp,re)
	return c:IsControler(tp) and c:IsType(TYPE_RITUAL) and not re:GetHandler():IsCode(98941049) and c:IsFaceup()
end
function c98941049.spcon2(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c98941049.spfilter,1,nil,tp,re)
end
function c98941049.filter1(c)
	return c:IsFaceup() and c:IsSetCard(0xaf) and c:IsAbleToHand()
end
function c98941049.filter2(c)
	return c:IsCode(table.unpack(cm.list)) and c:IsAbleToHand()
end
function c98941049.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=Duel.GetMatchingGroup(Card.IsAbleToGrave,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,nil)
	local b1=(#g>0)
	local b2=Duel.IsExistingMatchingCard(c98941049.filter2,tp,LOCATION_DECK,0,1,nil)
	if chk==0 then return b1 or b2 end
	local op=0
	if b1 and b2 then op=Duel.SelectOption(tp,aux.Stringid(98941049,0),aux.Stringid(98941049,1))
	elseif b1 then op=Duel.SelectOption(tp,aux.Stringid(98941049,0))
	else op=Duel.SelectOption(tp,aux.Stringid(98941049,1))+1 end
	e:SetLabel(op)
	if op==0 then
...
function c98941049.thop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if e:GetLabel()==0 then
		...
		local sg=Duel.SelectMatchingCard(tp,c98941049.filter2,tp,LOCATION_DECK,0,1,1,nil)
...
function c98941049.atklimitcon(e)
	return e:GetLabel()~=0
end
function c98941049.atklimittg(e,c)
	return c:GetFieldID()~=e:GetLabel()
end
function c98941049.checkop(e,tp,eg,ep,ev,re,r,rp)
	local fid=eg:GetFirst():GetFieldID()
	e:GetLabelObject():SetLabel(fid)
end
function c98941049.atkcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()==PHASE_MAIN1
end
function c98941049.fcfilter(c)
	return c:IsSetCard(0xb4) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and c:IsFaceup()
end
function c98941049.ffilter(c)
	return c:IsSetCard(0xb4) and c:IsAbleToHand()
end
function c98941049.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c98941049.fcfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(c98941049.ffilter,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil) end
	local dg=Duel.GetMatchingGroup(c98941049.ffilter,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,nil)
	local ct=math.min(3,dg:GetCount())
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local rg=Duel.SelectMatchingCard(tp,c98941049.fcfilter,tp,LOCATION_MZONE,0,1,ct,nil)
	if Duel.Remove(rg,0,REASON_COST+REASON_TEMPORARY)~=0 then
			...
			e1:SetCountLimit(1)
			e1:SetOperation(c98941049.retop)
...
function c98941049.retop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c98941049.atkop(e,tp,eg,ep,ev,re,r,rp)
	local dg=Duel.GetMatchingGroup(c98941049.ffilter,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,nil)
	local ct=e:GetLabel()
	if dg:GetClassCount(Card.GetCode)<ct then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=dg:SelectSubGroup(tp,c98941049.gcheck,false,1,ct)
