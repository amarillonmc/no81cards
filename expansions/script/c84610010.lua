...
function c84610010.initial_effect(c)
    ...
    e2:SetCondition(c84610010.condition)
    e2:SetTarget(c84610010.target)
    e2:SetOperation(c84610010.operation)
    c:RegisterEffect(e2)
    --indes
    local e1=Effect.CreateEffect(c)
    ...
    e1:SetRange(LOCATION_MZONE)
    e1:SetCondition(c84610010.damcon)
    e1:SetOperation(c84610010.damop)
    c:RegisterEffect(e1)
    local e2=e1:Clone()
...
function c84610010.spfilter(c,e,tp)
    return c:IsSetCard(0x71) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LINK,tp,false,false) 
end
...
function c84610010.condition(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsSummonType(SUMMON_TYPE_LINK)
end
function c84610010.filter(c)
    return c:IsSetCard(0x71) and c:IsAbleToHand()
end
function c84610010.target(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c84610010.filter,tp,LOCATION_DECK,0,1,nil) end
    Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c84610010.operation(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local g=Duel.SelectMatchingCard(tp,c84610010.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c84610010.damcon(e,tp,eg,ep,ev,re,r,rp)
    return eg:IsExists(c84610010.cfilter,1,nil,e:GetHandler())
end
function c84610010.damop(e,tp,eg,ep,ev,re,r,rp)
