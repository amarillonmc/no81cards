...
function s.rmcheck(g,tp)
	return g:Filter(Card.IsType,nil,TYPE_CONTINUOUS):GetCount()<=Duel.GetLocationCount(tp,LOCATION_SZONE) and g:Filter(Card.IsType,nil,TYPE_FIELD):GetCount()<=1
end
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL and a
end
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	local a,d=Duel.GetBattleMonster(p)
	if not a then return false end
	local e1=Effect.CreateEffect(c)
	local e2=Effect.CreateEffect(c)
	if a:GetFlagEffect(id+1)==0 then
		--no damage
		...
		a:RegisterFlagEffect(id+1,RESET_EVENT+RESETS_STANDARD,0,0)	
	end
	if a:GetFlagEffect(id+2)~=0 then
		a:ResetFlagEffect(id+2)
	end
	local num=math.random(4)
	local num2,num3=0,0
	local b1=true
	if num==1 then 
		if Duel.SelectEffectYesNo(p,e:GetHandler(),aux.Stringid(id,4)) then b1=false end
	elseif num==2 then
		if not Duel.SelectEffectYesNo(p,e:GetHandler(),aux.Stringid(id,5)) then b1=false end
	elseif num==3 then
		num2=math.random(2)
		num3=aux.SelectFromOptions(p,
			{true,aux.Stringid(id,num2-1),1},
			{true,aux.Stringid(id,2-num2),2})
		if num3-num2==0 then b1=false end
	else
		num2=math.random(3)
		num3=aux.SelectFromOptions(p,
