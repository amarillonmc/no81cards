...
function s.tgfilter(c)
	return c:IsLevel(10) and c:IsRace(RACE_BEAST) and c:IsAbleToGrave() and c:IsFaceupEx()
end
function s.mergefilter(c)
	return c:IsSetCard(0x191) and c:IsAbleToHand() and c:IsLevel(10) and c:IsRace(RACE_BEAST)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Group.CreateGroup()
	if Duel.IsExistingMatchingCard(s.tgfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) then
		g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_DECK,0,1,1,nil)
	else
		g=Duel.SelectMatchingCard(tp,s.mergefilter,tp,LOCATION_DECK,0,1,1,nil)
	end
	...
	if Duel.SendtoGrave(g,REASON_EFFECT)~=0 and g:FilterCount(Card.IsAttribute,nil,ATTRIBUTE_LIGHT) then
