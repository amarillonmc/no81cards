...
function c11533704.ctfil(c,e,tp) 
	return c:IsSetCard(0xb4) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c11533704.thfil,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,c) 
end 
function c11533704.thfil(c) 
	return c:IsAbleToHand() and c:IsSetCard(0xb4) 
end 
function c11533704.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c11533704.ctfil,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) and Duel.IsExistingMatchingCard(function(c) return c:IsSetCard(0xb4) and c:IsType(TYPE_MONSTER) and c:IsReleasable() end,tp,LOCATION_HAND,0,1,nil) end 
...
function c11533704.thop(e,tp,eg,ep,ev,re,r,rp) 
	local c=e:GetHandler() 
	local g=Duel.GetMatchingGroup(c11533704.thfil,tp,LOCATION_DECK+LOCATION_GRAVE,0,nil) 
	if g:GetCount()>0 then  
		local sg=g:Select(tp,1,1,nil) 
		Duel.SendtoHand(sg,tp,REASON_EFFECT) 
		Duel.ConfirmCards(1-tp,sg) 
		if Duel.IsExistingMatchingCard(function(c) return c:IsSetCard(0xb4) and c:IsType(TYPE_MONSTER) and c:IsReleasable() end,tp,LOCATION_HAND,0,1,nil) then 
			Duel.BreakEffect() 
			local rg=Duel.SelectMatchingCard(tp,function(c) return c:IsSetCard(0xb4) and c:IsType(TYPE_MONSTER) and c:IsReleasable() end,tp,LOCATION_HAND,0,1,1,nil) 
