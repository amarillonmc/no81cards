...
function cm.initial_effect(c)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(cm.tgfilter,tp,LOCATION_ONFIELD,0,1,e:GetHandler(),e,tp) end
	...
	local tc=Duel.SelectMatchingCard(tp,cm.tgfilter,tp,LOCATION_ONFIELD,0,1,1,c,e,tp):GetFirst()
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc):GetFirst()
		if sc then
			Duel.SpecialSummon(sc,0,tp,tp,false,false,POS_FACEUP,0x60)
			local fg=Duel.GetMatchingGroup(cm.atkfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	...
	if not tc then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
		tc=Duel.SelectMatchingCard(tp,cm.tgfilter2,tp,LOCATION_ONFIELD,0,1,1,c,tp):GetFirst()
		if tc then
