	...
	if not PNFL_MOVE_DELAY_CHECK then
		...
		Duel.Equip=function(p,c,...)
			...
			if ns and c:IsHasEffect(EFFECT_EQUIP_LIMIT) then
				cm.desop2(e1,0,Group.FromCards(c),0,0,e1,0,0)
			end
			return res
		end
		...
		function Duel.GetControl(v,p,...)
			...
			if ns then c:RegisterFlagEffect(11451848,RESET_CHAIN,0,1) c:RegisterFlagEffect(11451848,RESET_CHAIN,0,1) end
			local res=_GetControl(v,p,...)
			if ns then
				c:ResetFlagEffect(11451848)
				cm.desop2(e1,0,g,0,0,e1,0,0)
			end
			return res
		end
		local e1=Effect.CreateEffect(c)
