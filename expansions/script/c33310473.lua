...
function s.initial_effect(c)
	...
	--●2是取对象效果，因此保留EFFECT_FLAG_CARD_TARGET（只在选择●2时才会进行目标选择）。
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(id,9))
	e4:SetCategory(CATEGORY_SSET+CATEGORY_TOHAND+CATEGORY_TODECK)
...
--●以场上1张卡为对象，回到持有者手卡，那之后可取除1个灵感指示物让1张卡回到卡组最上面
...
function s.bncop(e,tp,eg,ep,ev,re,r,rp)
	...
	--那之后，可以把自己场上1个灵感指示物取除，再选双方墓地·除外的1张卡回到卡组最上面
	if not Duel.IsCanRemoveCounter(tp,1,0,COUNTER_INSPIRE,1,REASON_EFFECT) then return end
	if not Duel.IsExistingMatchingCard(s.topfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,1,nil) then return end
	...
	local g=Duel.SelectMatchingCard(tp,s.topfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,LOCATION_GRAVE+LOCATION_REMOVED,1,1,nil)
...
function s.sumtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return s.bncfilter(chkc) end
	...
	local b2=Duel.GetFlagEffect(tp,id+20)==0
		and Duel.IsExistingTarget(s.bncfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
	...
	if opt==1 then
		...
		local g=Duel.SelectTarget(tp,s.bncfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
