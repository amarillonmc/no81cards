...
function cm.initial_effect(c)
	...
	e4:SetCategory(CATEGORY_TOGRAVE+CATEGORY_CONTROL)
	...
	e5:SetCondition(function(e,tp) return e:GetHandler():IsSummonType(SUMMON_TYPE_PENDULUM) end)
...
function cm.condition(e,tp,eg,ep,ev,re,r,rp)
	return cm.ArcKnightPCardCheck(e) and rp==1-tp and re:IsActiveType(TYPE_SPELL+TYPE_TRAP)
end
function cm.check(e,tp,exc)
	return cm.PendConditionArcKnight(e,exc,nil,cm.ArcKnightPCardFilter,true)
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
	local exc = Duel.GetFieldCard(tp,LOCATION_PZONE,0)
	return exc and cm.check(e,tp,exc)
	end
end
