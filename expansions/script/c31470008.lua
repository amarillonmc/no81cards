...
function cm.spsfilter(c)
	return c:IsSetCard(0x3312) and not c:IsRace(RACE_DRAGON)
end
function cm.spstg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.spsfilter,tp,LOCATION_MZONE,0,1,nil) end
end
function cm.spsop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(cm.spsfilter,tp,LOCATION_MZONE,0,nil)
	if g:GetCount()>0 then
		local e1=Effect.CreateEffect(c)
...
function cm.imuop(e,tp,eg,ep,ev,re,r,rp)
	...
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(31470008,1))
	...
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(31470008,0))
