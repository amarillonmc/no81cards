...
function cm.splimit(e,c,tp)
	...
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429581,0,TYPES_TOKEN_MONSTER,0,4000,5,RACE_AQUA,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,2)
...
function cm.sumlimit(e,c,tp)
	...
	if not zone_check  
	or not Duel.IsPlayerCanSpecialSummonMonster(tp,7429581,0,TYPES_TOKEN_MONSTER,0,4000,5,RACE_AQUA,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) or not Duel.IsPlayerCanSpecialSummonCount(tp,1) then return false end
	return true
end
function cm.tokenop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_TOFIELD,(1<<(e:GetHandler():GetSequence()+1)))>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,7429581,0,TYPES_TOKEN_MONSTER,0,4000,5,RACE_AQUA,ATTRIBUTE_WATER,POS_FACEUP_ATTACK) then
		for i=0,0 do
			local token=Duel.CreateToken(tp,7429581)
...
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_MZONE,1,nil)  end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP_ATTACK)~=0 and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_MZONE,1,nil) then
		local g=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,0,LOCATION_MZONE,nil)
		if Duel.Remove(g,POS_FACEDOWN,REASON_EFFECT+REASON_TEMPORARY)~=0 then
			...
			while oc do
				oc:RegisterFlagEffect(m+1,RESET_EVENT+RESETS_STANDARD,0,1,fid)
				oc=og:GetNext()
			end
			c:RegisterFlagEffect(m+1,RESET_EVENT+RESET_TURN_SET+RESET_OVERLAY+RESET_TOFIELD,0,1,fid)
			og:KeepAlive()
			local e1=Effect.CreateEffect(c)
...
function cm.retfilter(c,fid)
	return c:GetFlagEffectLabel(m+1)==fid
end
function cm.retcfilter(c)
	return c:IsCode(7429581) and c:IsFaceup()
end
function cm.retcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetOwner()
	if not cm.retfilter(c,e:GetLabel()) then 
		e:Reset() 
		return false 
	end
	return eg:IsContains(c) and Duel.IsExistingMatchingCard(cm.retcfilter,c:GetControler(),LOCATION_MZONE,0,1,nil)
end
function cm.retop(e,tp,eg,ep,ev,re,r,rp)
	local fid=e:GetLabel()
	local rg=e:GetLabelObject():Filter(cm.retfilter,nil,fid)
	if #rg<=0 then return end
	Duel.Hint(HINT_CARD,0,m)
	for p in aux.TurnPlayers() do
		local tg=rg:Filter(Card.IsPreviousControler,nil,p)
		local ft=Duel.GetLocationCount(p,LOCATION_MZONE)
		if ft>0 and #tg>ft then
			Duel.Hint(HINT_SELECTMSG,p,HINTMSG_TOFIELD)
			local sg=tg:Select(p,ft,ft,nil)
...
function cm.btfilter(c)
	return c:IsFaceup() and c:IsCode(7429581)
end
