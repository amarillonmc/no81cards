...
function c75030024.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c75030024.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c75030024.exmzfilter(c,seq)
	return c:GetSequence()==seq
end
function c75030024.seqfilter(c,seq,tp)
	...
	if cloc==LOCATION_MZONE and seq>=5 and loc==LOCATION_MZONE
		and Duel.IsExistingMatchingCard(c75030024.exmzfilter,tp,0,LOCATION_MZONE,1,nil,seq) then
		return seq==5 and cseq==1 or seq==6 and cseq==3
	end
	return cseq==seq or seq<5 and cseq<5 and cloc==loc and math.abs(cseq-seq)==1
end
function c75030024.seqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c75030024.desfilter,tp,LOCATION_ONFIELD,0,1,nil) end
	local filter=0
	for i=0,15 do
		if not Duel.IsExistingMatchingCard(c75030024.seqfilter,tp,LOCATION_ONFIELD,0,1,nil,i,tp) then
			filter=filter|1<<(i)
		end
	end
