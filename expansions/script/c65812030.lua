...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DESTROY+CATEGORY_TOHAND+CATEGORY_SEARCH+CATEGORY_GRAVE_ACTION)
...
function s.tftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(s.tffilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,c,tp) and Duel.GetFlagEffect(tp,id)==0 end
...
function s.thfilter(c,code,e,tp)
	return  (c:IsLocation(LOCATION_DECK) or (c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) and Duel.IsExistingMatchingCard(s.shfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,e,tp))) and c:IsCode(code) and c:IsAbleToHand() and c:IsFaceupEx()
end
function s.tfop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not Duel.IsExistingMatchingCard(s.tffilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,c,tp) then return end
	local tc=Duel.SelectMatchingCard(tp,s.tffilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,1,c,tp):GetFirst()
	if tc and Duel.Destroy(tc,REASON_EFFECT)~=0 then
		if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
			local code=tc:GetCode()
			local sg=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,nil,code,e,tp)
