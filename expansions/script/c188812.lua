...
function c188812.cfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_UNION) and c:IsControler(tp)
end
...
function c188812.unfil1(c,tp)
	return c:IsType(TYPE_UNION) and Duel.IsExistingMatchingCard(c188812.unfil2,tp,LOCATION_GRAVE,0,1,nil,tp,c) and Duel.IsExistingMatchingCard(c188812.tgfil,tp,LOCATION_DECK,0,1,nil,tp,c)
end
...
function c188812.xeqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,c188812.unfil2,tp,LOCATION_GRAVE,0,1,1,nil,tp,tc)
		local ec=g:GetFirst()
