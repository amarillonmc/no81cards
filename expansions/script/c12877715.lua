...
function s.thfilter(c,code)
	return not c:IsCode(code) and c:IsSetCard(0x5a73) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:IsFaceup()
end
function s.e1tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,c:GetCode()) and c:GetFlagEffect(id)==0 end
...
function s.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thfilter),tp,LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil,c:GetCode())
	if #sg>0 and Duel.SendtoHand(sg,tp,REASON_EFFECT)>0 and sg:GetFirst():IsLocation(LOCATION_HAND) then
		Duel.ConfirmCards(1-tp,sg)
		if not Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_ONFIELD,0,1,nil,sg:GetFirst():GetCode()) 
			and sg:GetFirst():IsCanBeSpecialSummoned(e,0,tp,false,false)
