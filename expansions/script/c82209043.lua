...
function cm.thfilter(c)  
	return cm.isKieju(c:GetCode()) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and c:IsLevel(8) 
end  
...
function cm.negfilter(c,e,tp)  
	return c:IsType(TYPE_MONSTER) and cm.isKieju(c:GetCode()) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end  
function cm.negtg(e,tp,eg,ep,ev,re,r,rp,chk)  
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0  
		and Duel.IsExistingMatchingCard(cm.negfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end  
...
function cm.negop(e,tp,eg,ep,ev,re,r,rp)  
	...
	local g=Duel.SelectMatchingCard(tp,cm.negfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)  
	local tc=g:GetFirst()  
	if tc and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)~=0 and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then  
