...
function cm.eqcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return rc:IsSetCard(0x97e) and rc:IsType(TYPE_EQUIP) and re:GetType()&EFFECT_TYPE_ACTIVATE==0
end
function cm.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local tc=re:GetHandler():GetPreviousEquipTarget()
	if chkc then return chkc==tc end
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) and tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() and tc:IsCanBeEffectTarget(e) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 end
...
function cm.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetActivateEffect() and c:GetActivateEffect():IsActivatable(tp,true,true) end
