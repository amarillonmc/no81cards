...
function cm.relfilter(c,tp)
	return (c:IsType(TYPE_TOKEN) or (cm.Hastur(c) and c:IsLevelBelow(5))) and c:IsReleasable() and Duel.GetMZoneCount(tp,c)>0
end
...
function cm.spcon(e,c)
	...
	local b1=Duel.IsExistingMatchingCard(cm.relfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp)
	local b2=cm.get_available_field(tp,14002341) and Duel.IsExistingMatchingCard(cm.deck_cost_filter,tp,LOCATION_DECK,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	return b1 or b2
end
function cm.sptg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local b1=Duel.IsExistingMatchingCard(cm.relfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp)
	local fc=cm.get_available_field(tp,14002341)
	local b2=fc and Duel.IsExistingMatchingCard(cm.deck_cost_filter,tp,LOCATION_DECK,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	local op=0
	if b1 and b2 then
	...
	if op==0 then
		...
		local g=Duel.SelectMatchingCard(tp,cm.relfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp)
		g:KeepAlive()
		e:SetLabelObject(g)
		e:SetLabel(0)
	else
		Duel.Hint(HINT_CARD,0,14002341)
...
function cm.xyzfilter(c,e,tp,mc)
	return (cm.Hastur(c) or cm.Urara(c)) and c:IsType(TYPE_XYZ)
...
function cm.xyzop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFaceup() and c:IsRelateToEffect(e) and aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_XMATERIAL) and not c:IsImmuneToEffect(e) then
		...
		if tc then
			...
			if Duel.SpecialSummon(tc,SUMMON_TYPE_XYZ,tp,tp,true,false,POS_FACEUP)>0 then
				tc:CompleteProcedure()
			end
		end
	end
	if e:GetHandler():IsSummonType(SUMMON_VALUE_SELF) then
