function c98731001.initial_effect(c)
	...
	e6:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL+EFFECT_FLAG_IGNORE_IMMUNE)
	e6:SetCondition(c98731001.chcon)
...
function c98731001.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetActivityCount(tp,ACTIVITY_BATTLE_PHASE)==0 and not Duel.CheckPhaseActivity() and not e:GetHandler():IsLocation(LOCATION_PZONE) and (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1)) and Duel.GetTurnPlayer()==tp and Duel.GetFlagEffect(tp,98731001)==0
end
function c98731001.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.RegisterFlagEffect(tp,98731001,RESET_PHASE+PHASE_MAIN1,0,1)
	local c=e:GetHandler()
	if not e:GetHandler():IsLocation(LOCATION_PZONE) and (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1)) and Duel.SelectYesNo(tp,aux.Stringid(98731001,0)) then
		Duel.RegisterFlagEffect(tp,98731001,RESET_CHAIN,0,1)
		Duel.MoveToField(c,tp,tp,LOCATION_SZONE,POS_FACEUP,true)
	end
end
function c98731001.chcon(e,tp,eg,ep,ev,re,r,rp)
	return not re:GetHandler():IsCode(98731001)
end
