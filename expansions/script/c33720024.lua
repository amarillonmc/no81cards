--Sepialife - Dualism in Singularity
...
function s.con2(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPublic() and eg:IsExists(s.egfilter,1,nil,tp)
end
function s.op2(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_HAND,1,nil) and Duel.IsPlayerCanDraw(1-tp,1) and Duel.SelectYesNo(1-tp,aux.Stringid(id,2)) then
		Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_TODECK)
		local g=Duel.SelectMatchingCard(1-tp,aux.TRUE,tp,0,LOCATION_HAND,1,1,nil)
		if #g>0 and Duel.SendtoDeck(g,tp,0,REASON_EFFECT)>0 and g:GetFirst():IsLocation(LOCATION_DECK) and g:GetFirst():IsControler(tp) then
