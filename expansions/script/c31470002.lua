...
function cm.drawop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(31470002,1)) then
		Duel.Draw(tp,1,REASON_EFFECT)
	end
	local c=e:GetHandler()
	local rc=c:GetReasonCard()
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(31470002,0))
...
function cm.drawop2(e,tp,eg,ep,ev,re,r,rp)
	if r&REASON_SYNCHRO~=0 and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(31470002,1)) then
		Duel.Draw(tp,1,REASON_EFFECT)
	end
	e:Reset()
end
