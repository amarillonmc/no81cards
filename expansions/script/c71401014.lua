...
function c71401014.chainlm(e,rp,tp)
	return tp==rp or e:GetHandler():GetOriginalType()&TYPE_MONSTER==0
end
function c71401014.filter2(c,e,tp)
	return c:GetOriginalType()&TYPE_MONSTER~=0 and c:IsAbleToRemove(tp,POS_FACEDOWN) and not c:IsImmuneToEffect(e)
		and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,0,LOCATION_ONFIELD+LOCATION_GRAVE,LOCATION_ONFIELD+LOCATION_GRAVE,1,c,tp,POS_FACEDOWN)
end
function c71401014.op2(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and not c:IsImmuneToEffect(e) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
		...
		if Duel.MoveToField(c,tp,tp,LOCATION_SZONE,POS_FACEUP,true) then
			...
			c:RegisterEffect(e2)
			local mg=Duel.GetMatchingGroup(c71401014.filter2,tp,LOCATION_ONFIELD,0,c,e,tp)
			if ct>0 and mg:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(71401014,0)) then
				Duel.BreakEffect()
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
				local rg1=mg:Select(tp,1,1,c)
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
				local rg2=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,0,LOCATION_ONFIELD+LOCATION_GRAVE,LOCATION_ONFIELD+LOCATION_GRAVE,1,ct,rg1,tp,POS_FACEDOWN)
				rg2:Merge(rg1)
...
function c71401014.filter3(c,tp,ac)
	return c:GetOriginalType()&TYPE_MONSTER~=0 and c:IsAbleToRemove(tp,POS_FACEDOWN)
...
function c71401014.tg3(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and c71401014.filter3(chkc,tp,e:GetHandler()) end
	if chk==0 then return Duel.IsExistingTarget(c71401014.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c71401014.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp,e:GetHandler())
...
function c71401014.op3(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local rg=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,Group.FromCards(tc,e:GetHandler()),tp,POS_FACEDOWN)
