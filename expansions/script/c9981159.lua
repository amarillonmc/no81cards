...
function c9981159.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(9981159,0))
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCountLimit(1,9981159)
	e1:SetTarget(c9981159.target)
	e1:SetOperation(c9981159.operation)
	c:RegisterEffect(e1)
	--special summon
	local e1=Effect.CreateEffect(c)
...
function c9981159.scalefilter(c)
	return c:IsFaceup() and c:IsSetCard(0x6bc3) and c:GetLeftScale()~=11
end
function c9981159.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_PZONE) and chkc:IsControler(tp) and c9981159.scalefilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c9981159.scalefilter,tp,LOCATION_PZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c9981159.scalefilter,tp,LOCATION_PZONE,0,1,1,nil)
end
function c9981159.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		e3:SetTarget(c9981159.splimit)
		e3:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e3,tp)
	end
end
function c9981159.splimit(e,c)
