...
function cm.op(e,tp,eg,ep,ev,re,r,rp)
	...
	local cs,codes=false,{}
	for _,te in pairs(eset) do
		local code=te:GetHandler():GetOriginalCode()
		if Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK,0,1,nil,code) then
			cs=true
			codes[code]=true
		end
	end
	...
	for i,te in pairs(eset) do
		local code=te:GetHandler():GetOriginalCode()
		--if not cs or codes[code] then
			tab[#tab+1]=te:GetDescription()
			tab2[#tab]=i
		--end
	end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(m,1))
	local op=tab2[1+Duel.SelectOption(tp,table.unpack(tab))]
	local de=eset[op]
	...
	g=g:CancelableSelect(tp,1,1,nil)
	if g and #g>0 and Duel.SSet(tp,g)>0 then --and g:GetFirst():IsCode(code) then
