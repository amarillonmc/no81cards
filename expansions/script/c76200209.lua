...
function s.thfilter(c,tp,check)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x725a) and c:IsAbleToHand()
...
function s.thfilter2(c,code,lv)
	return c:IsRace(RACE_DRAGON) and not c:IsCode(code) and c:GetOriginalLevel()==lv and c:IsAbleToHand()
end
...
function s.cfilter(c)
	return c:IsFaceupEx() and c:IsRace(RACE_DRAGON) and c:IsAbleToRemoveAsCost()
end
