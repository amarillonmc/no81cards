...
function cm.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (not c:IsLocation(LOCATION_GRAVE) or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,m,0x353b,TYPES_NORMAL_TRAP_MONSTER,900,2100,4,RACE_ROCK,ATTRIBUTE_WATER))) and Duel.GetFlagEffect(tp,m)==0 and not c:IsLocation(LOCATION_ONFIELD) end
