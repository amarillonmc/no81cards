...
function c29065601.spfil(c,e,tp)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsRace(RACE_MACHINE) and c:IsLevelBelow(4) and not c:IsCode(29065601)
end
...
function c29065601.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c29065601.spfil,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil,e,tp)
...
function c29065601.repfilter(c,tp)
	return c:IsControler(tp) and c:IsLocation(LOCATION_ONFIELD)
		and c:IsReason(REASON_BATTLE+REASON_EFFECT) and not c:IsReason(REASON_REPLACE) and c:IsSetCard(0x7ad)
end
