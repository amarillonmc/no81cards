...
function c40009031.initial_effect(c)
	...
	e1:SetTarget(c40009031.seqtg)
	e1:SetOperation(c40009031.seqop)
	c:RegisterEffect(e1)
end
...
function c40009031.seqfilter(c)
	local tp=c:GetControler()
	return c:IsFaceup() and c:GetSequence()<5 and Duel.GetLocationCount(tp,LOCATION_MZONE,tp,LOCATION_REASON_CONTROL)>0
end
function c40009031.seqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c40009031.seqfilter(chkc) and chkc~=c end
	if chk==0 then return Duel.IsExistingTarget(c40009031.seqfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c) end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(40009031,1))
	Duel.SelectTarget(tp,c40009031.seqfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,c)
end
function c40009031.seqop(e,tp,eg,ep,ev,re,r,rp)
