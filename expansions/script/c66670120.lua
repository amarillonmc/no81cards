...
function s.initial_effect(c)
	...
	-- 这张卡在墓地存在的状态，自己场上的表侧表示的魔法师族以外的「苍途」怪兽因对方的效果从场上离开的场合或者被战斗破坏的场合才能发动，这张卡加入手卡
...
function s.cfilter(c,tp,rp)
	return c:IsPreviousPosition(POS_FACEUP) and c:IsPreviousControler(tp) and bit.band(c:GetPreviousTypeOnField(),TYPE_MONSTER)~=0
		and c:IsPreviousSetCard(0x666d) and not c:IsRace(RACE_SPELLCASTER) and (c:IsReason(REASON_BATTLE) or (rp==1-tp and c:IsReason(REASON_EFFECT)))
end
