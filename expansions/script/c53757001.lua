...
function cm.initial_effect(c)
	...
	e6_1:SetType(EFFECT_TYPE_FIELD)
	e6_1:SetCode(EFFECT_SSET_COST)
	e6_1:SetRange(LOCATION_FZONE)
	e6_1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_SET_AVAILABLE)
	e6_1:SetTargetRange(LOCATION_HAND,0)
	e6_1:SetCondition(function(e)
		return e:GetHandler():GetSequence()>4
	end)
	e6_1:SetTarget(cm.actarget3)
	e6_1:SetOperation(cm.repoperation)
	...
	if not Goron_Dimension_Check then
		...
		--[[cm[2]=Effect.IsActivatable
		Effect.IsActivatable=function(re,rp,...)
			local rc=re:GetHandler()
			local res=cm[2](re,rp,...)
			if re:IsHasType(EFFECT_TYPE_ACTIVATE) and rc:IsHasEffect(53757000) and rc:GetSequence()<5 then
				res=true
				if Duel.GetFlagEffect(rp,m)>0 then res=false end
				if rc:IsHasEffect(EFFECT_CANNOT_TRIGGER) then res=false end
				local pe={Duel.IsPlayerAffectedByEffect(rp,EFFECT_CANNOT_ACTIVATE)}
				for _,v in pairs(pe) do
					local val=v:GetValue()
					if not val or aux.GetValueType(val)=="number" or val(v,re,rp) then res=false end
				end
				local ae={Duel.IsPlayerAffectedByEffect(rp,EFFECT_ACTIVATE_COST)}
				for _,v in pairs(ae) do
					local cost=v:GetCost()
					if cost and not cost(v,re,rp) then
						local tg=v:GetTarget()
						if not tg or tg(v,re,rp) then res=false end
					end
				end
			end
			return res
		end--]]
		--[[cm[3]=Duel.RaiseEvent
		Duel.RaiseEvent=function(tg,reason,re,...)
			if aux.GetValueType(tg)=="Card" and reason==4179255 then
				if tg:IsHasEffect(53757000) and re:GetLabelObject() then re=re:GetLabelObject() end
			end
			return cm[3](tg,reason,re,...)
		end--]]
