...
function c22022820.initial_effect(c)
	...
	e5:SetDescription(aux.Stringid(22022820,2))
	...
	e5:SetCondition(c22022820.discon1)
	e5:SetCost(c22022820.discost1)
	e5:SetTarget(c22022820.distg)
	e5:SetOperation(c22022820.disop)
	c:RegisterEffect(e5)
end
c22022820.effect_canequip_hogu=true
function c22022820.cfilter(c)
	return c:IsSetCard(0x6ff1) and c:IsType(TYPE_MONSTER) and c:IsAbleToDeckAsCost()
end
...
function c22022820.discon1(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
		and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and Duel.IsPlayerAffectedByEffect(tp,22020980)
end
function c22022820.discost1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c22022820.discfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,c22022820.discfilter,1,1,REASON_COST,nil)
