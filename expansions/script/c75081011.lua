...
function c75081011.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.ReturnToField(tc)~=0 and Duel.GetFlagEffect(tp,75081012)==0 and tc:IsFaceup() and tc:IsControler(tp) and not tc:IsImmuneToEffect(e) and aux.MustMaterialCheck(tc,tp,EFFECT_MUST_BE_XMATERIAL) and Duel.IsExistingMatchingCard(c75081011.spfilter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,tc) and Duel.SelectYesNo(tp,aux.Stringid(75081011,0)) then
