...
function cm.drawcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if de>1 and ha>0 and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
		local max
		if (de-1)>ha then
		max=ha
		else
...
function cm.sertg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK+LOCATION_EXTRA+LOCATION_REMOVED)
	Duel.Hint(HINT_OPSELECTED,1-tp,e:GetDescription())
end
