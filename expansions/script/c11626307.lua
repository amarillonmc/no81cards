...
function c11626307.accost(e,tp,eg,ep,ev,re,r,rp,chk) 
	if chk==0 then return Duel.GetCustomActivityCount(11626307,tp,ACTIVITY_SPSUMMON)==0 end  
...
function c11626307.accost2(e,tp,eg,ep,ev,re,r,rp,chk) 
	if chk==0 then return Duel.GetFlagEffect(tp,11626307)==0 and Duel.GetCustomActivityCount(11626307,tp,ACTIVITY_SPSUMMON)==0 and Duel.CheckReleaseGroupEx(tp,c11626307.costfil,1,REASON_COST,true,nil) end  
	local e2=Effect.CreateEffect(e:GetHandler())
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetProperty(EFFECT_FLAG_PLAYER_TARGET+EFFECT_FLAG_OATH)
	e2:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	...
	e2:SetTarget(c11626307.splimit)
	Duel.RegisterEffect(e2,tp)
	local g=Duel.SelectReleaseGroupEx(tp,c11626307.costfil,1,1,REASON_COST,true,nil)
