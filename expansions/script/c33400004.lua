...
function c33400004.initial_effect(c)
	...
	e3:SetCost(c33400004.cost)
	e3:SetTarget(c33400004.thtg2)
	e3:SetOperation(c33400004.thop2)
	c:RegisterEffect(e3)
end
function c33400004.addct(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,2,0,0x34f)
end
function c33400004.addc(e,tp,eg,ep,ev,re,r,rp)
...
function c33400004.thfilter2(c)
	return c:IsSetCard(0x3340) and c:IsType(TYPE_SPELL) and c:IsType(TYPE_QUICKPLAY) and c:IsAbleToHand()
end
function c33400004.thtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c33400004.thfilter2,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c33400004.thop2(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c33400004.thfilter2,tp,LOCATION_DECK,0,1,1,nil)
	if #g>0 then
