...
function c95101148.initial_effect(c)
	...
	e3:SetCost(c95101148.spcost2)
	e3:SetTarget(c95101148.sptg)
	e3:SetOperation(c95101148.spop2)
	c:RegisterEffect(e3)
	--counter
...
function c95101148.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c95101148.tgfilter,tp,LOCATION_HAND,0,1,e:GetHandler())end-- and Duel.GetCustomActivityCount(95101148,tp,ACTIVITY_SPSUMMON)==0 end
...
function c95101148.thfilter(c)
	return c:IsSetCard(0xbbf) and c:IsAbleToHand()-- and c:IsType(TYPE_MONSTER)
end
...
function c95101148.tgfilter2(c)
	return c:IsSetCard(0xbbf) and c:IsFaceup() and c:IsAbleToGraveAsCost()
end
function c95101148.spcost2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c95101148.tgfilter2,tp,LOCATION_EXTRA,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c95101148.tgfilter2,tp,LOCATION_EXTRA,0,1,1,nil)
	Duel.SendtoGrave(g,REASON_COST)
end
function c95101148.spop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
	...
	e1:SetCode(EFFECT_CANNOT_SPECIAL_SUMMON)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1:SetTarget(c95101148.xlimit)
	e1:SetTargetRange(1,0)
	e1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1,tp)
end
function c95101148.xlimit(e,c,sump,sumtype,sumpos,targetp,se)
