...
function cm.desfilter(c,e,tp)
	return Duel.IsExistingMatchingCard(cm.spfilter,tp,LOCATION_EXTRA+LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,c)
end
function cm.spfilter(c,e,tp,mc)
	local b1=c:IsLocation(LOCATION_EXTRA) and c:IsFaceup() and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
	local b2=c:IsLocation(LOCATION_HAND+LOCATION_GRAVE) and Duel.GetMZoneCount(tp,mc)>0
	return (c:IsSetCard(0x341) or c:IsAttribute(ATTRIBUTE_WATER)) and (b1 or b2)
...
function cm.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and cm.desfilter(chkc,e,tp) end
...
function cm.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 then
		...
		local g=Duel.SelectMatchingCard(tp,cm.spfilter,tp,LOCATION_EXTRA+LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,e,tp,nil)
