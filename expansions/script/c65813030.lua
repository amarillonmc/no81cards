...
function s.GetLinkCount(c,lc)
	...
	if c:IsLocation(LOCATION_HAND+LOCATION_ONFIELD) and not c:IsType(TYPE_LINK) then
		return 1
	end
...
function s.GetLinkMaterials(tp,lc)
	...
	local mg2=Duel.GetMatchingGroup(s.HandFilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,nil,lc)
...
function s.desop2(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,1,nil)
	if g:GetCount()>0 then
