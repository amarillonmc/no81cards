...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local flag=5
	while flag>0 do
		if Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) and (flag==5 or Duel.SelectYesNo(tp,aux.Stringid(id,0))) then 
...
function s.filter(c)
	return (c:IsCode(33300905)or c:IsCode(33300906)) and c:IsAbleToHand()
end
