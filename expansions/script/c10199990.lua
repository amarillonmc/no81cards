...
function rsop.SelectSolve(selecthint,sp,filter,tp,loc1,loc2,minct,maxct,exceptg,solvefun,...)
	...
	if rsof.Check_Boolean(minct) then
		...
		local res=not solvefun and {g,g:GetFirst()} or {solvefun2(g,table.unpack(solvefunpar))}
		rsop.nohint=false
...
function rsop.SelectToGrave(sp,filter,tp,loc1,loc2,minct,maxct,exceptg,reason,...)
	...
	return rsop.SelectSolve("tg",sp,filter,tp,loc1,loc2,minct,maxct,exceptg,{rsop.SendtoGrave,reason},...)
end
...
function rsop.SendtoDeck(corg,p,seq,reason,nohint)
	reason= reason or REASON_EFFECT 
	local g=rsgf.Mix2(corg)
	if #g<=0 then return 0,nil end
	local hintg=g:Filter(Card.IsLocation,nil,LOCATION_ONFIELD+LOCATION_GRAVE+LOCATION_REMOVED)
...
function rsop.Destroy(corg,reason,desloc,nohint)
	reason= reason or REASON_EFFECT 
	desloc= desloc or LOCATION_GRAVE 
	local g=rsgf.Mix2(corg)
	if #g<=0 then return 0,nil end
	local hintg=g:Filter(Card.IsOnField,nil)
	if #hintg>0 and not nohint and not rsop.nohint then
		Duel.HintSelection(g)
	end
...
function rsop.Remove(corg,pos,reason,nohint)
...
function rsgf.SelectSolve(g,selecthint,sp,filter,minct,maxct,exceptg,solvefun,...)
	...
	local res=not solvefun and {tg,tg:GetFirst()} or {solvefun2(tg,table.unpack(solvefunpar))}
	rsop.nohint=false
	return table.unpack(res) 
end
