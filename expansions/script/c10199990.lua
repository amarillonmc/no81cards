...
function rsef.GetRegisterCategory(mixcategory)
	local category_string_list={"des","res","rm","th","td","tg","disd","dish","sum","sp","tk","pos","con","dis","diss","dr","se","eq","dam","rec","atk","def","ct","coin","dice","lg","lv","neg","an","fus","te","ga"}
	return rsof.Mix_Value_To_Table(mixcategory,category_string_list,rscate.catelist)
end
...
function rsef.FC_AttachEffect_Activate(cardtbl,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl,force)
	return rsef.FC_AttachEffect(cardtbl,force,0x1,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl) 
end
--Field Continues: Attach an extra effect before the base effect solving
function rsef.FC_AttachEffect_BeforeResolve(cardtbl,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl,force)
	return rsef.FC_AttachEffect(cardtbl,force,0x2,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl) 
end
--Field Continues: Attach an extra effect after the base effect solving
function rsef.FC_AttachEffect_Resolve(cardtbl,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl,force)
	return rsef.FC_AttachEffect(cardtbl,force,0x4,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl) 
end
function rsef.FC_AttachEffect(cardtbl,force,attachtime,desctbl,ctlimittbl,flag,range,attachcon,attachop,resettbl) 
	...
	local e0=rsef.I(cardtbl,nil,ctlimittbl,nil,flag,range,aux.FALSE,nil,nil,nil,resettbl)
...
function rsef.FC_AttachEffect_Solve(solveeffectlist,attachtime,e,tp,eg,ep,ev,re,r,rp)
	...
	if #attachgroup>0 and Duel.SelectYesNo(tp,aux.Stringid(m,selecthint)) then
		...
		local opt=Duel.SelectOption(tp,table.unpack(hintlist))+1
		...
		local _,index=rsof.Table_List(selectablelist,ae)
		table.remove(selectablelist,index)
...
function rsef.FC_PHASELEAVE(cardtbl,sg,times,whos,phase,leaveway,resettbl)
	...
	local e1=rsef.FC(cardtbl,EVENT_PHASE+phase,rshint.epleave,1,"ii",nil,rsef.FC_PhaseLeave_Con(cphase,turnctlist,turnp,fid,times,whos),rsef.FC_PhaseLeave_Op(leaveway,fid),resettbl)
	sg:KeepAlive()
...
function rsef.list(list_type_string,parameter1,...)
	...
	--2. { cfilter,gfilter }, { category_fun,category_string,select_hint } ,loc1 
	...
	--3. {A,B,C},{{D,E,F}} OR {A,B,C},{D,E,F} OR {{A,B,C}},{D,E,F}, to {{A,B,C},{D,E,F}}
		...
		for _,mix_parameter in pairs(mix_list) do 
			if rsof.Check_Boolean(mix_parameter[0],true) then
				for index,mix_parameter2 in pairs(mix_parameter) do
					if index~=0 then
						table.insert(target_list_total,mix_parameter2)
					end
				end	
			else
				table.insert(target_list_total,mix_parameter)
			end
		end
	end
	for _,target_list in pairs(target_list_total) do 
		target_list[0]=target_list[0] or list_type_string
	end  
...
function rstg.GetTargetAttribute(e,tp,eg,ep,ev,re,r,rp,target_list)
	...
	local category_value=type(target_list[2])=="table" and target_list[2] or {target_list[2]}
	local category_string_list0,category_fun,select_hint=table.unpack(category_value)
	local _,category_list,category_string_list=rsef.GetRegisterCategory(category_string_list0)
	--3.Locaion Self
	...
	return list_type,filter_card,filter_group,category_list,category_string_list,category_fun,select_hint,loc1,loc2,minct,maxct,except_fun
end
...
function rstg.TargetCheck(e,tp,eg,ep,ev,re,r,rp,chk,chkc,target_list_total)
	...
	local list_type,filter_card,filter_group,category_list,category_string_list,category_fun,select_hint,loc1,loc2,minct,maxct,except_fun= rstg.GetTargetAttribute(e,tp,eg,ep,ev,re,r,rp,target_list_total[1])
	--2. Get except group
	...
	if chkc then
		...
		for index=2,#target_list_total do 
			local target_list=target_list_total[index]
...
function rstg.TargetSelect(e,tp,eg,ep,ev,re,r,rp,target_list_total)
	...
	--2. Selecting 
	--for index,target_list in pairs(target_list_total) do 
	--cannot use above method because may have target_list_total[0]
	for index=1,#target_list_total do 
		local target_list=target_list_total[index]
		--2.0. Get Next target_list
		local target_list_next={}
		for index2,target_list_next_par in pairs(target_list_total) do 
			if index2>index then
				table.insert(target_list_next,target_list_next_par)
			end
		end
		--2.1. Get target list parameter
		local list_type,filter_card,filter_group,category_list,category_string_list,category_fun,select_hint,loc1,loc2,minct,maxct,except_fun = rstg.GetTargetAttribute(e,tp,eg,ep,ev,re,r,rp,target_list)
		...
		if rsof.Table_List(category_list,CATEGORY_RELEASE) then
			...
			if loc2 & 0xff-LOCATION_HAND+LOCATION_MZONE ~=0 then
		...
		elseif type(loc1)=="number" then
			...
			elseif rsof.Table_Intersection(category_list,player_target_list1) then 
				local value_player=loc1>0 and tp or 1-tp
				selected_group={[value_player]=minct,[value_player+2]=maxct,[1-value_player]=0,[1-value_player+2]=0}			   
			--2.4.2.2.3. Select 
		...
		if list_type=="cost" then
			local cost_result=rscost.CostSolve(selected_group,category_string_list,category_fun,e,tp,eg,ep,ev,re,r,rp,used_group,used_count_list)   
		...
		if list_type~="cost" then 
			for _,category in pairs(category_list) do 
				...
				local info_card_or_group,info_count,info_player,info_loc_or_parama = nil,0,0,0
				...
				if is_player then 
					info_loc_or_parama = minct
				else
					if aux.GetValueType(selected_group)~="Group" then 
						info_loc_or_parama = info_loc_or_parama|(loc1|loc2)
					end
				end
				Duel.SetOperationInfo(0,category,info_card_or_group,info_count,info_player,info_loc_or_parama)
				if is_player and e:IsHasProperty(EFFECT_FLAG_PLAYER_TARGET) then 
					Duel.SetTargetPlayer(info_player)
					Duel.SetTargetParam(info_loc_or_parama)
				end
			end
		end
...
function rstg.TargetFilter(c,e,tp,eg,ep,ev,re,r,rp,used_group,used_count_list,target_list1,target_list2,...)
	...
	if target_list1 then
		local list_type,filter_card,filter_group,category_list,category_string_list,category_fun,select_hint,loc1,loc2,minct,maxct,except_fun = rstg.GetTargetAttribute(e,tp,eg,ep,ev,re,r,rp,target_list1)
		if rsof.Table_List(category_list,CATEGORY_RECOVER) or rsof.Table_List(category_list,CATEGORY_DAMAGE) then
			--no_use
	...
	if target_list2 then
		local player_list1={ CATEGORY_RECOVER,CATEGORY_DAMAGE,CATEGORY_DECKDES,CATEGORY_DRAW }
		local list_type,filter_card,filter_group,category_list,category_string_list,category_fun,select_hint,loc1,loc2,minct,maxct,except_fun = rstg.GetTargetAttribute(e,tp,eg,ep,ev,re,r,rp,target_list2)
		local target_fun=list_type=="target" and Duel.IsExistingTarget or Duel.IsExistingMatchingCard 
		local except_group=rsgf.GetExceptGroup(e,tp,eg,ep,ev,re,r,rp,except_fun)
		except_group:Merge(used_group2)
		if type(loc1)=="boolean" then 
			return not except_group:IsContains(c) and rstg.TargetFilter(c,e,tp,eg,ep,ev,re,r,rp,used_group2,used_count_list2,target_list2,...) 
		else 
			local minct2=(type(minct)~="number" or minct==0) and 1 or minct
			if rsof.Table_Intersection(category_list,player_list1) then 
				minct2=1
...
--cost solve
function rscost.CostSolve(selected_group,category_string_list,category_fun,e,tp,eg,ep,ev,re,r,rp,used_group,used_count_list)
	...
	if not category_string_list or #category_string_list==0 then return true end
	return rsop.operationcard(selected_group,category_string_list[1],REASON_COST,e,tp,eg,ep,ev,re,r,rp)
end
...
function rsop.operationcard(selected_group,category_string,reason,e,tp,eg,ep,ev,re,r,rp)
	local ct=0
	local og
	if type(selected_group)=="table" then 
		if category_string=="dr" then 
			for p=0,1 do 
		...
		elseif category_string=="rec" then 
			for p=0,1 do 
				local minct=selected_group[p]
				if minct>0 then 
			...
			Duel.RDComplete()
		elseif category_string=="dam" then
			for p=0,1 do 
				local minct=selected_group[p]
				if minct>0 then 
			...
			Duel.RDComplete()
		elseif category_string=="disd" then
			for p=0,1 do 
				local minct=selected_group[p]
				local maxct=selected_group[p+2]
				local dct=minct
	...
	if category_string=="des" then ct=Duel.Destroy(selected_group,reason)
	elseif category_string=="des_rm" then ct=Duel.Destroy(selected_group,reason,LOCATION_REMOVED)
	elseif category_string=="rm" then ct=Duel.Remove(selected_group,POS_FACEUP,reason)
	elseif category_string=="rm_d" then ct=Duel.Remove(selected_group,POS_FACEDOWN,reason)
	elseif category_string=="th" then 
	...
	elseif category_string=="td" or category_string=="te" then ct=Duel.SendtoDeck(selected_group,nil,2,reason)
	elseif category_string=="te_u" then ct=Duel.SendtoExtraP(selected_group,nil,reason)
	elseif category_string=="td_t" then ct=Duel.SendtoDeck(selected_group,nil,0,reason)
	elseif category_string=="td_b" then ct=Duel.SendtoDeck(selected_group,nil,1,reason)
	elseif category_string=="tg" then 
	...
	elseif category_string=="tg_r" then 
		ct=Duel.SendtoGrave(selected_group,reason|REASON_RETURN)
	elseif category_string=="dish" then
		ct=Duel.SendtoGrave(selected_group,reason|REASON_DISCARD)
	elseif category_string=="res" then
		ct=Duel.Release(selected_group,reason)
	elseif category_string=="con" then
		if Duel.GetControl(selected_group,tp) then
			 ct=Duel.GetOperatedGroup():GetCount()
		end
	elseif category_string=="con_ep" then
		if Duel.GetControl(selected_group,tp,PHASE_END,1) then
			 ct=Duel.GetOperatedGroup():GetCount()
		end
	elseif category_string=="pos" then ct=Duel.ChangePosition(selected_group,POS_FACEUP_DEFENSE,POS_FACEDOWN_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)
	elseif category_string=="pos_a" then ct=Duel.ChangePosition(selected_group,POS_FACEUP_ATTACK)
	elseif category_string=="pos_d" then ct=Duel.ChangePosition(selected_group,POS_FACEUP_DEFENSE)
	elseif category_string=="pos_dd" then ct=Duel.ChangePosition(selected_group,POS_FACEDOWN_DEFENSE)
	elseif category_string=="set" then
	...
	elseif category_string=="sp" then
	...
	for index,par in pairs(bufflist) do
		...
		if vtype=="string" and par~="reset" then
			...
			for k,codestring in pairs(stringlist) do
				...
				if rsof.Table_List(limitlist,mainstring) then
				...
				if rsof.Table_List(matlimitlist,mainstring) then
					e1=rsef.SV_CANNOT_BE_MATERIAL(reglist,mainstring,effectvaluelist[k],nil,reset,"cd")
				end
