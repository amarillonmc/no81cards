...
function c60001179.setop(e,tp,eg,ep,ev,re,r,rp)
	local tg=eg:Filter(c60001179.setter,nil)
...
function c60001179.kop(e,tp,eg,ep,ev,re,r,rp)
	local tg=eg:Filter(c60001179.kter,nil)
	for tc in aux.Next(tg) do
...
function c60001179.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g1>0 then
		...
		if Duel.SendtoHand(g3,nil,REASON_EFFECT)>0 and #g2>0 and e:GetLabel()>0 and Duel.SelectYesNo(tp,aux.Stringid(60001179,1)) then
...
function c60001179.hsop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		if Duel.SendtoHand(c,nil,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(c60001179.filter,tp,LOCATION_ONFIELD,0,1,nil,c) and #g>0 and Duel.SelectYesNo(tp,aux.Stringid(60001179,2)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
