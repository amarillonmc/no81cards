...
function cm.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingMatchingCard(cm.filter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) end
...
function cm.filter(c)
	return c:IsAttack(1320) and c:IsDefense(1450) and not c:IsAttribute(ATTRIBUTE_DARK) and not c:IsForbidden()
end
