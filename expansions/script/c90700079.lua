...
function cm.thfilter(c,race)
	return c:IsRace(race) and c:IsSetCard(0x13d) and c:IsAbleToHand()
end
...
function cm.bancon(e,tp,eg,ep,ev,re,r,rp)
	...
	local flag=true
	local all_race=RACE_ALL 
	local now_race=1
	while all_race>0 do
		if g:FilterCount(Card.IsRace,nil,now_race)>=2 then
			flag=false
		end
		if all_race==1 then
			all_race=0
		else
			all_race=bit.rshift(all_race,1)
		end
		now_race=bit.lshift(now_race,1)
	end
	return g:GetCount()>=3 and flag
end
function cm.banfilter(c,race)
	return bit.band(c:GetRace(),race)==0 and c:IsAbleToRemove() and c:IsType(TYPE_MONSTER)
end
function cm.bantg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		g:ForEach(
		function (c)
			race=bit.bor(race,c:GetRace())
		end)
		return Duel.IsExistingMatchingCard(cm.banfilter,tp,LOCATION_DECK,0,1,nil,race)
	end
...
function cm.banop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,cm.banfilter,tp,LOCATION_DECK,0,1,1,nil,race)
	if g:GetCount()>0 then
