...
function c28335633.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_TOGRAVE+CATEGORY_TOKEN+CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetRange(LOCATION_GRAVE)
	e2:SetCondition(c28335633.tkcon)
	e2:SetCost(aux.bfgcost)
	e2:SetTarget(c28335633.tktg)
	e2:SetOperation(c28335633.tkop)
	c:RegisterEffect(e2)
end
...
function c28335633.tkcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(Card.IsSummonLocation,1,nil,LOCATION_HAND)
end
function c28335633.tgfilter(c,tp)
	return c:IsSetCard(0x286) and c:IsAbleToGrave() and Duel.IsPlayerCanSpecialSummonMonster(tp,c:GetCode(),0x286,TYPES_TOKEN_MONSTER+TYPE_TUNER,2000,2000,4,RACE_AQUA,c:GetAttribute()) and c:IsType(TYPE_MONSTER)
end
function c28335633.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c28335633.tgfilter,tp,LOCATION_DECK,0,1,nil,tp) and Duel.GetMZoneCount(tp)>0 end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK)
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c28335633.tkop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tc=Duel.SelectMatchingCard(tp,c28335633.tgfilter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
	if not tc then return end
	local code=tc:GetCode()
	local attr=tc:GetAttribute()
	local fid=e:GetHandler():GetFieldID()
	local g=Group.CreateGroup()
	if Duel.SendtoGrave(tc,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_GRAVE) and Duel.GetMZoneCount(tp)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,code,0x286,TYPES_TOKEN_MONSTER+TYPE_TUNER,2000,2000,4,RACE_AQUA,attr) then
		local token=Duel.CreateToken(tp,28335634)
		local e1=Effect.CreateEffect(e:GetHandler())
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_CHANGE_CODE)
