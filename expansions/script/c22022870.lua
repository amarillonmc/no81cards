...
function c22022870.initial_effect(c)
	aux.AddMaterialCodeList(c,22020000)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsCode,22020000),aux.NonTuner(Card.IsSetCard,0xff1),1)
	c:EnableReviveLimit()
	...
	e4:SetValue(ATTRIBUTE_LIGHT+ATTRIBUTE_DARK)
	c:RegisterEffect(e4)
	--spsummon
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(22022870,0))
	e5:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetRange(LOCATION_MZONE)
	e5:SetCountLimit(1)
	e5:SetTarget(c22022870.sptg)
	e5:SetOperation(c22022870.spop)
	c:RegisterEffect(e5)
end
...
function c22022870.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.SelectOption(tp,aux.Stringid(22022870,1))
	Duel.SelectOption(tp,aux.Stringid(22022870,2))
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c22022870.espfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
		Duel.SelectOption(tp,aux.Stringid(22022870,3))
