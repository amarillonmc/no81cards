...
function c11513041.initial_effect(c)
	...
	e1:SetValue(function(e,lc,mg,c,tp)
	if e:GetHandler()~=lc then return false,nil end
	return true,not mg or not mg:IsExists(Card.IsControler,1,nil,1-tp) end)
	c:RegisterEffect(e1)
	--spsummon proc
	--local e1=Effect.CreateEffect(c)
	--e1:SetType(EFFECT_TYPE_FIELD)
	...
	return Duel.GetCurrentPhase()==PHASE_DAMAGE_CAL and bc and bc:IsSummonType(SUMMON_TYPE_SPECIAL) and bc:GetAttack()>0 end)
	e3:SetValue(function(e,c)
	local bc=e:GetHandler():GetBattleTarget()
	return bc:GetAttack() end)
	c:RegisterEffect(e3)
	local e3=Effect.CreateEffect(c) 
...
function c11513041.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return bc and bc:IsSummonType(SUMMON_TYPE_SPECIAL) and aux.NegateEffectMonsterFilter(bc)   
end 
...
function c11513041.tdcon(e,tp,eg,ep,ev,re,r,rp) 
	return e:GetHandler():IsSummonType(SUMMON_TYPE_LINK) and e:GetHandler():GetReasonPlayer()==1-tp 
end 
