...
function c20250339.initial_effect(c)
	...
	e6:SetCondition(c20250339.imcon)
	e6:SetOperation(c20250339.imop)
	c:RegisterEffect(e6)
end

function c20250339.imcon(e,tp,eg,ep,ev,re,r,rp)
...
function c20250339.filter(c,e,tp)
	return c:IsSetCard(0x54a) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
...
function c20250339.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		local counter_group=Duel.GetMatchingGroup(Card.IsCanAddCounter,tp,LOCATION_MZONE,0,nil,0x154a,1)
		if ct==0 or counter_group:GetCount()==0 then return end
		for i=1,ct do
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_COUNTER)
			local sg=counter_group:Select(tp,1,1,nil)
