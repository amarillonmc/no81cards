...
function s.filter2(c)
	return c:IsAbleToHand() and c:IsSetCard(0x607)
end
function s.prop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
	if Duel.GetLP(tp)-Duel.GetLP(1-tp)>=2000 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.filter2),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
