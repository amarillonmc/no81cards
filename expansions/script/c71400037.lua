...
function c71400037.filter1(c,e,tp)
...
function c71400037.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c71400037.filter1,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	local tg=1
	if Duel.IsExistingMatchingCard(c71400037.filter1a,tp,LOCATION_MZONE+LOCATION_GRAVE,LOCATION_MZONE+LOCATION_GRAVE,1,nil) then
		tg=2
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c71400037.filter1,tp,LOCATION_GRAVE,0,1,tg,nil,e,tp)
