--烈星壳·炎魔
function c189.initial_effect(c)
	--synchro summon
	...
	e1:SetCondition(c189.lzcon)
	e1:SetTarget(c189.lztg)
	e1:SetOperation(c189.lzop)
	c:RegisterEffect(e1)  
	--send to grave 2
	...
	e2:SetCost(c189.tgcost)
	e2:SetTarget(c189.tgtg)
	e2:SetOperation(c189.tgop)
	c:RegisterEffect(e2)
end
function c189.lzcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO)
end
function c189.lztg(e,tp,eg,ep,ev,re,r,rp,chk)
	local x=e:GetHandler():GetMaterialCount()
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,x,tp,LOCATION_DECK)
end
function c189.lzop(e,tp,eg,ep,ev,re,r,rp)
...
function c189.efilter1(e,te)
	return te:IsActiveType(TYPE_MONSTER)
end
function c189.efilter2(e,te)
	return te:IsActiveType(TYPE_SPELL)
end
function c189.efilter3(e,te)
...
function c189.tgcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c189.fil,tp,LOCATION_GRAVE,0,1,nil) end
	local g=Duel.SelectMatchingCard(tp,c189.fil,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c189.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c189.tgop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local cg=c:GetColumnGroup():Filter(Card.IsControler,nil,1-tp)
	...
	e1:SetOperation(c189.disop)
	e1:SetReset(RESET_PHASE+PHASE_END,2)
	Duel.RegisterEffect(e1,tp)
end
end
end
function c189.disop(e,tp)
	return e:GetLabel()
end
