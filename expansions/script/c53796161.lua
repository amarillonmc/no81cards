...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return Duel.IsPlayerCanDraw(tp,1) and not c:IsPublic() and Duel.IsExistingMatchingCard(Card.IsXyzSummonable,tp,LOCATION_EXTRA,0,1,nil,mg)
	end
	local e1=Effect.CreateEffect(c)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
		...
		Duel.XyzSummon(tp,sg:GetFirst(),mg,1,99)
	end
end
