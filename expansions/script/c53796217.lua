...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(function(c)return c:IsFaceup() and c:IsCode(53796216) and c:GetSequence()<5 and c:IsStatus(STATUS_EFFECT_ENABLED)end,tp,LOCATION_SZONE,0,nil)
	if #g==0 then
...
function s.act(e,tp,g)
	for tc in aux.Next(g) do
		...
		if tc:GetFlagEffect(id+499)==0 then
			tc:RegisterFlagEffect(id+499,RESET_EVENT+RESETS_STANDARD,0,1)
