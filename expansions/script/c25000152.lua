...
function cm.discon(e,tp,eg,ep,ev,re,r,rp)
	...
	local lab=e:GetHandler():GetFlagEffectLabel(m)
	if not lab then return false end
	if bit.band(lab,8)==0 then return false end
	local l=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	return re:IsActiveType(TYPE_MONSTER) and bit.band(l,LOCATION_HAND+LOCATION_GRAVE)~=0
end
