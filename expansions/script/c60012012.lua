...
function cm.filter(c)
	return c:IsLevelBelow(4) and c:IsAbleToGrave() and c:IsCanHaveCounter(0x62a) and Duel.IsCanAddCounter(tp,0x62a,1,c) and c:IsType(TYPE_MONSTER)
end
...
function cm.atg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(cm.afil,tp,LOCATION_HAND,0,1,nil) end
end
function cm.aop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SUMMON)
	local g=Duel.SelectMatchingCard(tp,cm.afil,tp,LOCATION_HAND,0,1,1,nil)
	if g:GetCount()>0 then
