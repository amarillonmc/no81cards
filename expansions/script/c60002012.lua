...
function cm.cvop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetTurnCount()+t>=4 and Duel.SelectYesNo(tp,aux.Stringid(m,1)) then
		Duel.SetLP(tp,4000)
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	local t=Duel.GetFlagEffect(tp,60002009)
	if Duel.GetTurnCount()+t>=4 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,cm.filter,tp,LOCATION_DECK,0,0,2,nil)
		if g:GetCount()>0 then
