...
function c60152315.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(60152315,1))
	...
	e99:SetDescription(aux.Stringid(60152315,2))
...
function c60152315.e1tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c60152315.e1tgfilter,tp,LOCATION_GRAVE+LOCATION_OVERLAY,LOCATION_GRAVE+LOCATION_OVERLAY,1,nil,e) and e:GetHandler():IsFaceup() and e:GetHandler():IsLocation(LOCATION_MZONE) end
...
function c60152315.e1op(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c60152315.e1tgfilter),tp,LOCATION_GRAVE+LOCATION_OVERLAY,LOCATION_GRAVE+LOCATION_OVERLAY,1,1,nil,e)
...
function c60152315.e2tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return ct>0 and Duel.IsExistingMatchingCard(c60152315.e2tgfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,turn) end
	local g=Duel.GetMatchingGroup(c60152315.e2tgfilter,tp,LOCATION_GRAVE,0,nil,e,tp,turn)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		if g:GetCount()>0 then
			...
			if Duel.IsPlayerAffectedByEffect(tp,60152321) then
				...
				e1:SetCode(EFFECT_IMMUNE_EFFECT)
				e1:SetRange(LOCATION_MZONE)
				e1:SetValue(c60152315.e99opfilter)
				e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+RESET_CHAIN)
