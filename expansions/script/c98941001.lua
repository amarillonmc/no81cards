...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and (Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_GRAVE+LOCATION_HAND,0,1,nil,e,tp) or Duel.IsPlayerAffectedByEffect(e:GetHandlerPlayer(),98941057))
		or e:IsCostChecked() and e:IsHasType(EFFECT_TYPE_ACTIVATE) and (Duel.CheckReleaseGroup(tp,s.cfilter,1,nil,tp) or Duel.IsPlayerAffectedByEffect(e:GetHandlerPlayer(),98941057))
			and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)) end
	if not e:IsCostChecked() and not Duel.IsPlayerAffectedByEffect(e:GetHandlerPlayer(),98941057) then e:SetLabel(0) end
