...
function s.initial_effect(c)
	...
	c:EnableReviveLimit()
	--[[When this card is Ritual Summoned: You can add, to your hand, 1 card that was banished this turn.]]
	...
	c:RegisterEffect(e1)
	--[[Each time a card(s) you control is destroyed by your opponent: Send the top card of your Deck to the GY for each.]]
	...
	c:RegisterEffect(e2)
	--[[If this card is destroyed by your opponent: You can Special Summon 1 "Edmund, Leg of MYTH-337" from your GY, but banish it when it leaves the field.]]
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,nil,Duel.GetTurnCount()) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,PLAYER_ALL,LOCATION_REMOVED)
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil,Duel.GetTurnCount())
	if #g>0 then
...
function s.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_DECKDES,nil,0,tp,val)
end
