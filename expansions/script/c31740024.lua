...
function s.filter(c,e)
	return c:IsFaceup() and c:IsType(TYPE_PENDULUM) and aux.IsCodeListed(c,31740001) and c~=e:GetHandler()-- 假设0xfe是只狼系列的系列代码
end
...
function s.extg(e, tp, eg, ep, ev, re, r, rp, chk)
	if chk == 0 then
		return Duel.IsExistingMatchingCard(s.filter, tp, LOCATION_EXTRA, 0, 2, nil,e)
	end
...
function s.exop(e, tp, eg, ep, ev, re, r, rp)
	...
	local g = Duel.SelectMatchingCard(tp, s.filter, tp, LOCATION_EXTRA, 0, 2, 2, nil,e)
	if g:GetCount() < 2 then return end
	
	-- 将选择的卡返回卡组
	if Duel.SendtoDeck(g, nil, SEQ_DECKSHUFFLE, REASON_EFFECT) > 0 and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.ShuffleDeck(tp)
		local c=e:GetHandler()
		local tcode=31740025
		Duel.MoveToField(c, tp, tp, LOCATION_PZONE, POS_FACEUP, true)
		c:SetEntityCode(tcode)
		c:ReplaceEffect(tcode,0)
