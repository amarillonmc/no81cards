...
function s.filter(c,tp)
	local g=Duel.GetMatchingGroup(s.tgfilter,tp,LOCATION_HAND+LOCATION_MZONE+LOCATION_EXTRA,0,nil,c:GetCode())
	return c:IsFaceup() and c:IsSetCard(0x9a7b) and g:GetClassCount(Card.GetCode)>=7
end
function s.tgfilter(c,code)
	return c:IsFaceupEx() and c:IsSetCard(0x9a7b) and not c:IsCode(code) and c:IsAbleToGrave()
end
function s.skitg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,7,tp,LOCATION_HAND+LOCATION_MZONE+LOCATION_EXTRA)
end
function s.skiop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if not (tc:IsRelateToEffect(e) and tc:IsFaceup()) then return end
	local tg=Duel.GetMatchingGroup(s.tgfilter,tp,LOCATION_HAND+LOCATION_MZONE+LOCATION_EXTRA,0,nil,tc:GetCode())
