	...
	if tc:IsRelateToEffect(e) and Duel.SendtoGrave(tc,REASON_EFFECT)>0 then
		...
		if g:GetCount()>0 then
			...
			if Duel.SpecialSummonStep(sc,nil,tp,tp,false,false,POS_FACEUP) then
				...
				sc:RegisterFlagEffect(115399521,RESET_EVENT+0x1fe0000,0,1)
...
function c115399521.mfilter1(c,tp,tc)
	return c:IsFaceup() and c:IsControler(tp) and c:IsOnField() and c:IsType(TYPE_TUNER) and Duel.GetLocationCountFromEx(tp,tp,Group.FromCards(c),tc)>0
end
function c115399521.mfilter2(c,e,tp,tc)
...
function c115399521.syncon1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsFacedown() and eg:IsExists(c115399521.mfilter1,1,nil,tp,c) and Duel.IsExistingMatchingCard(c115399521.mfilter2,tp,LOCATION_DECK,0,1,nil,e,tp,c) and not Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_MZONE+LOCATION_SZONE,0,1,nil,115399521) and re:GetHandler():IsSetCard(0x1066) and re:GetHandler():IsType(TYPE_MONSTER)
end
function c115399521.synop1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsCanBeSpecialSummoned(e,nil,tp,true,false) then return end
	if Duel.GetFlagEffect(tp,115399521)==0 and Duel.SelectYesNo(tp,aux.Stringid(115399521,2)) then
		...
		local g=Duel.SelectMatchingCard(tp,c115399521.mfilter2,tp,LOCATION_DECK,0,1,1,nil,e,tp,c)
		if g:GetCount()>0 and Duel.SpecialSummon(g,nil,tp,tp,false,false,POS_FACEUP)>0 then
			...
			Duel.SpecialSummon(c,nil,tp,tp,true,false,POS_FACEUP)
			local e1=Effect.CreateEffect(c)
			e1:SetType(EFFECT_TYPE_SINGLE)
