...
function c22022770.initial_effect(c)
	...
	e6:SetDescription(aux.Stringid(22022770,3))
	e6:SetCategory(CATEGORY_CONTROL)
	e6:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_SPSUMMON_SUCCESS)
	e6:SetRange(LOCATION_GRAVE)
	e6:SetCountLimit(1,22022770)
	e6:SetCondition(c22022770.erecon)
	e6:SetCost(c22022770.erecost)
	e6:SetTarget(c22022770.target)
	e6:SetOperation(c22022770.operation)
	c:RegisterEffect(e6)
	local e7=e6:Clone()
	...
	e8:SetDescription(aux.Stringid(22022770,4))
	e8:SetType(EFFECT_TYPE_QUICK_O)
	e8:SetCode(EVENT_CHAINING)
	e8:SetRange(LOCATION_GRAVE)
	e8:SetCountLimit(1,22022771)
	e8:SetCondition(c22022770.chcon1)
	e8:SetCost(c22022770.erecost)
	e8:SetTarget(c22022770.chtg)
	e8:SetOperation(c22022770.chop)
	c:RegisterEffect(e8)
end
...
function c22022770.filter1(c,e,sp)
	return c:IsSetCard(0x6ff1) and c:IsCanBeSpecialSummoned(e,0,sp,false,false)
end
...
function c22022770.erecon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsPlayerAffectedByEffect(tp,22020980)
end
function c22022770.erecost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.Hint(HINT_CARD,0,22020980)
	Duel.PayLPCost(tp,math.floor(Duel.GetLP(tp)/2))
end
function c22022770.chcon1(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and (re:IsActiveType(TYPE_MONSTER)
		or (re:GetActiveType()==TYPE_SPELL or re:GetActiveType()==TYPE_TRAP) and re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsPlayerAffectedByEffect(tp,22020980)
end