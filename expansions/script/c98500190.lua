--异种 泰坦
function c98500190.initial_effect(c)
	...
	e1:SetCountLimit(1,98500190)
	e1:SetCondition(c98500190.condition)
	e1:SetCost(c98500190.cost)
	e1:SetTarget(c98500190.target)
	e1:SetOperation(c98500190.operation)
	c:RegisterEffect(e1)
	--spsummon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(98500190,1))
	...
	e2:SetTarget(c98500190.hsptg)
	e2:SetOperation(c98500190.hspop)
	c:RegisterEffect(e2)
	--flip
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(98500190,2))
	e3:SetCategory(CATEGORY_TODECK+CATEGORY_MSET)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_FLIP+EFFECT_TYPE_TRIGGER_O)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,98500192)
	e3:SetTarget(c98500190.destg)
	e3:SetOperation(c98500190.desop)
	c:RegisterEffect(e3)
	--tribute limit
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetCode(EFFECT_TRIBUTE_LIMIT)
	e4:SetValue(c98500190.tlimit)
	c:RegisterEffect(e4)
	--summon with 3 tribute
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(98500190,2))
	e5:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e5:SetType(EFFECT_TYPE_SINGLE)
	e5:SetCode(EFFECT_LIMIT_SUMMON_PROC)
	e5:SetCondition(c98500190.ttcon)
	e5:SetOperation(c98500190.ttop)
...
function c98500190.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end 
	local op=Duel.SelectOption(tp,aux.Stringid(98500190,9),aux.Stringid(98500190,10))
	if op==0 then
	...
	if Duel.ConfirmCards(tp,g2)~=0 then
		...
		if  Duel.SelectYesNo(tp,aux.Stringid(98500190,11)) then
			...
			if Duel.IsExistingMatchingCard(c98500190.filter,tp,LOCATION_DECK,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then
				ts[index]=aux.Stringid(98500190,3)
				index=index+1
			end
			if Duel.IsExistingMatchingCard(c98500190.filter2,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_EXTRA,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
			   ts[index]=aux.Stringid(98500190,4)
			   index=index+1
			end
			local c=e:GetHandler()
			local opt=Duel.SelectOption(tp,table.unpack(ts))
			if ts[opt+1]==aux.Stringid(98500190,3) then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
				local g=Duel.SelectMatchingCard(tp,c98500190.filter,tp,LOCATION_DECK,0,1,1,nil)
				local tc=g:GetFirst()
				Duel.SSet(tp,tc)
			...
			elseif ts[opt+1]==aux.Stringid(98500190,4) then
...
function c98500190.hsptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if Duel.IsPlayerAffectedByEffect(tp,98500080) then
		...
		local g=Duel.SelectTarget(tp,c98500190.filter3,tp,LOCATION_MZONE,0,1,1,nil)
	end
	Duel.SetOperationInfo(0,CATEGORY_POSITION,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SUMMON,nil,1,0,0)
end
function c98500190.filter4(c)
	return c:IsFacedown()
end
function c98500190.hspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetHandler():IsSummonable(true,nil) then
		ts[index]=aux.Stringid(98500190,7)
		index=index+1
	end
	if e:GetHandler():IsMSetable(true,nil) then
	   ts[index]=aux.Stringid(98500190,8)
	   index=index+1
	end
	local c=e:GetHandler()
	local opt=Duel.SelectOption(tp,table.unpack(ts))
	if ts[opt+1]==aux.Stringid(98500190,7) then
		Duel.Summon(tp,c,true,nil)
	elseif ts[opt+1]==aux.Stringid(98500190,8) then
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() then
		if Duel.ChangePosition(tc,POS_FACEDOWN_DEFENSE)~=0 then
			...
			e1:SetTarget(c98500190.tg)
			e1:SetValue(c98500190.efilter)
			e1:SetReset(RESET_PHASE+PHASE_END)
			Duel.RegisterEffect(e1,tp)
		end
	end
end
function c98500190.tg(e,c)
	return c:IsFacedown()
end
function c98500190.efilter(e,te)
	return te:GetHandler():GetControler()~=e:GetHandlerPlayer()
end
function c98500190.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,0,0)
end
function c98500190.desop(e,tp,eg,ep,ev,re,r,rp)
...
function c98500190.tlimit(e,c)
	return not c:IsPosition(POS_FACEDOWN)
end
function c98500190.ttcon(e,c,minc)
	if c==nil then return true end
	return minc<=3 and Duel.CheckTribute(c,3)
end
function c98500190.ttop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.SelectTribute(tp,c,3,3)
	c:SetMaterial(g)
	Duel.Release(g,REASON_SUMMON+REASON_MATERIAL)
end
function c98500190.postg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c98500190.filter3(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c98500190.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_POSCHANGE)
	local g=Duel.SelectTarget(tp,c98500190.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_POSITION,g,1,0,0)
end
function c98500190.posop2(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetTargetsRelateToChain():Filter(c98500190.filter3,nil)
