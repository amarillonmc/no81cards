...
function s.spfilter(c)
	return c:IsSetCard(0x9569) and c:IsAbleToDeck() and c:IsFaceupEx()
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		if Duel.SendtoDeck(g,nil,2,REASON_EFFECT)>0 then
			...
			if math.abs(lp-lp2)>=2000 and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_ONFIELD,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
				Duel.BreakEffect()
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
