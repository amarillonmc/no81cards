...
function s.initial_effect(c)
	...
	e1b:SetHintTiming(0,TIMING_END_PHASE)
	...
	e2b:SetHintTiming(0,TIMING_END_PHASE)
	e2b:SetCondition(s.quickcon)
	c:RegisterEffect(e2b)
end

function s.quick_filter(c)
	return c:IsAttribute(ATTRIBUTE_WATER) and c:IsType(TYPE_SYNCHRO+TYPE_LINK)
		and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
function s.has_quick_condition(tp)
	return Duel.IsExistingMatchingCard(s.quick_filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil)
end
function s.igncon(e,tp,eg,ep,ev,re,r,rp)
	return not s.has_quick_condition(tp)
end
function s.quickcon(e,tp,eg,ep,ev,re,r,rp)
	return s.has_quick_condition(tp)
end
