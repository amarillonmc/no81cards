...
function cm.actop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ffcount>=4 and Duel.IsExistingMatchingCard(cm.actfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(90700007,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
		local g=Duel.SelectMatchingCard(tp,cm.actfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
		if g:GetCount()>0 then
			local sc=g:GetFirst()
			if sc:IsAbleToRemove() and Duel.SelectOption(tp,aux.Stringid(90700007,1),aux.Stringid(90700007,2))==1 then
