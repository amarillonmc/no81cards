...
function cm.thfilter(c)
	return c:IsSetCard(0x6343,0x5343)  and c:IsAbleToHand()
end
function cm.spfilter(c,e,tp)
	return  c:IsSetCard(0x5342) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function cm.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 then
	   if  Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)~=0 and Duel.IsExistingMatchingCard(cm.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,0)) then 
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
		local g1=Duel.SelectMatchingCard(tp,nil,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,1,e:GetHandler())
