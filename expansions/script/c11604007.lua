...
function s.filter(c)
	return c:IsType(TYPE_QUICKPLAY) and c:IsSetCard(0x9224) and c:IsFaceup()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) and Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end  
