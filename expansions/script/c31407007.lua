...
function cm.serop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsExistingMatchingCard(cm.serfilter,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
...
function cm.serop2(e,tp,eg,ep,ev,re,r,rp)
	if r&REASON_SYNCHRO~=0 and Duel.IsExistingMatchingCard(cm.serfilter,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,cm.serfilter,tp,LOCATION_DECK,0,1,1,nil)
		if g then
			Duel.SendtoHand(g,tp,REASON_EFFECT)
