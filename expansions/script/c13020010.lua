...
function cm.ffilter(c, c2)
	return c:CheckEquipTarget(c2) and c2:IsFaceup()
end
...
function cm.operation(e, tp, eg, ep, ev, re, r, rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsCanBeDisabledByEffect(e) then
		...
		local tcg = Duel.SelectMatchingCard(tp, cm.filter2, tp, oc, 0, 1, 1, nil, tc):GetFirst()
...
function cm.operation2(e, tp, eg, ep, ev, re, r, rp)
	...
	if Duel.Recover(tp, 200, REASON_EFFECT) > 0 then
		if num >= 6 and #g2 > 0 and zzx > 0 and xg.ky(tp, m, 1) then --g1 ~= nil and
