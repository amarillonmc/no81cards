...
function c22020700.filter1(c,e,tp)
	...
	return c:IsFaceup()
		...
		and Duel.IsExistingMatchingCard(c22020700.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,c:GetRank()+1,no)
end
function c22020700.filter2(c,e,tp,mc,rk,no)
...
function c22020700.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c22020700.filter1,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
...
function c22020700.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c22020700.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank()+1,no)
	local sc=g:GetFirst()
	if sc then
...
function c22020700.costfilter(c,e,tp,att,mc)
	return c:IsType(TYPE_TOKEN) and c:IsReleasable() and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
end
