...
function s.thfilter(c,e,tp)
	return c:IsSetCard(0xca7d) and c:GetLevel()~=2 and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function s.desfilter(c)
    return c:IsSetCard(0xCA7D) and c:IsType(TYPE_MONSTER)
end
function s.sphtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.desfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,nil,1,tp,LOCATION_HAND+LOCATION_DECK)
end
function s.sphop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local sg=Duel.SelectMatchingCard(tp,s.desfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil)
	if #sg>0 then
