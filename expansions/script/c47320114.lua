...
function s.sprule(c)
    c:EnableReviveLimit()
	aux.AddFusionProcFunRep(c,s.mfilter1,2,true)
end
function s.mfilter1(c)
    return c:IsType(TYPE_MONSTER) and c:IsRace(RACE_ILLUSION) and c:IsFusionAttribute(ATTRIBUTE_WATER)
end
function s.select(c)
...
function s.thfilter(c)
	return c:IsSetCard(0x6c12) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
...
function s.reftg(e,c)
	return c:IsFaceup() and c:IsCode(47320100)
end
