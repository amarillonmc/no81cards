...
function s.removen(c,tp,n)
	if Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)<n or Duel.GetDecktopGroup(tp,n):FilterCount(Card.IsAbleToRemove,nil)<n then return end
	...
	if Duel.Remove(g,POS_FACEUP,REASON_COST+REASON_TEMPORARY)>0 then
		...
		for tc in aux.Next(og) do
			...
			--tc:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,0,aux.Stringid(id,1))
			local e1=Effect.CreateEffect(c)
...
function s.drcon1(e,tp,eg,ep,ev,re,r,rp)
	...
	return eg:IsExists(s.spfilter,1,nil,1-tp)
		and not Duel.IsChainSolving() and c:IsFaceup()
end
function s.drop1(e,tp,eg,ep,ev,re,r,rp)
	s.removen(e:GetHandler(),tp,2)
end
function s.regcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	--if not c:IsFaceup() then return false end
	return eg:IsExists(s.spfilter,1,nil,1-tp)
		and Duel.IsChainSolving() and c:IsFaceup()
end
...
function s.drcon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	--if not c:IsFaceup() then return false end
	return Duel.GetFlagEffect(tp,id)>0 and c:IsFaceup()
end
function s.drop2(e,tp,eg,ep,ev,re,r,rp)
	local n=Duel.GetFlagEffect(tp,id)
	Duel.ResetFlagEffect(tp,id)
	s.removen(e:GetHandler(),tp,2*n)
end