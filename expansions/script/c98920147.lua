...
function cm.tgsfilter(c,e,tp)
	return c:IsSetCard(0x152) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
end
...
function cm.srfilter(c)
	return c:IsSetCard(0x1151,0x2151) and c:IsType(0x1) and c:IsAbleToHand()
end
...
function cm.srop(e,tp,eg,ep,ev,re,r,rp)
    if Duel.IsChainDisablable(0) and Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,0,0x02,1,nil) and Duel.SelectYesNo(1-tp,aux.Stringid(m,1)) then
