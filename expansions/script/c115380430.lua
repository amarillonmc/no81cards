...
function c115380430.initial_effect(c)
    ...
    e3:SetCondition(c115380430.descon)
    ...
    e4:SetCondition(c115380430.rmcon)
    e4:SetCost(c115380430.rmcost)
    e4:SetTarget(c115380430.rmtg)
    e4:SetOperation(c115380430.rmop)
    c:RegisterEffect(e4)
end
...
function c115380430.descon(e,tp,eg,ep,ev,re,r,rp)
    return re and re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSetCard(0x105)
end
function c115380430.desfilter(c)
    return c:IsDestructable()
end
function c115380430.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chkc then return chkc:IsOnField() and c115380430.desfilter(chkc) end
    if chk==0 then return Duel.IsExistingTarget(c115380430.desfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
    local g=Duel.SelectTarget(tp,c115380430.desfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
...
function c115380430.rmcon(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetTurnCount()==e:GetHandler():GetTurnID()+1
end
function c115380430.rmcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return e:GetHandler():IsAbleToDeckAsCost() end
    Duel.SendtoDeck(e:GetHandler(),tp,2,REASON_COST)
end
function c115380430.rmfilter(c)
    return c:IsRace(RACE_WYRM) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsAbleToHand()
end
function c115380430.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c115380430.rmfilter,tp,LOCATION_DECK,0,1,nil) end
    Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c115380430.rmop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
    local g=Duel.SelectMatchingCard(tp,c115380430.rmfilter,tp,LOCATION_DECK,0,1,1,nil)
