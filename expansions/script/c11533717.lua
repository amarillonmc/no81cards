...
function c11533717.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c11533717.xtdfil,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) end 
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,2,tp,LOCATION_GRAVE)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c11533717.spop(e,tp,eg,ep,ev,re,r,rp) 
	local c=e:GetHandler()
	if Duel.IsExistingMatchingCard(c11533717.xtdfil,tp,LOCATION_GRAVE,0,1,e:GetHandler(),e,tp) then 
		local g=Duel.SelectMatchingCard(tp,c11533717.xtdfil,tp,LOCATION_GRAVE,0,1,1,e:GetHandler(),e,tp) 
		g:AddCard(c) 
