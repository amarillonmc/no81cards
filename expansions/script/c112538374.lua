function c112538374.initial_effect(c)
    ...
    e3:SetOperation(c112538374.retreg)
    c:RegisterEffect(e3)
    local e4=Effect.CreateEffect(c)
    e4:SetCategory(CATEGORY_TODECK)
    e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
    e4:SetRange(LOCATION_REMOVED)
    e4:SetCountLimit(1,98725201)
...
function c112538374.retreg(e,tp,eg,ep,ev,re,r,rp)
    e:GetHandler():RegisterFlagEffect(112538374,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,2)
end
function c112538374.retcon(e,tp,eg,ep,ev,re,r,rp)
    return tp~=Duel.GetTurnPlayer() and e:GetHandler():GetFlagEffect(112538374)>0
end
...
function c112538374.rettg(e,tp,eg,ep,ev,re,r,rp,chk)
    local c=e:GetHandler()
    if chk==0 then return Duel.IsExistingMatchingCard(c112538374.retfilter,tp,LOCATION_GRAVE,0,1,nil) end
