...
function s.setfilter(c,tp)
	if not (c:IsCode(3055018) and not c:IsForbidden()) then return false end
	for p=0,1 do
		if c:CheckUniqueOnField(p) and c:IsCanPlaceInFieldZone(tp,p) then
			return true
		end
	end
	return false
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local p=tp
	local b1=tc:CheckUniqueOnField(tp) and tc:IsCanPlaceInFieldZone(tp,tp)
	local b2=tc:CheckUniqueOnField(1-tp) and tc:IsCanPlaceInFieldZone(tp,1-tp)
