...
function c87498781.pspfil(c,e,tp) 
	...
	local lv=c:GetOriginalLevel()   
	 return c:IsSetCard(0x143) and c:IsType(TYPE_TRAP) and c:IsType(TYPE_CONTINUOUS) and Duel.IsPlayerCanSpecialSummonMonster(tp,c:GetCode(),0x143,TYPES_NORMAL_TRAP_MONSTER,c:GetTextAttack(),c:GetTextDefense(),c:GetOriginalLevel(),c:GetOriginalRace(),c:GetOriginalAttribute()) and lv>0 and lv>lsc and lv<rsc   
end 
...
function c87498781.pspop(e,tp,eg,ep,ev,re,r,rp)
	...
	tc:AddMonsterAttribute(TYPE_NORMAL+TYPE_TRAP,at,rc,lv,atk,def)
...
function c87498781.filsn(c)
	return c:IsCode(95440946) and not c:GetFlagEffectLabel(87498781)
end
