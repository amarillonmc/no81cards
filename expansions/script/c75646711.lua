...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_RECOVER+CATEGORY_ATKCHANGE+CATEGORY_SEARCH+CATEGORY_TOHAND)
...
function s.tgfilter(c)
	return c:IsSetCard(0x52c1) and c:IsAbleToGrave()
end
...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if g:GetCount()>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) and e:GetLabel()==1 then
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.BreakEffect()
