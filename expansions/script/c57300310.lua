...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and s.etfilter(chkc) and chkc~=e:GetHandler() end
	if chk==0 then return Duel.IsExistingTarget(s.etfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.etfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,e:GetHandler())
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
		...
		if g:IsExists(Card.IsSetCard,1,nil,0x9520) and g:IsExists(aux.NOT(Card.IsSetCard),1,nil,0x9520) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
			local sc=g:FilterSelect(tp,Card.IsSetCard,1,1,nil,0x9520)
