...
function s.costfilter(c,e,tp)
	local g=Duel.GetMatchingGroup(Card.IsType,tp,LOCATION_MZONE,LOCATION_MZONE,c,TYPE_LINK)
	local zone=0
	for tc in aux.Next(g) do zone=zone|(tc:GetLinkedZone()) end
	return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,zone,c:GetBaseAttack(),c) and (c:IsControler(tp) or c:IsFaceup())
end
function s.filter(c,e,tp,zone,atk,tc)
	return c:GetBaseAttack()<atk and Duel.GetLocationCountFromEx(tp,tp,tc,c) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,tp,zone)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local zone=Duel.GetLinkedZone(tp)
	if chk==0 then return e:IsCostChecked() and Duel.CheckReleaseGroup(tp,s.costfilter,1,nil,e,tp) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,zone,e:GetLabel(),nil):GetFirst()
