...
function s.hspfilter(c,tp,sc)
	return c:IsType(TYPE_NORMAL) and c:IsLevel(6) and c:IsRace(RACE_AQUA) and c:IsControler(tp) and Duel.GetLocationCountFromEx(tp,tp,c,sc)>0
...
function s.pcfilter(c)
	return c:IsType(TYPE_PENDULUM) and c:IsAttribute(ATTRIBUTE_WATER) and c:IsFacedown() and not c:IsForbidden()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.Destroy(c,REASON_EFFECT)~=0 then
		...
		if g:GetCount()>0 then
			...
			if g:GetFirst():GetLocation()==LOCATION_HAND and (Duel.CheckLocation(tp,LOCATION_PZONE,0) or Duel.CheckLocation(tp,LOCATION_PZONE,1))
		and Duel.IsExistingMatchingCard(s.pcfilter,tp,LOCATION_EXTRA,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1))then
				Duel.BreakEffect()
...
function s.pcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.AND(Card.IsFaceup,Card.IsCode),tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,9540040)
end
...
function s.discon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsDestructable() and Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,c) and Duel.IsExistingMatchingCard(aux.AND(Card.IsFaceup,Card.IsCode),tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,72929454) and Duel.GetFlagEffect(tp,id)==0
end
