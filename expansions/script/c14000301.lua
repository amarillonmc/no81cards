...
function cm.fpop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFacedown() then
		if Duel.ChangePosition(tc,POS_FACEUP_DEFENSE)~=0 and tc:IsFaceup() and Duel.IsExistingMatchingCard(function(c) return c:IsAbleToHand() and c:IsCode(tc:GetCode()) end,tp,LOCATION_DECK,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(m,3)) then
			Duel.BreakEffect()
