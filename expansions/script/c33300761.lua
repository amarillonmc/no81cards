...
function s.costfil(c,g)
	return c:IsReleasable() and g:IsContains(c) and not c:IsStatus(STATUS_BATTLE_DESTROYED)
end
function s.rehcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local lg=e:GetHandler():GetLinkedGroup()
	local c=e:GetHandler()
	local b1=Duel.IsExistingMatchingCard(s.costfil,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,lg)
	...
	if b1 and (not b2 or Duel.SelectOption(tp,aux.Stringid(id,1),aux.Stringid(id,2))==0)then
		local g=Duel.SelectMatchingCard(tp,s.costfil,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,c,lg)
