...
function cm.thfilter(c,e,tp)
	...
	return cm.blazed(c) and (c:IsAbleToHand() or (ft>0 and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)))
end
function cm.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return (c:GetFlagEffect(m)<1 or (c:IsHasEffect(m) and c:GetFlagEffect(m)<2)) and Duel.IsExistingMatchingCard(cm.thfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
...
function cm.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc then
		if tc:IsAbleToHand() and (not (tc:IsType(TYPE_MONSTER) and tc:IsCanBeSpecialSummoned(e,0,tp,false,false)) or ft<=0 or Duel.SelectOption(tp,1190,1152)==0) then
