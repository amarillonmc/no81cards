...
function cm.ntrtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsControlerCanBeChanged,tp,0,LOCATION_MZONE,1,nil) and c:IsSSetable() end
...
function cm.ntrop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and c:IsSSetable() and not tc:IsImmuneToEffect(e) and tc:IsControlerCanBeChanged() then
		Duel.SSet(tp,c)
		if tc:IsFaceup() then
			local e1=Effect.CreateEffect(c)
			e1:SetType(EFFECT_TYPE_SINGLE)
