...
function c9911461.initial_effect(c)
	...
	e4:SetOperation(c9911461.adjustop)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	e7:SetCountLimit(1,9911462)
	e7:SetCondition(c9911461.thcon2)
	e7:SetTarget(c9911461.thtg2)
	e7:SetOperation(c9911461.thop2)
	c:RegisterEffect(e7)
	local e8=e7:Clone()
...
function c9911461.adjustop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetFlagEffect(tp,9911473)~=0 then
		e1:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_CANNOT_INACTIVATE+EFFECT_FLAG_CAN_FORBIDDEN)
	else
		e1:SetProperty(EFFECT_FLAG_DELAY)
	end
end
function c9911461.scon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFlagEffect(tp,9911474)~=0
end
